...
The left side of the chart displays the top three referrers that visitors are arriving from. All other referrers are aggregated under the other referrals path. Each path displays a total number of views that came from that traffic source. A total number of direct traffic views is also displayed.

The different paths converge to a total number shown in the center of the chart.

The right side of the chart displays the top three site pages users navigated to. The other site pages users navigated to are aggregated under the other pages path. Any users that ended their session from this page (i.e. navigated out of your site or became inactive for over thirty minutes) are shown in the drop-off path.
