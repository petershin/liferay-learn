...
1. 조직의 API 기본 사항](./liferay-w2h3.zip)을 다운로드하여 압축을 풉니다.
...
1. cURL 스크립트를 사용하여 인스턴스에 새 조직을 추가합니다. 명령줄에서 `컬` 폴더로 이동합니다. `Organization_POST_ToInstance.sh` 스크립트를 실행합니다.
...
| 라인(약어)                                                                             | 묘사                                                                |
|:---------------------------------------------------------------------------------- |:----------------------------------------------------------------- |
| `OrganizationResource.Builder builder = ...`                                       | `OrganizationResource` 서비스 인스턴스를 생성하기 위해 `Builder` 가져옵니다.         |
| `OrganizationResource organizationResource = builder.authentication(...).build();` | 기본 인증을 지정하고 `OrganizationResource` 서비스 인스턴스를 생성합니다.               |
| `Organization organization = organizationResource.postOrganization(...);`          | `organizationResource.postOrganization` 메소드를 호출하고 게시할 데이터를 전달합니다. |
...
```{important}
[OrganizationResource](https://github.com/liferay/liferay-portal/blob/[$LIFERAY_LEARN_PORTAL_GIT_TAG$]/modules/apps/headless/headless-admin-user/headless-admin-user-client/src/main/java/com 참조) /liferay/headless/admin/user/client/resource/v1_0/OrganizationResource.java) 서비스 세부정보를 확인하세요.
```
...
```{tip}
인스턴스 ``Organization`` ID를 얻으려면 ``Organizations_GET_FromInstance.[java|sh]``를 사용하십시오.
```
...
다음 cURL 및 Java 명령을 사용하여 기존 조직을 완전히 덮어씁니다. `1234` 조직의 ID로 바꿉니다.
...
[API Explorer](../../headless-delivery/consuming-apis/consuming-rest-services.md) 에는 `Organization` 서비스와 스키마가 모두 표시되며 각 서비스를 시험해 볼 수 있는 인터페이스가 있습니다.
