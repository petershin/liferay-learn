...
In older versions, widget pages provided exclusive features, such as custom layouts and customizable columns. Many of these exclusive features are available on [content pages](../using-content-pages.md) in 7.3 and above, so in most cases, you'll want to create a content page.
...
```{note}
While portlets are rendered according to [render-weight](https://resources.learn.liferay.com/reference/latest/en/dxp/definitions/liferay-portlet-app_7_4_0.dtd.html#render-weight) on widget pages, that is not true for content pages. Portlets are rendered in the order they appear on the page on content pages (i.e. left to right, top to bottom), so you may notice some portlets are rendered sooner or later, depending on their placement on the content page.
```
...
1. Enter this script in the script window, making sure to replace the Group ID with your own. The Groovy script uses the [`BulkLayoutConverter` interface](https://resources.learn.liferay.com/reference/latest/en/dxp/javadocs/modules/apps/layout/com.liferay.layout.api/com/liferay/layout/util/BulkLayoutConverter.html) to convert all widget pages with the given Group ID to content pages:
