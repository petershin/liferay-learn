...
**Order Terms By:** Liferay 7.4 U54+/GA54+では、ファセットのタームソート戦略を選択することができます。Term Frequency Descending (デフォルト)、Term Frequency Ascending、Term Value Ascending、Term Value Descendingのいずれかを選択できます。 デフォルトのソートでは、ファセット用語のマッチ数が多いものから少ないものへと並べ替えられます。 用語値オプションは、ファセット用語をアルファベット順に並べる。
