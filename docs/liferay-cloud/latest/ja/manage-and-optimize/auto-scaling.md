...
この機能を使用して、サービスはLiferay DXPインスタンスの数を [定義された最大数](#setting-the-maximum-number-of-instances) （デフォルトでは10）まで自動的に増加（アップスケール）したり、[`LCP.json`](../reference/configuration-via-lcp-json.md)の`スケール`プロパティで指定された数まで減少（ダウンスケール）したりできます。`scale`プロパティは、実行するインスタンスの最小数を指定します。
