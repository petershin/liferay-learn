...
![ドロップダウンメニューの各オプションは、スタイルブックのトークンの1つのカテゴリに対応しています。](./frontend-token-definitions/images/01.png)
...
![ボタンプライマリトークンセットには、標準テーマにおけるメインボタンのカスタマイズ可能な色がすべて含まれています。](./frontend-token-definitions/images/02.png)
...
`type`：トークンが表示するデータのタイプ。 `"Integer"`、`"Float"`、または`"String"`を使用して、これらのタイプの値を保持するテキストフィールドを表示します。 チェックボックスを表示するには、`"Boolean"`を使用します。 `"Length"` を使用して、ピクセル、エムズ、パーセンテージなどのカスタマイズ可能な測定単位を提供します。
