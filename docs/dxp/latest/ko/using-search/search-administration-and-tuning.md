---
toc:
- ./search-administration-and-tuning/synonym-sets.md
- ./search-administration-and-tuning/result-rankings.md
- ./search-administration-and-tuning/elasticsearch-indexes-reference.md
- ./search-administration-and-tuning/search-administration.md
---
# 검색 관리 및 조정

```{toctree}
:maxdepth: 2

search-administration-and-tuning/synonym-sets.md
search-administration-and-tuning/result-rankings.md
search-administration-and-tuning/elasticsearch-indexes-reference.md
search-administration-and-tuning/search-administration.md
```

* [검색 관리 패널 개요](./search-administration-and-tuning/search-administration.md)
- 현지화된 검색 이해하기(출시 예정)

## 튜닝 검색

* [동의어 세트](search-administration-and-tuning/synonym-sets.md)
* [결과 순위](search-administration-and-tuning/result-rankings.md)


## 검색 문제 해결

* [검색 인사이트](search-pages-and-widgets/search-insights.md)
- Elasticsearch 로깅 조정(출시 예정)

## 참조

* [Elasticsearch 인덱스 참조](search-administration-and-tuning/elasticsearch-indexes-reference.md)
