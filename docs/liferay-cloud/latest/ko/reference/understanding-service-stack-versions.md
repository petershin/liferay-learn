...
모든 Liferay Cloud 환경은 서비스의 Docker 이미지 버전을 다양한 위치에 표시합니다. 환경의 _개요_ 또는 _서비스_ 페이지 또는 각 개별 서비스의 페이지를 포함하여 이러한 위치 중 하나에서 표시된 이미지 이름 끝에 있는 번호를 검사하여 환경의 서비스 스택 버전을 확인할 수 있습니다.

![환경으로 이동한 후 표시되는 첫 번째 페이지는 서비스 버전을 보여주는 페이지입니다.](./understanding-service-stack-versions/images/01.png)

## 리포지토리에서 서비스 스택 버전 확인

Docker 이미지 버전은 서비스 스택이 4.xx로 업그레이드되었는지 여부에 따라 다른 위치에 정의됩니다. 이 때문에 각 버전마다 다른 위치를 확인해야 합니다. 4.xx로 업그레이드하기 전과 후의 차이점에 대한 자세한 내용은 [버전 4](./liferay-cloud-project-changes-in-version-4.md)의 Liferay Cloud 프로젝트 변경 사항을 참조하십시오.
