...
```{note}
If you set defaults for all channels and for individual channels, the individual defaults takes priority if eligible.
```
...
```{note}
Users with this permission appear in the channel account manager dropdown. For the user to view and manage orders, you must also enable order related permissions. 
