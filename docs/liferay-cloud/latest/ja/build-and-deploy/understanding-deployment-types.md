...
* Liferay_サービスと_Backup_サービスは、NFSに保存されているドキュメントライブラリへのアクセスを共有できるように、**Deployment**タイプを使用します。

* _Database_サービスと _Web Server_サービスも**Deployment**型を使用しているため、軽量なまま迅速に起動することができます。

* _CI_サービスは**StatefulSet**型を使用し、専用のSSDを持つことによるファイルアクセス速度の向上を利用できる。

* また、_Search_サービスも**StatefulSet**型を使用することで、検索速度を向上させ、サービスが再起動してもSSDに保存された検索インデックスが失われないようにしている。
