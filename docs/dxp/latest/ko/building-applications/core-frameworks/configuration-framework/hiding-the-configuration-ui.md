...
모든 상황에서 구성 UI를 숨기려면 구성 인터페이스에 `ExtendedObjectClassDefinition` 주석 속성 `generateUI` 포함합니다. 속성을 `false`로 설정합니다. 이렇게 하면 모든 범위에 대한 구성 UI가 숨겨집니다.
...
1. [구성 UI 숨기기](./liferay-g8v3.zip) 다운로드 및 압축 해제
...
`@Component` 주석이 있는 구성 인터페이스를 식별합니다. `구성 요소` 주석의 `configuration.pid` 구성 인터페이스의 정규화된 클래스 이름과 일치해야 합니다.
