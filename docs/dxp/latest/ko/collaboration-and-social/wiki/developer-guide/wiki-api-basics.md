...
Liferay의 Headless Delivery 애플리케이션은 [Wiki](../getting-started-with-wikis.md) 애플리케이션에 대한 REST 서비스를 제공합니다. 이러한 서비스를 사용하면 Wiki 노드와 페이지를 추가하고, 정보를 나열하고, 콘텐츠를 수정하거나 완전히 제거할 수 있습니다. 여기에서는 cURL 명령과 Java 클래스를 사용하여 해당 서비스를 호출합니다.
...
이 튜토리얼에서는 Headless API를 시연하기 위한 샘플 코드를 제공합니다. 이 코드에는 튜토리얼 전체에서 사용할 샘플 cURL 및 Java 파일이 모두 포함되어 있습니다.

다음 명령을 실행하여 [샘플 코드](https://resources.learn.liferay.com/dxp/latest/en/collaboration-and-social/wiki/developer-guide/liferay-q8u2.zip) 을 다운로드하고 압축을 풉니다.
...
이 연습에서는 cURL 명령이나 Java 클래스를 사용하여 `WikiNode` API를 호출할 수 있습니다. 다음 출력 예는 제공된 Java 클래스의 출력과 약간 다른 cURL 명령에 해당합니다.
...
1. 사이트 ID를 매개변수로 사용하여 `WikiNode_POST_ToSite`를 실행합니다. 그러면 지정된 사이트에 새 Wiki 노드가 생성됩니다.
...
1. 해당 매개변수에 대한 사이트 ID를 사용하여 `WikiNodes_GET_FromSite`를 실행합니다. 그러면 지정된 사이트의 모든 Wiki 노드 목록이 반환됩니다.
   ...
   **자바의 경우:**
   ...
   ```{note}
   모든 DXP/포털 인스턴스에는 `Main`이라는 기본 Wiki 노드가 함께 제공됩니다. 이 노드는 생성한 노드와 함께 위 출력에 표시됩니다.
   ```

1. 해당 매개변수에 Wiki 노드 ID를 사용하여 `WikiNode_PUT_ById`를 실행합니다. 그러면 지정된 Wiki 노드의 세부 정보가 API 호출에 제공된 세부 정보로 대체됩니다.
...
1. 매개변수에 동일한 Wiki 노드 ID를 사용하여 `WikiNode_DELETE_ById`를 실행합니다. 그러면 지정된 노드가 삭제됩니다.
...
1. 해당 매개변수에 동일한 Wiki 노드 ID를 사용하여 `WikiNode_GET_ById`를 실행합니다. 지정된 노드가 있는 경우 해당 노드에 대한 세부 정보가 반환됩니다.
   ...
   **자바의 경우:**
   ...
   이전 단계에서 노드를 삭제했으므로 다음 메시지가 반환됩니다.
...
cURL 명령이나 Java 클래스를 사용하여 `WikiPage` API를 호출할 수 있습니다. 다음 출력 예는 제공된 Java 클래스의 출력과 약간 다른 cURL 명령에 해당합니다.
...
1. Wiki 페이지용 노드를 생성하려면 `WikiNode_POST_ToSite`를 실행하세요.
   ...
   **자바의 경우:**
   ...
   ```{note}
   Wiki 페이지는 Wiki 노드에 저장되므로 Wiki 페이지를 생성하려면 최소한 하나의 노드가 있어야 합니다.
   ```

1. `WikiPage_POST_ToNode` 호출에 사용하기 위해 출력에서 노드 ID를 복사합니다.
...
1. 매개변수로 위의 노드 ID를 사용하여 `WikiPage_POST_ToNode`를 실행합니다. 그러면 지정된 노드에 대한 새 Wiki 페이지가 생성됩니다.
...
1. `WikiPage_POST_ToParent` 호출에 사용할 페이지 ID를 복사합니다.
...
1. 해당 매개변수에 위의 Wiki 페이지 ID를 사용하여 `WikiPage_POST_ToParent`를 실행합니다. 그러면 지정된 Wiki 페이지에 대한 하위 페이지가 생성됩니다.
...
1. 해당 매개변수에 대한 Wiki 노드 ID를 사용하여 `WikiPages_GET_FromNode`를 실행합니다. 그러면 지정된 노드에 추가된 모든 Wiki 페이지 목록이 반환됩니다. 여기에는 새로 생성된 Wiki 페이지가 모두 포함됩니다.
...
1. 해당 매개변수에 대한 Wiki 상위 페이지 ID를 사용하여 `WikiPages_GET_FromParent`를 실행합니다. 그러면 지정된 Wiki 페이지에 대한 기존 하위 페이지가 반환됩니다.
...
1. 해당 매개변수에 위의 Wiki 페이지 ID 중 하나를 사용하여 `WikiPage_PUT_ById`를 실행합니다. 그러면 원본 페이지의 콘텐츠가 API 호출에 정의된 콘텐츠로 대체됩니다.
...
1. 매개변수에 원하는 Wiki 페이지 ID를 사용하여 `WikiPage_DELETE_ById`를 실행합니다. 지정된 Wiki 페이지가 삭제됩니다.
...
1. 삭제된 위키 페이지의 ID를 해당 매개변수로 사용하여 `WikiPage_GET_ById`를 실행합니다. 지정된 페이지가 있는 경우 해당 페이지에 대한 세부 정보를 반환합니다.
...
제공된 샘플 코드에는 다음 `WikiNode` 및 `WikiPage` API에 대한 cURL 스크립트와 Java 클래스가 포함되어 있습니다.
...
| 위키노드 | `GET`    | `/v1.0/sites/[siteId]/wiki-nodes` | 지정된 사이트에 있는 모든 Wiki 노드의 전체 목록을 반환합니다. 결과는 페이지 매김, 필터링, 검색 및 정렬이 가능합니다. |
| 위키노드 | `GET`    | `/v1.0/wiki-nodes/[wikiNodeId]`   | 지정된 노드 엔터티에 대한 세부 정보를 반환합니다.                                           |
| 위키노드 | `PUT`    | `/v1.0/wiki-nodes/[wikiNodeId]`   | 지정된 노드의 세부정보를 API 호출에 제공된 세부정보로 바꿉니다.                                  |
...
| 위키페이지 | `GET`    | `/v1.0/wiki-nodes/[wikiNodeId]/wiki-pages`       | 지정된 Wiki 노드에 추가된 모든 Wiki 페이지의 전체 목록을 반환합니다. 결과는 페이지 매김, 필터링, 검색 및 정렬이 가능합니다.     |
| 위키페이지 | `GET`    | `/v1.0/wiki-pages/[parentWikiPageId]/wiki-pages` | 지정된 Wiki 페이지에 추가된 모든 Wiki 하위 페이지의 전체 목록을 반환합니다. 결과는 페이지 매김, 필터링, 검색 및 정렬이 가능합니다. |
