...
# Using Object APIs With Base64 Formatted Files
...
When using objects with [attachments fields](../../creating-and-managing-objects/fields/attachment-fields.md), you can generally upload files from your computer or use files uploaded to the [Documents and Media](../../../../content-authoring-and-management/documents-and-media.md) application. Both cases require multiple steps.
...
Alternatively, you can post a Base64 formatted document in one step by using a REST API call and attaching Base64 formatted files to it.
...
1. Select the new *Object* draft, go to the *Fields* tab, and add the following fields:
   ...
   | usersComputerAttachment2 | usersComputerAttachment2 | Attachment | `Upload Directly from the User's Computer` (Toggle on the `Show Files in Documents and Media` option) |
   | docsAndMediaAttachment   | docsAndMediaAttachment   | Attachment | `Upload or Select from Documents and Media Item Selector`                                             |

   Alternatively, you can run the following code in your terminal to create the object using a cURL command (then, you can access the object definition at Global Menu &rarr; Control Panel &rarr; Objects and continue with the next step):
...
1. Execute `Ables_POST_FromUsersComputer1.sh`. This creates an object entry with a Base64 file attached to it.
...
1. Execute `Ables_POST_FromUsersComputer2.sh`. This creates an object entry with a Base64 file attached to it.
...
1. Execute `DocumentsAndMedia_POST_Folder.sh` to create the folder to store the uploaded file. Use your site ID as a parameter. In this example, the site ID is `20122`

   !!! tip
       Locate your site ID at *Site Menu* (![Site Menu](../../../../images/icon-product-menu.png)) &rarr; *Configuration* &rarr; *Site Settings* &rarr; *Site Configuration* under Platform.
...
1. Then, execute `Ables_POST_ToDocumentsAndMedia.sh`. This creates an object entry with a Base64 file attached to it and places it in the recently created folder. Use the folder's external reference code (ERC) and the site ID as parameters. In this example, the ERC is `able-folder` and the site ID is `20122`.

   !!! note
       If the `siteId` is not included, null, or empty, the file is uploaded to the default site's root folder.

   ```bash
   ./Ables_POST_ToDocumentsAndMedia.sh able-folder 20122
   ```
...
1. Run `Ables_GET_FromCompany.sh` to return a list of all created entries.
...
To get the Base64 encoded attached file in the response, you must request it as a nested field by using the `nestedFields=[ERC].fileBase64` query parameter.
