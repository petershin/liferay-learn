...
1. 원하는 항목에 대해 **작업** 버튼(![Action Button](../../../../images/icon-actions.png))을 클릭하고 독립 실행형 작업을 선택합니다.
   ...
   ![원하는 항목에 대해 작업 버튼을 클릭하고 독립 실행형 작업을 선택합니다.](./using-manual-actions/images/02.png)
...
1. **REST 애플리케이션** 클릭하고 원하는 개체 정의를 선택합니다. 사용자 정의 개체는 `c/[pluralobjectlabel]` 명명 패턴(예: `c/tickets`)을 사용합니다.

각 독립 실행형 작업에 대해 Liferay는 두 개의 PUT API를 생성합니다. 하나는 항목 ID를 사용하고 다른 하나는 항목의 외부 참조 코드(ERC)를 사용합니다.

![Liferay는 두 개의 PUT API를 생성합니다. 하나는 항목 ID를 사용하고 다른 하나는 항목의 ERC를 사용합니다.](./using-manual-actions/images/03.png)
...
Liferay의 기본 버튼 조각이 사용 사례를 만족시키지 못하는 경우, 해당 HTML 태그에 `data-lfr-editable-id="action"` 및 `data-lfr-editable-type="action"` 속성을 추가하여 사용자 정의 버튼에 액션을 매핑할 수 있습니다. 조각에 속성을 추가하는 방법에 대한 자세한 내용은 [조각별 태그 및 속성 참조](../../../../사이트-빌딩/개발자 가이드/참조/조각/조각별 태그 참조.md)를 참조하세요.
...
1. 레이아웃에 **Button** 조각을 추가합니다. 콘텐츠 페이지를 편집하는 방법을 알아보려면 [콘텐츠 페이지에 요소 추가](../../../../site-building/creating-pages/using-content-pages/adding-elements-to-content-pages.md) 참조하세요.

1. 버튼을 한 번 클릭해 보세요. 버튼 옵션에서 유형으로 **동작** 선택합니다. 이제 버튼에 작업을 매핑할 수 있습니다.

1. 버튼을 두 번째로 클릭하세요. 오른쪽에 작업 메뉴가 열립니다.

1. 매핑 탭에서 **항목** 필드를 클릭하거나 필드 옆에 있는 **항목** 선택 아이콘(![Select Item](../../../../images/icon-add-app.png))을 클릭합니다.
...
1. 특정 개체 항목을 선택합니다.
   ...
   ![특정 개체 항목 선택](./using-manual-actions/images/05.png)

1. 버튼 라벨에 매핑할 **필드** 선택합니다.
...
```{tip}
작업에 매핑된 구성 요소가 있는 컬렉션 표시 조각이나 양식 조각을 사용할 수 있습니다. [컬렉션 표시](../../../../site-building/displaying-content/additional-content-display-options/displaying-collections.md) 및 [조각을 사용하여 양식 작성](../../using-fragments-to-build-forms.m) 에서 자세한 내용을 확인하세요.
```
...
1. [표시 페이지 템플릿을 생성하고](../../../../site-building/displaying-content/using-display-page-templates/creating-and-managing-display-page-templates.md) 개체를 콘텐츠 유형으로 선택합니다.
   ...
   ![디스플레이 페이지 템플릿을 생성할 때 개체를 콘텐츠 유형으로 선택하십시오.](./using-manual-actions/images/06.png)

1. 레이아웃에 **Button** 조각을 추가합니다.

1. 버튼을 한 번 클릭해 보세요. 버튼 옵션에서 유형으로 **동작** 선택합니다.
...
1. 버튼을 두 번째로 클릭하세요.
   ...
   ```{note}
   특정 콘텐츠를 선택해야 하는 경우 *소스* 필드를 클릭하고 *특정 콘텐츠*를 선택한 다음 특정 *항목*을 선택하세요.
   ```
...
![사용자가 작업에 매핑된 버튼이나 조각을 클릭할 때 작업이 수행된 후 어떤 일이 발생하는지 선택할 수 있습니다. 아무것도 하지 않거나, 사용자가 알림을 받거나, 사용자가 페이지로 리디렉션되거나, 사용자가 외부 URL로 리디렉션됩니다.](./using-manual-actions/images/07.png)

사용자가 작업을 트리거한 후에 선택한 옵션이 실행됩니다.

## 작업 권한

Liferay는 독립 실행형 작업을 트리거할 수 있는 역할을 관리하기 위한 권한을 자동으로 생성합니다. 각 권한은 `작업을 따릅니다.[actionName]` 명명 패턴(예: `action.sendReminder`). 개체 권한에 대한 자세한 내용은 [권한 프레임워크 통합](../../understanding-object-integrations/permissions-framework-integration.md) 참조하세요.

![독립 실행형 권한은 작업을 사용합니다. [actionName] 명명 패턴.](./using-manual-actions/images/08.png)
