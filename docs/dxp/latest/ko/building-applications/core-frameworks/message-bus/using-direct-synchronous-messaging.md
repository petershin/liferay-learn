...
1. Gogo Shell 명령 필드에 `x6n5:sendMessage` 을 입력한 다음 메시지를 입력합니다. 예를 들어,
...
4개의 예제 모듈에는 하나의 클래스가 있습니다. 클래스에서 목적지를 관리하고 다른 클래스는 메시지를 보내고 다른 두 클래스는 목적지로 전송된 메시지를 수신합니다.
...
1. 사용자가 `x6n5:sendMessage` Gogo 셸 명령을 실행하면 `X6N5BakerOSGiCommands` 는 메시지 페이로드의 명령 인수를 `acme/x6n5_able` 대상으로 보냅니다.
...
`X6N5BakerOSGiCommands` 은 자체 클래스 유형의 서비스 `구성 요소` 입니다. `@Reference` 주석을 사용하여 *다이렉트* 모드로 설정된 `SynchronousMessageSender` 을 삽입합니다(주석의 `target = "(mode=DIRECT)"` 속성으로 지정됨).
