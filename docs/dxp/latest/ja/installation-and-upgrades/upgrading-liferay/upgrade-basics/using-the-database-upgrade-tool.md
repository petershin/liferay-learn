...
1. アップグレードツールを起動します。 次にコマンドの例を示します。 
    ...
    上記のコマンドは、アプリケーションサーバーで推奨されているのと同じ JVM オプションでアップグレードツールを実行します。ファイルエンコーディング (`UTF-8`)、タイムゾーン (`GMT`)、国、言語、メモリ設定 (`-Xmx value`) はすべてアプリケーションサーバーの設定に合わせる必要があります。データ量が 10 GB を超えるデータベースについては、`-Xmx` オプションを使用して追加のメモリを割り当ててください。
