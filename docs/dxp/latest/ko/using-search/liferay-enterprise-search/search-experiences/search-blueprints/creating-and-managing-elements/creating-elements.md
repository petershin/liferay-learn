...
요소 소스 편집기에서 미리 정의된 변수를 요소에 삽입하려면 커서를 원하는 위치에 놓은 다음 왼쪽 사이드바에서 변수를 클릭하십시오. Custom JSON Element에서 변수를 편집기에 직접 입력해야 합니다.
...
- 쿼리 절에 필드 값을 전달할 때와 같이 요소의 다른 위치에서 필드를 참조하는 데 사용할 수 있는 `이름` 입력합니다: `${configuration.name}`.
...
- `typeOptions` 속성을 사용하여 각 필드 유형에 사용할 수 있는 옵션을 구성합니다. 예를 들어 선택 필드에 대해 `옵션` 표시를 설정합니다. 
   ...
   - `unitSuffix` 단위가 있는 숫자 필드에 표시할 단위 표기법을 설정합니다(예를 들어, `unit` 이 ``인 경우 `unitSuffix` `` 또는 ``로 설정할 수 있음).
