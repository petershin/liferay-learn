...
1. Fragment Toolkit을 사용하여 Fragment Collection을 Docker 컨테이너로 가져옵니다. 또는 [Fragment를 수동으로 가져오기](../../creating-pages/page-fragments-and-widgets/using-fragments/managing-fragments.md)할 수 있습니다.
...
1. 조각 세트를 사용할 수 있는지 확인합니다. *사이트 메뉴*(![사이트 메뉴](../../../images/icon-product-menu.png))를 열고 *디자인*으로 이동합니다. *파편*. 세트가 목록에 나타나야 합니다.
...
1 페이지에서 *C7F8 카드* 선택하여 조각 구성 메뉴를 엽니다. *일반* 탭에서 구성 요소의 텍스트 스타일을 어둡게 또는 밝게 설정합니다.
...
**조각 편집기 사용:** UI *[조각 편집기로 조각을 편집하고](./using-the-fragments-editor.md) 구성* 탭을 클릭합니다. Fragment의 구성이 편집기에 나타납니다.

**조각 구성 파일 편집:** 조각 파일이 없는 경우 조각의 작업 메뉴를 열고 *내보내기*클릭하여 조각에서 파일을 내보냅니다. Fragment의 `fragment.json` 파일에 있는 `configurationPath` 필드(선택 사항)는 구성 `.json` 파일을 지정합니다. 구성 파일이 없는 경우 구성 파일을 만들고 `configurationPath` 필드를 구성 파일 이름으로 설정합니다.
...
위의 구성 파일은 조각에 *dark* 또는 *light* 텍스트 스타일을 적용하기 위한 선택기를 지정합니다. 구성에는 `c7f8TextStyle`이라는 필드가 있습니다. 필드의 `유형` `select`이며 선택기 구성 요소가 됩니다. 자세한 내용은 [단편 구성 유형](../reference/fragments/fragment-configuration-types-reference.md) 을 참조하십시오. 선택기의 유효한 값은 `dark` 또는 `light`입니다. `"dataType": "string"` 속성은 선택기가 문자열 데이터에서 작동함을 의미합니다.
...
사용자가 선택한 구성 값은 FreeMarker 컨텍스트를 통해 HTML에서 사용할 수 있습니다. HTML에서 `${configuration.fieldName}`표기법으로 참조됩니다. 예제(`${configuration.textAppliedStyle}`)는 사용자가 선택한 구성 값에 따라 `dark` 또는 `light` 반환하고 CSS 클래스를 `text-light` 또는 `text-dark`으로 설정합니다.
...
이 예는 선택 구성을 보여줍니다. 사용 가능한 조각 구성 유형의 전체 목록은 [구성 유형 참조](../reference/fragments/fragment-configuration-types-reference.md) 참조하십시오.
...
1. *C7F8 Set*을 선택하고 C7F8 카드에 대해 *Actions* 버튼(![Actions Icon](../../../images/icon-actions.png))을 클릭한 다음 *Edit*선택합니다. 조각 편집기가 열립니다.
...
1. *Code* 탭의 *HTML* 창으로 돌아가서 조건문으로 단락 요소를 래핑하여 확인란의 값을 확인합니다. *게시* 클릭하여 변경 사항을 적용합니다. 
...
1. 콘텐츠 페이지에 반영되도록 변경 사항을 전파합니다. C7F8 카드에 대해 *Actions* (![Action Icon](../../../images/icon-actions.png))를 클릭하고  *View Usages*선택합니다.
...
1. *C7F8 카드* 다시 선택하면 오른쪽에 선택 패널이 표시됩니다.

1. *일반* 탭에서 *설명 표시* 확인란을 선택/선택 취소하여 카드의 텍스트를 표시하거나 숨깁니다.

   ![프래그먼트에 대해 원하는 만큼의 구성 옵션을 가질 수 있습니다.](./adding-configuration-options-to-fragments/images/05.png)
