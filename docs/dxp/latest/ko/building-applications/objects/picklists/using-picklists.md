...
1. 선택 목록에 *이름* 을 입력하고 *저장*을 클릭합니다. 이 값은 Liferay UI에서 목록을 식별하고 생성 후 현지화할 수 있습니다.
...
1. 모달 창에서 항목에 대해 *이름* 및 *키* 을 입력합니다.
...
1. *저장*을 클릭합니다. 이렇게 하면 선택 목록이 새 항목으로 즉시 업데이트됩니다.
...
목록이 생성되면 개체 필드를 생성할 때 선택할 수 있습니다. *선택 목록* 필드 유형을 선택하고 원하는 목록을 선택합니다. 자세한 내용은 [개체에 필드 추가](../creating-and-managing-objects/fields/adding-fields-to-objects.md) 을 참조하십시오.
...
이를 통해 선택 목록 정의(`ListTypeDefinition`) 및 선택 목록 항목(`ListTypeEntry`) 모두에 대해 CRUD 작업을 수행할 수 있습니다.

## 개체 API 호출의 선택 목록 필드

{bdg-secondary}`7.4+ U1+/GA5+의 경우`

선택 목록 필드를 포함하는 개체에 대해 POST, PATCH 및 PUT API 호출을 수행할 때 다음 형식을 사용해야 합니다.
