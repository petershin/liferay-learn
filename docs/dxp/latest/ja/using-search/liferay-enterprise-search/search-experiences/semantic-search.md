...
**セマンティック** 検索は、検索されたフレーズの背後にある意図を評価する。 一方、 **キーワード** 検索は、検索されたフレーズをインデックスされたテキストフィールドにマッチさせるもので、意味に基づくマッチを作り出すことはできない。
...
![自然言語の検索フレーズは、セマンティック検索システムによって処理される。](./semantic-search/images/03.png)

セマンティック検索は、追加のコンテンツ処理パイプラインを可能にする。 有効化すると、プラットフォームはテキスト埋め込みと呼ばれる入力テキストのベクトル表現を生成し、Elasticsearchのインデックス・ドキュメントに格納する。 検索時には、ユーザーによって入力された検索キーワードは、同じベクトル化と埋め込みプロセスを経て、ユーザーにとってより意味のある関連検索結果を提供する類似検索を行うことが可能になる。 すべてのコンテンツタイプがテキスト埋め込みをサポートしているわけではありません：
...
1. **Global Menu**(![Global Menu](../../../images/icon-applications-menu.png)) を開き、 **Control Panel** &rarr; **Instance Settings** をクリックする。
1. **Platform** &rarr; **Feature Flags** セクションを開く。
1. ベータの項目で、セマンティック検索を有効にする。

![セマンティック検索はベータ機能であり、インスタンス設定で有効にする必要があります。](./semantic-search/images/02.png)

[ポータル・プロパティ](../../../installation-and-upgrades/reference/portal-properties.md)を使ってセマンティック検索を有効にするには、これを `portal-ext.properties`に追加します：
...
1. [テキストの埋め込みインデックスを再作成する。](#re-indexing-the-text-embeddings)
...
例えば、ユーザーが 「**スケートはどのように動くのか？** と検索した場合、海洋生物学について訓練されたモデルは、レクリエーションについて訓練されたモデルとは異なる結果を提供するだろう。
...
テキスト埋め込みのインデックスを作成するには、「インデックスアクション」画面を使用し、「セマンティック検索システム」または「インスタンス設定」の「アセットエントリクラス名」設定で有効にしているモデルタイプに対してのみ、「**Reindex**」ボタンをクリックします。
...
**最大文字数：** テキスト埋め込みプロバイダーに送信する最大文字数を設定します。 デフォルトでは最大500文字が送信され、ベクトル表現に変換される。 ここでの理想的な値は、どの [テキスト埋め込みプロバイダー](./semantic-search/setting-up-a-text-embedding-provider.md) を使用しているかによって異なります。

**テキストの切り捨て戦略：** テキストの先頭（デフォルト）、中間、または末尾のいずれから変換前のサンプルを抽出するかを選択します。 この設定は、ソース・テキストが最大文字数より長い場合にのみ適用されます。 ストラテジーと最大文字数に応じて、タイトル/件名とコンテンツ/本文の一部をテキスト埋め込みに使用することができます。

**タイプ：** 変換するコンテンツタイプを選択します。 デフォルトでは、ブログエントリー、ナレッジベース記事、ウェブコンテンツ記事、ウィキページの4つのタイプが処理されます。 メッセージボード メッセージエンティティは必要に応じて設定できる。

```{note}
現在、基本的なウェブコンテンツ記事のみがサポートされています。
```

**言語：** 変換する言語とローカライズを選択します。 デフォルトでは、リストされたすべての言語が選択されています。 **Ctrl + クリック** を使用して、リストから複数の言語を選択します。
...
1. 完全なセマンティック検索では、通常のインデクサーは検索ブループリントで無効にされ、関連するコンテンツを検索するためにテキスト埋め込みのみが使用されます。
...
* [LIFERAY] 追加の意味検索処理が行われる：
  ...
  * [LIFERAY] テキスト埋め込み処理の結果は、 [Liferay Company Index](../../search-administration-and-tuning/elasticsearch-indexes-reference.md) に、 [dense_vector](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/dense-vector.html) フィールドとして各ドキュメント[^1]に格納されます。
  * System/Instance Settingsでの設定に続いて、テキストスニペットがテキスト埋め込みプロバイダーに送られ、テキスト埋め込みが行われ、トランスフォーマーが使用するモデルに基づいてベクトル表現が作成されます。 テキスト埋め込み処理の結果は、 [Liferay Company Index](../../search-administration-and-tuning/elasticsearch-indexes-reference.md) に、 [dense_vector](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/dense-vector.html) フィールドとして各ドキュメント[^1]に格納されます。
