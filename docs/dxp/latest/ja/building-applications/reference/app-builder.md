...
オブジェクトは、すべてのApp Builderアプリケーションの基盤です。 これは、アプリケーションが収集および管理する情報を定義します。 例えば、Guestbookオブジェクトには少なくとも_Name_フィールドと_Message_フィールドが含まれます。
...
テーブルビューには、オブジェクトについて収集されたデータレコードが表示されます。 さらに、権限を持つ人は、_Add new entry_機能にアクセスし、エントリーの詳細を確認し、アプリケーションのエントリーを管理することができます。 テーブルビューには、オブジェクトのフィールドのすべてまたはサブセットを表示できます。 各アプリには少なくとも1つもテーブルビューがありますが、オブジェクトには必要な数だけ設定できます。 ドラッグアンドドロップインターフェイスを使用すると、既存のフィールドからテーブルビューを簡単に作成できます。
