...
* **バックアップ履歴の表示**：選択した環境で保持されているバックアップの完全なリストを表示できます。 各エントリには、バックアップの名前、サイズ、タイプ（`Auto`、`Manual`、または`Uploaded`）および作成時刻が表示されます。 赤色のバックアップアイコンは、バックアップに失敗したことを示します。 その横には、_Backup failed_というメッセージとサポートに連絡するためのリンクが表示されます。
