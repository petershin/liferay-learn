...
구성 프레임워크를 사용하면 [여러 수준의 범위에 대한 애플리케이션 구성](./scoping-configurations.md)을 설정할 수 있습니다. 인스턴스 및 사이트 범위 구성이 `ConfigurationProvider`를 사용하는 경우 포틀릿 범위 구성은 아래 예와 같이 `PortletDisplay`를 사용합니다.

구성 프레임워크는 포틀릿 환경 설정과 함께 사용할 수 있으므로 앱이 시스템 설정의 구성 UI와 포틀릿의 설정 탭의 환경 설정 UI를 모두 가질 수 있습니다. 

사용자가 포틀릿 환경 설정을 구현하고 설정한 경우 애플리케이션의 구성이 무시됩니다. 자세한 내용은 [포틀릿 기본 설정](../../developing-a-java-web-application/using-mvc/portlet-preferences.md) 참조하십시오.
...
`X7Y2PortletInstanceConfiguration` 메소드는 `PortletDisplay` 사용하여 포틀릿 인스턴스 구성을 얻습니다. `render()` 메소드는 JSP 파일의 요청에서 읽을 수 있도록 요청 객체에 구성을 추가합니다.
