...
* **고아 데이터**: 연결되지 않은 미사용 개체를 확인합니다. 여기 몇 가지 예가 있어요.
...
로컬/원격 스테이징이 활성화되어 있고 스테이징된 사이트에 저장된 콘텐츠 또는 데이터가 있는 경우 라이브 사이트에 [게시](../../../site-building/publishing-tools/staging/configuring-remote-live-staging.md) 합니다. 이 단계를 건너뛰면 업그레이드 후 전체 게시(또는 수동으로 변경 사항 게시)를 실행해야 합니다. 시스템에서 마지막 게시 날짜 이후 변경된 내용을 알 수 없기 때문입니다. 최종 게시를 완료한 후 업그레이드하기 전에 스테이징을 끕니다.
