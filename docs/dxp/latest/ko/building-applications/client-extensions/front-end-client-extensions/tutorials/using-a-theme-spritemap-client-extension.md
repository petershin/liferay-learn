...
```{note}
SVG 스프라이트 맵은 각각 고유한 ID를 가진 함께 그룹화된 여러 아이콘을 포함하는 이미지입니다. SVG를 로드하고 ID별로 개별 아이콘을 참조할 수 있습니다. 이렇게 하면 모든 사이트 아이콘을 표시하기 위해 스프라이트 맵을 한 번 다운로드하고 캐시할 수 있으므로 성능이 약간 향상됩니다.
```
...
클라이언트 확장은 ID `liferay-sample-theme-spritemap-1` 가지며 유형 및 추가할 스프라이트 맵 파일을 포함하여 테마 스프라이트 맵 클라이언트 확장에 대한 주요 구성을 포함합니다. 사용 가능한 속성에 대한 자세한 내용은 [테마 스프라이트 맵 YAML 구성 참조](../theme-sprite-map-yaml-configuration-reference.md) 을 참조하세요.
