...
HTML, 단락, 구분 기호 및 소셜 조각을 제외한 모든 조각에는 일반 설정이 있습니다. 이러한 옵션 중 일부는 표준(예: 가시성 및 프레임)이고 일부는 각 조각마다 고유합니다. 다음 참조에는 일반 설정이 있는 조각만 포함되어 있습니다.
...
![페이지 요소 측면 패널의 일반 설정에 액세스하려면 조각을 선택하세요.](./general-settings-reference/images/01.png)
...
| 분야       | 묘사                                                                                                                                 |
|:-------- |:---------------------------------------------------------------------------------------------------------------------------------- |
| 프래그먼트 숨김 | 다양한 뷰포트에 대해 조각을 숨기려면 확인란을 선택하세요. 이 설정이 페이지 뷰포트와 상호 작용하는 방식에 대해 자세히 알아보려면 [조각 가시성 구성](../configuring-fragment-visibility.md) 참조하세요. |
...
| 필드 | 설명 |
| :--- | :--- |
| 링크 | 컨테이너가 URL, 페이지 또는 매핑된 URL에 연결되어 있는지 확인합니다. 이 설정은 다음 필드 중 사용 가능한 필드를 결정합니다. |
| URL(*URL용*) | 컨테이너의 현지화 가능한 URL을 입력하세요. 다른 언어에 대한 URL을 설정하려면 먼저 편집 표시줄을 통해 페이지 또는 템플릿의 언어를 전환하세요. 그런 다음 원하는 URL을 입력하세요. |
| 페이지(*페이지용*) | 컨테이너에 대한 Liferay 페이지를 선택합니다. |
| 항목(*매핑된 URL의 경우*) | 문서 및 미디어, 웹 콘텐츠, 블로그, 카테고리 중에서 원하는 항목을 선택하세요. 선택한 항목은 매핑을 위한 필드를 제공합니다. |
| 필드(*매핑된 URL*) | 컨테이너에 매핑할 URL 필드를 선택합니다. 유효한 필드를 선택하면 *URL* 필드가 자동으로 채워집니다. |
| URL(*매핑된 URL의 경우*) | 이 필드는 선택한 *필드* URL로 자동으로 채워집니다. |
| 새 탭에서 열기 | 링크가 새 탭에서 열리는지 확인합니다. |
| 콘텐츠 표시 | 컨테이너의 콘텐츠가 블록, Flex 행 또는 Flex 열에 표시되는지 여부를 결정합니다. flex 옵션에는 추가 구성이 있습니다. |
...
| 콘텐츠 정당화(*Flex Row용*) {bdg-secondary}`7.4 U19+/GA19+` | 행의 요소가 정렬되는 방식(시작, 중심, 끝, 사이 또는 주변)을 결정합니다. |
| 컨테이너 폭 | 컨테이너가 유동 폭을 사용하는지 고정 폭을 사용하는지 결정합니다. |
...
| 거터 표시 | 드롭존 모듈 사이에 패딩을 추가하거나 제거합니다. |
| 레이아웃 | 각 행에 드롭 존 모듈 수를 결정합니다(1, 2, 3 또는 6). 사용 가능한 옵션은 모듈 수에 따라 다릅니다. |
...
| 작풍 | 버튼의 스타일을 결정합니다: 기본, 보조, 링크, 개요 기본 또는 개요 보조. |
| 규모 | 버튼 크기를 작음, 보통, 큼으로 결정합니다.                   |
...
| 분야                    | 묘사                                            |
|:--------------------- |:--------------------------------------------- |
| 유형                    | 버튼의 스타일을 결정합니다: 기본, 보조, 링크, 개요 기본 또는 개요 보조.   |
| 규모                    | 버튼 크기를 작음, 보통, 큼으로 결정합니다.                     |
| 패널 유형                 | 드롭다운 메뉴 패널의 크기(일반, 전체 너비 또는 메가 메뉴)를 결정합니다.    |
| 호버 시 표시               | 버튼 위에 커서를 놓을 때 메뉴 패널이 표시되는지 확인하려면 확인란을 선택합니다. |
| 패널을 편집 모드에서 열린 상태로 유지 | 편집하는 동안 패널이 열려 있는지 확인하려면 확인란을 선택하세요.          |<!-- See \[Using the Dropdown Fragment\]() for more information. -->## 외부 비디오 옵션
...
| 제거하다     | 선택한 색상을 제거하려면 지우기를 클릭합니다.        |<!-- For Liferay DXP 7.4 U9+, these settings are listed under the \[Styles\](./styles-reference.md) tab.-->## 슬라이더 옵션
...
| 필드 | 설명 |
| :--- | :--- |
| 대상 수집 | 필터와 함께 사용할 컬렉션을 선택하세요. |
| 필터 | 필터 유형(키워드 또는 카테고리)을 결정합니다. 카테고리를 사용할 때 최종 사용자는 사전 정의된 필터 카테고리에서 선택할 수 있습니다. 키워드를 사용할 때 자체 필터 값을 수동으로 입력할 수 있습니다. |
| 출처(*카테고리*) | 대상 컬렉션을 필터링하는 데 사용되는 어휘 또는 카테고리를 선택하세요. |
| 단일 선택(*카테고리*용) | 최종 사용자가 컬렉션에 하나 이상의 필터만 적용할 수 있는지 여부를 결정합니다. |
| 검색 필드 포함(*카테고리*) | 최종 사용자가 사용 가능한 필터 값을 검색할 수 있도록 필드를 포함할지 여부를 결정합니다. |
| 라벨 표시 | UI에 필터 라벨을 표시하려면 선택하세요. |
...
| 필드 | 설명 |
| :--- | :--- |
| 아이템 | 문서 및 미디어, 웹 콘텐츠, 블로그 또는 카테고리에서 조각과 연결하려는 항목을 선택합니다. |
...
| 필드 | 설명 |
| :--- | :--- |
| 콘텐츠 유형 | 사용자 정의 개체를 선택합니다. 그러면 양식 컨테이너와 해당 조각이 선택한 개체에 매핑됩니다. 양식 컨테이너는 한 번에 하나의 개체에만 매핑할 수 있습니다. |
| 성공 메시지 | 양식 제출 성공 메시지가 현재 페이지에 포함되어 있는지 또는 사용자를 Liferay 페이지 또는 외부 URL로 리디렉션하는지 확인합니다. |
...
| 항목 정렬(*Flex 열용*) | 열의 요소 정렬 방법(시작, 중심, 끝, 늘이기 또는 기준선)을 결정합니다. |
...
| 분야      | 묘사                                                                                                                                                                                                                   |
|:------- |:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 문자 수 표시 | 입력한 문자 수를 표시하고 최대 문자 수를 초과하면 경고를 표시합니다. 최대 문자 수는 개체의 필드 구성에서 구성할 수 있습니다. 자세한 내용은 [필드 유형 UI 참조](../../../../../building-applications/objects/creating-and-managing-objects/fields/adding-fields-to-objects.md) 참조하세요. |

### 분류

| 분야    | 묘사                                                                                                                                                                                                                                                                                                                                                                             |
|:----- |:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 어휘 유형 | 어휘의 가시성에 따라 카테고리를 표시합니다. 자세한 내용은 [어휘 가시성](../../../../../content-authoring-and-management/tags-and-categories/organizing-content-with-categories-and-tags.md#vocabulary-visibility) 참조하세요. 이 설정에 액세스하려면 개체에서 `분류 활성화` 활성화해야 합니다. 자세한 내용은 [개체 만들기](../../../../../building-applications/objects/creating-and-managing-objects/creating-objects.md#editing-object-drafts) 참조하세요. |
