...
1. 블랙리스트 [구성 파일](../../../system-administration/configuring-liferay/configuration-files-and-factories/using-configuration-files.md)을 생성합니다. 이름은 Liferay 버전에 따라 다릅니다.
...
1. `conf` 폴더를 `solr-[version]/server/solr/liferay` 에서 방금 생성한 `liferay_configs` 폴더로 복사합니다.
