...
ZIP 파일 또는 Docker 이미지를 통해 설치되는 Liferay Tomcat 번들에는 Liferay로 시작하는 Elasticsearch 노드가 포함되어 있습니다. Liferay 7.3+ 및 7.4+의 노드는 별도의 JVM에서 사이드카 서버로 실행되며, Liferay 7.2의 노드는 Liferay 서버에 내장되어 있습니다.

Elasticsearch 서버는 다음 URL에서 액세스할 수 있습니다:

* <http://localhost:9201> 라이프 레이 7.3+ 및 7.4+에서
* <http://localhost:9200> on Liferay 7.2

다음은 사이드카 서버 출력 예시입니다:
...
번들로 제공되는 Elasticsearch 서버는 개발과 테스트에는 편리하지만, 프로덕션에는 적합하지 않습니다.
...
프로덕션 환경에서 HSQL과 같은 임베디드 데이터베이스를 실행하지 않을 것이며, 번들로 제공되는 Elasticsearch 서버도 프로덕션 환경에서 실행해서는 안 됩니다. 대신, 원격 모드에서 독립 실행형 서버 또는 서버 노드 클러스터로 Elasticsearch를 실행하세요.

```{important}
검색 튜닝 앱 [동의어 세트](../../search-행정 및 튜닝/synonym-sets.md) 및 [결과 순위](../../search-행정 및 튜닝/결과 순위.md)는 Liferay 7.2 및 7.3의 기본 데이터 저장소에 대한 검색 인덱스를 사용했습니다. 이러한 앱에 대한 데이터는 Liferay 데이터베이스에 저장되지 않았습니다. 따라서 Liferay 7.2 및 7.3에서 사이드카 또는 임베디드 Elasticsearch를 사용하는 동안 동의어 세트 또는 결과 순위가 구성된 경우 원격 Elasticsearch 서버로 전환하고 다시 색인해도 해당 구성이 복원되지는 _않습니다_. 대신 동의어 세트와 결과 순위를 원격 Elasticsearch 클러스터로 수동으로 가져와야 합니다. 이러한 인덱스를 보존하기 위해 Elastic의 [스냅샷 및 복원](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/snapshot-restore.html) 기능을 사용하는 방법에 대한 자세한 내용은 [업그레이드 가이드](../elasticsearch/upgrading-elasticsearch.md)를 참조하세요.
```

## 번들형 Elasticsearch Server 사용 사례

다음은 기본 Elasticsearch 서버(사이드카 및 임베디드)의 일반적인 용도입니다:

* 사용자 지정 [검색 및 인덱싱 코드 테스트](../../developer-guide.html)
* Kibana를 통해 Elasticsearch에서 직접 쿼리를 실행하여 검색 쿼리 개발하기
* [검색 튜닝](../../search-administration-and-tuning.md) 기능 테스트
* [검색 위젯 탐색 및 구성하기](../../search-pages-and-widgets.md)

## 앱 서버의 차이점

Elasticsearch 사이드카 서버는 Liferay 7.4(DXP 및 Portal), Liferay DXP 7.3, Liferay Portal 7.3 GA4+의 Tomcat 번들 및 Docker 이미지와 함께 포함되어 있습니다. 지원되는 애플리케이션 서버에 Liferay WAR을 설치하는 경우 몇 가지 주요 차이점이 있습니다.

```{note}
아래 표에서 _7.3+_ 표기는 Liferay 7.4(DXP 및 포털)를 포함합니다.
```

| 라이프레이 DXP 플레이버       | 기본 Elasticsearch | 사전 설치    | 수동 개입 필요           |
|:-------------------- |:---------------- |:-------- |:------------------ |
| 톰캣 번들: 7.3+          | Sidecar          | &#10004; | &#10008;           |
| Tomcat: 7.3+         | Sidecar          | &#10008; | &#10008; (자동 다운로드) |
| 도커: 7.3+             | Sidecar          | &#10004; | &#10008;           |
| JBoss: 7.3+          | Sidecar          | &#10008; | &#10008; (자동 다운로드) |
...
[WebSphere](../../../installation-and-upgrades/installing-liferay/installing-liferay-on-an-application-server/installing-on-websphere.md) 및 [Weblogic](../../../installation-and-upgrades/installing-liferay/installing-liferay-on-an-application-server/installing-on-weblogic.md) 애플리케이션 서버에 대한 Liferay 설치 지침에는 사이드카 서버를 초기화하는 데 필요한 Elasticsearch 아카이브를 수동으로 제공하는 지침이 포함되어 있습니다.

```{important}
번들로 제공되는 Elasticsearch 서버는 개발 및 테스트 목적으로 유용하며 프로덕션 환경에서는 사용해서는 안 됩니다. 원격 검색 엔진 설치에 대해 알아보려면 [Elasticsearch 설치하기](./getting-started-with-elasticsearch.md)를 참조하세요.
```
...
| Elasticsearch의 OSS 버전 사용    | Liferay 7.3은 OSS 버전의 Elasticsearch를 사용합니다<br />Liferay DXP 7.4 U17+ 및 Liferay Portal 7.4 GA17+는 더 이상 OSS Elasticsearch를 사용하지 않습니다. |
| <http://localhost:9200>     | <http://localhost:9201>                                                                                                                  |
| 모든 Liferay 배포판에 사전 설치됨      | 항상 사전 설치되어 있는 것은 아닙니다.                                                                                                                   |
| 프로덕션에는 지원되지 않음              | 프로덕션에는 지원되지 않음                                                                                                                           |
| 모든 앱 서버에 특별한 단계가 필요하지 않습니다. | [일부 앱 서버(](#app-server-differences) )에는 추가 단계가 필요합니다.                                                                                    |

## 사이드카/임베디드 Elasticsearch 문제 해결

이러한 오류는 시작 시 Elasticsearch 사이드카 설치에 문제가 있음을 나타냅니다:
...
1. 삭제  `[Liferay 홈]/elasticsearch-sidecar/` 또는 `[Liferay 홈]/elasticsearch7`. 이 폴더에는 사이드카 엘라스틱서치 런타임이 저장됩니다.

1. 삭제 `[Liferay 홈]/데이터/엘라스틱서치7`. 이 폴더에는 인덱스 데이터가 저장됩니다.

1. 라이프레이를 다시 시작합니다.

1. 포털은 시작 시 [에서 일부 앱 서버(예: Tomcat)의](./using-the-sidecar-or-embedded-elasticsearch.md#embedded-versus-sidecar) 에서 Elasticsearch 런타임을 다운로드하고 추출합니다.
