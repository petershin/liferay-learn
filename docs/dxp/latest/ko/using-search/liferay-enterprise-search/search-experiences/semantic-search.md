...
**시맨틱** 검색은 검색된 구 뒤에 있는 의도를 평가합니다. 한편, **키워드** 검색은 검색된 구문을 인덱스 텍스트 필드에 일치시키고 의미를 기반으로 일치를 생성할 수 없습니다.
...
예를 들어 사용자가 **검색하면 스케이트는 어떻게 움직일까요?** 볼 수 있듯이 해양 생물학에 대해 훈련된 모델은 레크리에이션에 대해 훈련된 모델과 다른 결과를 제공합니다.
...
텍스트 임베딩을 색인화하려면 색인 작업 화면을 사용하고 시맨틱 검색 시스템 또는 인스턴스 설정의 자산 항목 클래스 이름 설정에서 활성화한 모델 유형에 대해서만 **재색인** 버튼을 클릭합니다.
