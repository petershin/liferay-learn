...
**頻度の閾値：** 用語がファセット用語のリストに表示されるために必要な最小頻度を設定します。 例えば、あるファセットの頻度閾値が `3`に設定されている場合、一致する結果が2つある用語は、ファセットの用語結果リストで省かれる。

**Order Terms By:** Liferay 7.4 U63+/GA63+では、ファセットのタームソート戦略を選択することができます。条件頻度降順または条件頻度昇順。 用語頻度 降順では、ファセット用語のマッチ数が多いものから少ないものへと並びます。 デフォルトでは、ファセット用語はタイマー期間設定の順番で静的に表示されます。

Ranges Configurationでは、時間範囲の修正、削除、作成を行うことができます。
