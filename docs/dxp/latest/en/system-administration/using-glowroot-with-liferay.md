...
Or if using a Tomcat bundle, run Glowroot from the command line. Navigate to your `$CATALINA_HOME/bin` folder of your bundle. Then execute `./catalina.sh glowroot run`.

## Accessing Glowroot

To access Glowroot, first log in to your Liferay instance with [your administrator account](../getting-started/introduction-to-the-admin-account.md#accessing-liferay-dxp-as-the-admin-user). Note, Glowroot can only be accessed by users with an administrator role.
...
By default, Glowroot is configured with an embedded collector that does not preserve data in the event of a JVM crash. Configure a central collector by creating a `glowroot.properties` file in your `$LIFERAY_HOME/glowroot` folder. Add the following properties,
...
Glowroot can be configured to monitor Liferay upgrades. Create a `config.json` file and place it in your `$LIFERAY_HOME/glowroot/` folder. Copy and paste the following into the JSON file.
...
Note that this configuration is needed for Glowroot to start up at the beginning of when Liferay starts up to monitor the upgrade. Trigger a Liferay upgrade with the [auto upgrade feature](../installation-and-upgrades/upgrading-liferay/upgrade-basics/upgrading-via-auto-upgrade.md).

Once the upgrade is complete, in Glowroot, a new transaction type called upgrade will appear in the dropdown menu. Select `Upgrade` from the dropdown menu to view the dashboard.

![Select upgrade from the dropdown menu to view the dashboard.](./using-glowroot-with-liferay/images/02.png)
