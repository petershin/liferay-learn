...
1. 以下のフィールドに入力します（値の例を次に示します）。
    ...
    * **［Store Currency］**：USD
    * **［Priority］**：1.0
1. _［Account Groups］_下で、_［Select］_をクリックします。
1. この価格表にアクセスする1つ以上のアカウントグループを選択します（アカウントグループの例を以下に示します）。
    * _一般_
    * _自動車修理店_
1. _［追加］_をクリックします。
1. _［Accounts］_下で、_［Select］_をクリックします。
...
価格表（この例では_VIP顧客_）が作成され、選択したアカウントグループとアカウントに適用されました。 必要に応じて、手順を繰り返して他の価格表を作成します。
