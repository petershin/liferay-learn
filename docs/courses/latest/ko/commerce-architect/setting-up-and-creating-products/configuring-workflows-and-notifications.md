...
주문 관리자인 Preston Palmer는 예산상의 이유로 Almighty Syrup의 구매자인 Robert Read가 주문한 내용을 검토하고 승인하려고 합니다. Delectable Bonsai의 B2B 사이트는 기본 제공되는 단일 승인자 워크플로를 사용하여 구매자 주문 승인 워크플로를 사용합니다. 이렇게 하면 구매자가 접수한 모든 주문이 주문 관리자의 승인을 받게 됩니다. 이 워크플로는 구매에 할당된 예산이 있고 승인이 필요할 때 도움이 됩니다.

1. *글로벌 메뉴* (![Global Menu](../../images/icon-applications-menu.png))을 열고 *Commerce* → *Channels*으로 이동합니다.

1. 앞서 생성한 B2B 채널을 선택합니다.

1. 주문 섹션의 구매자 주문 승인 워크플로 드롭다운에서 *단일 승인자(버전 1)* 선택합니다.

   ![단일 승인자 구매자 주문 승인 워크플로를 선택합니다.](./configuring-workflows-and-notifications/images/01.png)
...
1. *전역 메뉴* (![Global Menu](../../images/icon-applications-menu.png))을 열고 *제어판* → *템플릿*으로 이동합니다.

1. *추가* (![Add](../../images/icon-add.png))을 클릭하고 *이메일*선택하십시오.

1. 다음 세부정보를 입력하세요.

   **이름: 맛있는 분재** - 주문 승인 알림 템플릿

   **설명:** Delectable Bonsai에서 주문을 수락할 때 전송되는 이메일에 대한 알림 템플릿입니다.

   **종료:** `[%COMMERCEORDER_AUTHOR_EMAIL_ADDRESS%]`
...
1. *전역 메뉴* (![Global Menu](../../images/icon-applications-menu.png))을 열고 *제어판* → *개체*으로 이동합니다.

1. *상거래 주문* 시스템 개체를 선택합니다.

1. *작업* 탭으로 이동하여 *추가* (![Add](../../images/icon-add.png))을 클릭합니다.

1. 작업 라벨로 *Order Accepted* 입력합니다. 작업 이름이 자동으로 채워집니다.

1. *Action Builder*로 이동합니다.

1. 트리거 아래 드롭다운에서 *주문 상태 업데이트* 을 선택합니다.

1. *활성화 조건* 토글을 활성화하고 다음 표현식을 입력합니다.

   `orderStatus=10`

   각 [주문 상태](https://learn.liferay.com/web/guest/w/commerce/order-management/orders/order-life-cycle) 정수에 해당합니다. 자세한 내용은 아래를 참조하세요.
...
1. 작업 아래에서 *알림* 선택하고 *맛있는 분재 주문 승인* 알림 템플릿을 선택합니다.

   ![개체 작업에 대한 트리거, 조건 및 작업을 입력합니다.](./configuring-workflows-and-notifications/images/02.png)

1. *저장*클릭하세요.

이는 상거래 주문 시스템 개체에 대한 개체 작업을 생성합니다. 주문이 수락될 때마다 트리거되고 템플릿을 기반으로 이메일을 보냅니다.

다음: [창고 생성 및 구성](./creating-and-configuring-warehouses.md)
