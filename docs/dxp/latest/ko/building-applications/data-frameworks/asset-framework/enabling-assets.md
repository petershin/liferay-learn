...
Liferay의 많은 애플리케이션(예: 블로그, 문서 및 미디어, 메시지 보드 등)은 즉시 자산을 사용할 수 있습니다. [자산 게시자 위젯](../../../site-building/displaying-content/using-the-asset-publisher-widget/displaying-assets-using-the-asset-publisher-widget.md) 사용하여 자산을 게시하거나 [자산 라이브러리](../../../content-authoring-and-management/asset-libraries/asset-libraries-overview.md)생성할 수도 있습니다. [서비스 빌더](../service-builder.md) 의 도움으로 맞춤형 애플리케이션을 자산화할 수 있습니다. 자세한 방법은 아래 샘플 프로젝트를 참조하세요.
...
1. 페이지에 S5E6 포틀릿을 추가합니다. 샘플 위젯에서 예제 포틀릿을 찾을 수 있습니다.

1. 또한 자산 게시자 위젯을 동일한 페이지에 추가합니다. 콘텐츠 관리에서 위젯을 찾을 수 있습니다.

1. S5E6 포틀릿으로 항목(예: `s5e6_name1` 및 `s5e6_description1`)을 추가하고 `제출`을 클릭하십시오.

1. S5E6 포틀릿은 자산이 활성화되어 있으므로 추가된 항목이 자산 게시자 위젯에 자동으로 나타납니다.

   ![추가된 항목은 Asset Publisher 위젯에 자동으로 나타납니다.](./enabling-assets/images/01.png)

## 검색 활성화 전제 조건

이 자습서에서는 이미 검색이 활성화된 Service Builder 애플리케이션이 있다고 가정합니다. 애플리케이션에는 최소한 다음이 필요합니다.

`*SearchRegistrar.java` - 애플리케이션의 엔티티에 대한 검색 프레임워크에 검색 서비스를 등록합니다(예: `com.acme.s5e6.search.S5E6SearchRegistrar.java` 클래스).

`*ModelIndexerWriterContributor.java` - 항목의 재인덱싱 및 배치 재인덱싱 동작을 구성합니다(예: `com.acme.s5e6.search.S5E6EntryModelIndexerWriterContributor.java` 클래스).
...
1. 아직 정의하지 않은 경우 다음 데이터 필드를 추가합니다.
   ...
   자산 프레임워크는 애플리케이션의 데이터를 추적하기 위해 이러한 필드가 필요합니다.

1. 닫는 `</entity>` 태그 바로 앞에 자산 항목 엔터티 참조를 추가합니다. 나중에 새 애플리케이션 항목이 추가될 때 해당 항목이 Liferay의 `AssetEntry` 테이블에 추가되도록 변경합니다.
...
자산은 엔터티의 표시 버전이므로 `제목`, `설명`및 `요약`와 같은 필드를 포함합니다. Liferay는 이러한 필드를 사용하여 자산을 표시합니다. 자산 렌더러는 이러한 필드를 통해 엔티티를 자산으로 변환합니다.

1. 응용 프로그램에서 Liferay의 `BaseJSPAssetRenderer` 클래스를 확장하는 `-AssetRender` 클래스를 만듭니다. 예를 들어,
...
3. 다양한 getter 메서드를 사용하여 자산 렌더러를 자산에 연결합니다.
   ...
   이 예제에서 `getTitle()` 메서드는 `이름` 속성으로 설정되고 `getSummary()` 메서드는 응용 프로그램의 `설명` 속성으로 설정됩니다.
...
1. 클래스 선언 위에 `@Component` 주석을 만듭니다. 이 주석은 자산의 팩토리 인스턴스를 등록합니다. `서비스` 요소는 `AssetRenderFactory.class` 인터페이스를 가리켜야 합니다.
...
`S5E6Portlet`의 `addS5E6Entry` 메서드는 `S5E6Entry` 인스턴스를 추가하는 요청을 처리합니다.
