...
MVC 렌더 명령은 렌더링할 페이지를 처리하는 클래스입니다. 이 명령은 `MVCPortlet` 렌더링 URL 및 요청에 의해 호출됩니다. 렌더링 로직이 간단한 경우 [포틀릿 클래스에서 모든 것을 구현할 수 있습니다](./rendering-views-with-mvc-portlet.md). 렌더링 로직이 복잡하거나 렌더링 경로를 깔끔하게 분리하려면 MVC 렌더링 명령을 사용하세요.
...
1. **Go to Baker** 클릭하여 MVC 렌더링 명령을 호출하여 Baker 보기를 방문하십시오. `A4P1BakerMVCRenderCommand` `render` 메서드 호출을 기록하고 Baker 보기를 렌더링합니다. 
...
1. 다른 MVC 렌더링 명령을 호출하고 **Go to Able** 을 클릭하여 Able 보기를 다시 방문하십시오. `A4P1AbleMVCRenderCommand` `render` 메서드 호출을 기록하고 Able 보기를 다시 렌더링합니다. 
...
```{note}
각 포틀릿에 대해 별도의 `javax.portlet.name` 프로퍼티를 선언하여 `MVCRenderCommand` 컴포넌트를 여러 포틀릿과 연결할 수 있습니다:
...
포틀릿이 MVC 명령 이름 `/a4p1/able`지정하는 요청 매개변수를 수신하면 `A4P1AbleMVCRenderCommand`의 `렌더링` 메소드가 실행됩니다. 이 `render` 메서드는 자신을 식별하는 메시지를 기록한 다음 렌더링할 뷰의 경로를 반환합니다.

`A4P1BakerMVCRenderCommand` MVC 명령 이름이 `/a4p1/baker` 이고 렌더링 메서드가 보기 경로 `/a4p1/baker.jsp`반환한다는 점을 제외하면 `A4P1AbleMVCRenderCommand` 과 유사합니다.

이러한 예제 MVC 렌더 명령은 데모 목적으로 사소한 기능을 제공합니다. 보기를 렌더링하는 데 필요한 논리를 사용하여 MVC Render Command의 `렌더링` 메서드를 구현합니다.

예제 URL은 MVC 렌더링 명령을 트리거합니다.

## 포틀릿 렌더링 URL 검사

`able.jsp` 및 `baker.jsp` 파일은 포틀릿 렌더링 URL을 사용하여 서로 간접적으로 연결됩니다. 다음은 `able.jsp`입니다.
...
`baker.jsp` 포틀릿 렌더링 URL `mvcRenderCommandName` 매개변수 값이 `/a4p1/able`이라는 점을 제외하면 `able.jsp` 과 유사합니다. 각 JSP의 `portlet:renderURL` 태그는 MVC Render Command의 `mvc.command.name` 속성 값을 태그의 `mvcRenderCommandName` 포틀릿 매개변수에 할당하여 MVC Render Command에 매핑합니다.
