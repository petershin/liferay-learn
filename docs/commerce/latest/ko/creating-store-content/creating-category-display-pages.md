...
1. 원하는 **채널** 을 선택하고 **범주 디스플레이 페이지** 탭을 클릭합니다. 기본 범주 표시 페이지 섹션에서 **을 클릭하고** 을 선택하고 생성한 페이지를 선택합니다.
...
1. 스토어에서 새 [**위젯 페이지**](https://learn.liferay.com/dxp/latest/en/site-building/creating-pages/understanding-pages/understanding-pages.html#widget-pages) 을 생성하고 원하는 다른 페이지 요소와 함께 **카테고리 콘텐츠** 위젯을 추가합니다.

1. **글로벌 메뉴 열기**(![Global Menu](../images/icon-applications-menu.png)) &rarr; 상거래 탭 &rarr; **매장 관리** &rarr; **채널** .

1. 원하는 **채널** 을 선택하고 **Category Display Pages 탭** 을 클릭한 다음 **Add** 버튼(![Add Button](../images/icon-add.png))을 클릭합니다.
