...
プロジェクトでバージョン `3.x.x` のサービスを使用している場合、CLI ツールで変更をデプロイする前に、まず [](#preparing-lcpjson-files-in-project-version-3) `LCP.json` ファイルを準備する必要があります。
...
1. `<project>\liferay\configs\dev\`にアクセスし、`portal-env.properties` ファイルに以下のプロパティを追加してください。
   ...
   ```{note}
   バージョン3.x.xのサービスを使用している場合は、適切なフォルダパスは、`lcp/liferay/deploy/dev`となります。 ディレクトリ構造の違いについては [Liferay Cloud Project Changes in Version 4](../reference/liferay-cloud-project-changes-in-version-4.md) を、サービスのバージョンの確認方法については [Understanding Service Stack Versions](../reference/understanding-service-stack-versions.md) をご参照ください。
   ```
