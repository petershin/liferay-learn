...
자세한 정보는 [GraphQL 클라이언트](https://graphql.org/graphql-js/graphql-clients/)를 통해 API 또는 해당 설명서를 탐색하여 확인할 수 있습니다. Liferay에는 실행 중인 인스턴스에서 `[server][:port]/o/api` (오른쪽 상단에서 _GraphQL_ 선택)에서 찾을 수 있는 기본 제공 클라이언트가 포함되어 있습니다.
