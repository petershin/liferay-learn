...
1. カスタムレイアウトのカテゴリー設定を有効または無効にします。

   有効にすると、カスタムレイアウトのフィールドタブにカテゴリー設定ブロックを追加できます。 [フィールドタブへのカテゴリー設定の追加](./layouts/adding-categorization-to-fields-tabs.md) を参照してください。
