...
* [내장된 위젯](#include-widgets-within-a-fragment)
* [현지화 가능한 조각 필드](#localizing-fragment-configurations)
* [날짜 조각](#using-date-fragments)
* [객체 작업을 버튼에 매핑](#making-buttons-action-ready)

Liferay Portal 7.3 GA3 및 Liferay DXP 7.3부터 `lfr-editable` 태그로 편집 가능한 요소를 래핑하는 대신 `data-lfr-editable*` 속성을 사용하여 요소를 편집 가능으로 정의할 수 있습니다.
...
값을 재평가해야 하는 경우에는 사용하지 마세요. 예를 들어 `${언어Util.get(locale,'word')}`를 사용하여 값을 현지화하는 경우 해당 단어는 페이지에 조각을 추가할 때만 현지화됩니다. 포털의 언어를 변경하더라도 코드가 다시 실행되지 않습니다.
```
...
![편집 가능한 요소를 사용하여 편집 가능한 텍스트, 이미지 및 링크가 포함된 조각을 만들 수 있습니다. 이 조각은 편집하여 다양한 컨텍스트에서 재사용할 수 있습니다.](./fragment-special-tags-reference/images/01.png)
...
이미지 요소에 `data-lfr-editable-type="text"` 속성을 포함하여 조각 텍스트를 편집 가능하게 만들 수 있습니다. `data-lfr-editable-id`는 다음 예와 같이 고유 ID여야 합니다.
...
텍스트나 색상 스타일과 같은 서식 옵션이 필요한 경우 `rich-text`를 사용하세요.
...
이미지는 텍스트와 동일한 `data-lfr-editable-type` 속성을 사용하지만 `image` 유형을 사용합니다. `data-lfr-editable-id`는 고유 ID여야 합니다.
...
'image' 유형의 'lfr-editable' 태그를 조각에 추가한 후 해당 조각을 페이지에 추가하면 편집 가능한 이미지를 클릭하고 콘텐츠 페이지 편집기에서 이미지 소스 및 기타 속성을 구성할 수 있습니다. 사이드바.

![You have several options for defining an image on a content page.](./fragment-specific-tags-reference/images/02.png)

대부분의 이미지는 이와 같이 처리할 수 있지만 편집 가능한 배경 이미지를 추가하려면 배경 이미지 ID `data-lfr-Background-image-id`를 설정하는 추가 속성을 추가해야 합니다. 배경 이미지 ID는 조각의 기본 `div`에 설정되며 편집 가능한 이미지 ID와 동일합니다.
...
`data-lfr-editable-type`을 `link`로 설정하여 일반 링크 요소를 편집 가능하게 만들 수 있습니다. `data-lfr-editable-id`는 고유 ID여야 합니다.
...
편집 가능한 링크에 대한 자세한 내용은 [편집 가능한 링크](../../../creating-pages/page-fragments-and-widgets/using-fragments/configuring-fragments/fragment-sub-elements-reference를 참조하세요. md#링크-설정) .

## 편집 가능한 HTML 만들기

`data-lfr-editable-type` 속성을 `html`로 설정하여 일반 HTML 요소도 편집 가능하게 만들 수 있습니다.
...
또한 `PortletRegistry`를 사용하여 포틀릿의 별칭을 등록 및 등록 취소하려면 `@Activate` 및 `@Deactivate` 라이프 사이클 메소드를 설정하세요.
...
페이지의 대상 언어에 대한 조각 구성을 현지화할 수 있습니다. 예를 들어, 버튼 조각에서 페이지 언어가 en-US인 경우 하나의 버튼 유형을 정의하고 페이지 언어가 es-ES인 경우 다른 버튼 유형을 정의할 수 있습니다. 프래그먼트 구성 필드를 현지화하려면 'localized' 속성을 사용하세요.

```{note}
`configurationRole` 속성이 `style`로 설정된 조각 구성 필드에는 `localized` 속성을 사용할 수 없습니다.
```

다음 코드 발췌에서 버튼 프래그먼트 구성은 `fieldSets` 아래의 `fields` 섹션에 대해 `localized` 속성을 `true`로 설정합니다. 'localized' 속성은 필드 수준에서 설정됩니다. 이 예에는 `buttonType` 필드가 하나만 있습니다. 여러 필드가 있는 프래그먼트가 있는 경우 각 필드에 `localized` 속성을 설정할 수 있습니다.
...
언어에 대한 맞춤 구성을 지정하지 않는 'localized' 속성이 있는 조각은 기본 페이지 언어 구성을 사용합니다.

## 날짜 조각 사용

지역의 날짜 형식을 준수하는 것은 여러 상황에서 어려울 수 있습니다. 조각에 `data-lfr-editable-id="date-time"` 및 `data-lfr-editable-type="date-time"` 속성을 포함하면 날짜 조각을 사용하여 날짜 형식을 사용자 정의하고 현지화할 수 있습니다. `data-lfr-editable-id`는 다음 예와 같이 고유 ID여야 합니다.
...
기본적으로 가장 일반적인 날짜 형식을 선택하거나 [`SimpleDateFormat`](https://devdocs.io/openjdk~8/java/text/simpledateformat) 에 따라 날짜 형식을 맞춤설정할 수 있습니다.
...
`data-lfr-editable`을 포함하여 버튼 구성요소를 사용하여 [객체 작업](../../../../building-applications/objects/creating-and-managing-objects/actions.md)을 트리거할 수 있습니다. -id="action"` 및 `data-lfr-editable-type="action"` 속성을 버튼 태그에 추가합니다. `data-lfr-editable-id`는 다음 예와 같이 고유 ID여야 합니다.
