...
[검색 청사진과 함께 제공되는 시스템 요소](../search-blueprints-elements-reference.md)가 많이 있습니다. 어떤 시스템 요소도 요구 사항을 충족하지 못하는 경우 고유한 검색 청사진 요소를 만드는 다양한 방법이 있습니다.
...
* [편리한 미리보기 기능과 시스템의 미리 정의된 템플릿 변수 목록을 포함하여 보다 강력한 요소 편집기를 사용하여 요소를 작성하려면 요소 추가 소스 편집기](#using-the-add-element-source-editor) 을 사용하십시오.
* [시스템 요소](./managing-elements.md) 을 복제하여 고유한 사용자 정의 요소 개발을 시작하십시오.
* [Elasticsearch 쿼리 DSL](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/query-dsl.html) 사용에 익숙하고 Elasticsearch 쿼리를 기반으로 요소를 빠르게 요리해야 하는 경우 [Paste Any Elasticsearch 쿼리 시스템 요소](../search-blueprints-elements-reference.md#paste-any-elasticsearch-query) 을 사용합니다.
...
JSON 편집기에 입력하면 자동 완성 기능이 사용 가능한 속성을 표시합니다. 사용 가능한 JSON 속성을 자세히 이해하려면 [스키마 이해](#understanding-the-element-schema)을 참조하십시오.
...
이 요소는 블루프린트의 매개변수 구성을 사용하여 추가할 수 있는 사용자 지정 변수에 따라 다릅니다. 청사진에 사용자 지정 변수를 추가하는 방법에 대해 알아보려면 [검색 청사진 구성 참조](../search-blueprints-configuration-reference.md#parameter-configuration) 을 참조하십시오.
...
요소 소스 편집기에서 미리 정의된 변수를 요소에 추가하려면 변수가 있을 사이트에 커서를 놓은 다음 왼쪽 사이드바에서 변수를 클릭하여 삽입하십시오. Custom JSON Element에서 변수를 편집기에 직접 입력해야 합니다.
...
각 요소에는 `elementDefinition` 및 `title_i18n`의 두 가지 필수 최상위 속성이 있습니다. `elementDefinition` 은 `카테고리` 및 `구성` &rarr; `queryConfiguration` 속성을 포함해야 합니다.
...
- `카테고리` 은 요소의 동작을 분류하는 문자열을 제공합니다. `일치`, `부스트`, `조건부`, `필터`, `숨기기`또는 `사용자 지정`을 지정합니다.
- `구성` 은 `queryEntries` 속성을 통해 기여하는 쿼리 절을 보유하는 `queryConfiguration`을 제공합니다.
- `icon` 은 요소에 사용할 수 있는 아이콘을 설정하는 문자열을 설정합니다. [어휘 아이콘 라이브러리](../../../../building-applications/developing-a-java-web-application/using-mvc/tag-libraries/clay-tag-library/clay-icons.md) 에서 사용할 수 있는 모든 이미지를 사용할 수 있습니다(예: `엄지 위로`).
- `uiConfiguration` 은 UI에 표시할 구성 요소를 설정한 다음 구성된 값과 함께 사용자 지정 요소로 전달합니다.
...
`구성` 속성은 중첩된 속성 `queryConfiguration` &rarr; `queryEntries`을 보유합니다.

`queryEntries` JSON으로 다이빙하면 속성 `절`, `조건`, `활성화`, `postFilterClauses`및 `rescores`을 포함할 수 있습니다. 

- 위 스니펫은 `절` (요소 배열)을 사용하여 요소가 블루프린트 기반 검색에 기여하는 쿼리 절을 추가하는 방법을 보여줍니다. `쿼리`외에 `추가`, `부스트`, `콘텐츠`, `비활성화`, `필드`, `이름`, `발생`, `부모`, `유형`또는 `값` 속성.
- `조건` 은 부울 검사를 제공합니다. true인 경우 제공된 절이 검색 쿼리에 포함되고 false인 경우 제외됩니다.
- 요소를 비활성화하려면 `활성화됨` 을 거짓(기본적으로 true임)으로 설정합니다.
- `postFilterClauses` 을 추가합니다( `절` 요소의 배열). `추가`, `부스트`, `콘텐츠`, `비활성화`, `필드`, `이름`, `발생`, `부모`, `쿼리`, `유형`또는 `값` 속성을 추가할 수 있습니다.
- 쿼리 결과에 대한 관련성 점수를 다시 계산하려면 `rescores` 을 추가합니다. `쿼리`, `queryWeight`, `rescoreQueryWeight`, `scoreMode`및 `windowSize`를 지정합니다. 자세한 내용은 [Elasticsearch 설명서](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/filter-search-results.html#rescore) 을 참조하십시오.

`uiConfiguration` 속성은 중첩된 속성 `fieldSet` 및 `field`를 보유합니다.

UI 구성의 각 필드에 대해 추가할 수 있는 여러 가지 구성 속성이 있습니다.

- `defaultValue` 을 사용하여 필드의 초기 값을 입력하십시오.
- Blueprints UI의 필드에 대한 도움말 텍스트를 표시하려면 `helpText` 을 입력합니다.
- 사람이 읽을 수 있는 `라벨`을 입력하세요.
- 쿼리 절에 필드 값을 전달할 때와 같이 요소의 다른 사이트에서 필드를 참조하는 데 사용할 수 있는 `이름` 을 입력합니다: `${configuration.name}`.
- 필드의 `유형` 을 설정합니다. `date`, `fieldMapping`, `fieldMappingList` (사용 가능한 필드 목록, 해당 로케일 및 필드 부스트 포함), `itemSelector`, `json`, `키워드`, `multiselect`, `number`중에서 선택 `선택`, `슬라이더`및 `텍스트`
- `typeOptions` 속성을 사용하여 각 필드 유형에 사용할 수 있는 옵션을 구성합니다. 예를 들어 선택 필드에 대해 `옵션` 표시를 설정합니다. 
   - `부스트` 은 필드당 숫자 부스트 값을 설정합니다.
   - 'format'은 [허용되는 날짜 형식](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/mapping-date-format.html)을 결정합니다(예: `yyyyMMddHHmmss`).
   - `nullable` 은 null 값을 전달할 수 있는지 여부를 설정합니다.
   - `옵션` 은 선택 필드의 옵션을 설정합니다.
   - `required` 구성에 값이 있어야 하는지 여부를 설정합니다.
   - `단계` 은 숫자 또는 슬라이더 필드의 숫자 증가 또는 감소 값을 설정합니다.
   - `단위` 은 숫자 필드의 측정 단위를 설정합니다.
   - `unitSuffix` 은 단위가 있는 숫자 필드에 표시할 단위 표기법을 설정합니다(예를 들어, `unit` 이 ``인 경우 `unitSuffix` 을 `` 또는 ``로 설정할 수 있음).
