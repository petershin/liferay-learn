...
```{important}
Liferay DXP 7.4および7.3.10 FP1から、アセットライブラリでステージングがサポートされています。 ただし、ステージングされたサイトとステージングされたアセットライブラリのみを接続し、その逆もまた然りです。 詳しくは、[Using Staging in Asset Libraries](./staging/using-staging-in-asset-libraries.md) をご覧ください。
```
...
サイトのステージングを構成するときに、*ページバージョニング*機能を有効にできます。 この機能により、サイトのコンテンツページとウィジェットページのバリエーションを同時に開発することができます。 ページバージョンの完全な履歴を確認し、必要に応じてページを以前のバージョンに戻すことができます。 バリエーションの作成と管理の方法については[Page Versioning](./staging/page-versioning.md)を、各バリエーションの権限を管理する方法については[ステージング権限の管理](./staging/managing-staging-permissions.md)を参照してください。
