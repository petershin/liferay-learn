...
| `LCP_WEBSERVER_MODSECURITY`               | `Off`       | ModSecurity를 활성화하거나 비활성화합니다. 규칙을 활성화하고 처리하려면 이 값을 'On'으로 설정하고, 중단 작업을 실행하지 않고 규칙을 처리하려면 'DetectionOnly'로 설정하세요. [웹 응용 프로그램 방화벽](../infrastructure-and-operations/security/web-application-firewall.md) 참조하세요. |
