...
* **アンチウイルス** ：ファイルアップロード時にウイルスをスキャンするためのデフォルトのLiferayDXP機能は使用できません。 Liferay Cloudの [アンチウイルスソリューション](./liferay-cloud-infrastructure.md#antivirus) が代わりに使用されます。 アップロードされたコンテンツはスケジュールに沿ってスキャンされるため、ファイルがアップロードされた時点ではリスクが検出されない場合があります。
...
* **最大カスタムドメイン数** ：(デフォルトのWebサーバーに加えて)複数のサービスを環境外に公開している場合、 [最大カスタムドメイン数](../infrastructure-and-operations/networking/custom-domains.md) が50個まで設定されています。 ただし、ウェブサーバーが唯一のエントリーポイントである場合には、1500個のカスタムドメインの制限を使用することができます。 Webサーバーのサービスを変更して、この制限を超えて使用量を増やす場合は、 [サポートチケットを発行する必要があります](https://help.liferay.com/) 。

* **最大SSL証明書** ：最大14個のカスタムSSL証明書を許可します。 また、証明書を発行するプロバイダーは、これを少なくするために独自の制限を課すことがあります。

* **送信接続** : プロジェクト環境から外部エンドポイントへの接続には、1接続あたり120秒のタイムアウトを伴うNATソリューションが使用されます。 短時間に次々と新しい接続が作成されると、ポートの枯渇や送信トラフィックのドロップにつながる可能性があります。 コネクションプーリングは、この問題を防ぐことができます。
