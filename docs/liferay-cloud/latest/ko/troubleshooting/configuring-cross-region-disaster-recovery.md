...
VPN 연결에 대한 자세한 내용은 [VPN 연결](../infrastructure-and-operations/networking/connecting-a-vpn-server-to-liferay-cloud.md)을 참조하십시오.
...
* **LCP** EXTERNAL **PROJECT_ID** : 프로덕션 환경의 프로젝트 ID(예: `acme-prd`)

* **LCP** BACKUP **RESTORE_SCHEDULE** : 자동 백업 빈도를 정의하는 [cron 스케줄링](https://crontab.guru/) 값. 자세한 내용은 [자동 백업 및 정리 예약](../platform-services/backup-service/backup-service-overview.md#scheduling-automated-backups-and-cleanups) 을 참조하십시오.

DR 환경에서 이 값을 [비밀](../infrastructure-and-operations/security/managing-secure-environment-variables-with-secrets.md) 로 설정합니다.

* **LCP** EXTERNAL **PROJECT** MASTER **TOKEN** : 프로덕션 환경의 마스터 토큰
...
1. DR 환경의 **설정** &rarr; **VPN** 페이지로 이동하여 VPN이 DR 환경에 연결되어 있는지 확인합니다.
    ...
    적절한 VPN이 연결되지 않은 경우 연결을 설정하십시오. 자세한 내용은 [VPN 서버를 Liferay Cloud](../infrastructure-and-operations/networking/connecting-a-vpn-server-to-liferay-cloud.md) 에 연결을 참조하십시오.
...
1. DNS 레코드를 업데이트하고 DR 환경에 사용자 지정 도메인을 추가합니다. 자세한 내용은 [사용자 정의 도메인](../infrastructure-and-operations/networking/custom-domains.md)을 참조하십시오.
...
1. DNS 레코드를 업데이트하고 프로덕션 환경에 사용자 지정 도메인을 다시 추가합니다. 자세한 내용은 [사용자 정의 도메인](../infrastructure-and-operations/networking/custom-domains.md)을 참조하십시오.
