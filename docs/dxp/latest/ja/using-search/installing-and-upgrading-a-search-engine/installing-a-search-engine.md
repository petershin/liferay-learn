...
* 検索エンジンは環境変数 `JAVA_HOME` または `ES_JAVA_HOME` ( [Elasticsearch 8](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/install-elasticsearch.html#jvm-version) ) を必要とする。 それを検索エンジンのホストに設定します。
...
Elasticsearch 7 コネクタは HTTP 経由で通信するため、Java のバージョンと配布の要件は Liferay 7.3+ には適用されません。 詳細は [Elastic の High-Level REST Client](https://www.elastic.co/guide/en/elasticsearch/client/java-rest/7.x/java-rest-high.html) のドキュメントを参照してください。
