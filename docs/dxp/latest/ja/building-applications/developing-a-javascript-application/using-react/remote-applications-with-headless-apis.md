...
1. *グローバル メニュー* (![グローバル メニュー](../../../images/icon-applications-menu.png)) を開き、*コントロール パネル* タブをクリックして、*システム設定に移動します。* &rarr; *セキュリティ ツール*。

1. *Portal Cross-Origin Resource Sharing (CORS)* タブをクリックし、*Default Portal CORS Configuration* をクリックします。

1. `/o/c/*` 値で *URL パターン* を追加し、*保存* をクリックします。 これにより、すべてのオブジェクト API で CORS が有効になります。

1. **追加**ボタン(![ボタン追加](../../../images/icon-add.png))をクリックし、以下の値を入力します：

   | フィールド | 値 | 
   | :--- | :--- |
   | ラベル | J4X7-Object | J4X7-Object | 
   | 複数形ラベル | J4X7-Objects |
   | 名前 | J4X7Object |
