...
1. Liferay Experience Cloud(LXC)를 사용하는 경우 [`lcp` CLI 도구](https://learn.liferay.com/liferay-cloud/latest/en/reference/command-line-tool.html)이 있어야 합니다.
...
`into`: 빌드된 `.zip` 파일에서 추가 리소스를 포함할 위치를 지정합니다. 예를 들어 많은 프런트 엔드 클라이언트 확장(예: JavaScript 또는 CSS 클라이언트 확장)의 소스 파일은 Liferay가 사이트의 정적 리소스로 제공할 수 있도록 `static` 디렉토리에 있어야 합니다.
