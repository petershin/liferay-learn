...
```{warning}
Instance Settingsのユーザーインターフェースは自動生成され、SAMLの設定を容易にするフィールド検証やその他の機能は提供されない。 SAMLインスタンスを設定するには、 [SAML管理](./saml-admin.md) インターフェースを使用してください。 Instance Settingsインターフェイスを使用して[設定ファイル](./saml-settings.md)を作成したい上級者向けのドキュメントをここに掲載します。
```
