...
![대체 경로로 기본 사용자 지정 요소 원격 응용 프로그램을 만들고 경로 속성을 사용하여 렌더링되는 항목을 구성합니다.](./using-routes-with-custom-elements/images/01.png)
...
1. 애플리케이션이 성공적으로 생성되었는지 확인합니다.
   ...
   이 스크립트는 다음 요소를 사용하여 `j1v3-remote-app` 이라는 새 React 애플리케이션을 만듭니다.
...
J1V3-Remote-App은 인스턴스화할 수 있으므로 각각 고유한 독립적인 구성을 가진 많은 앱을 페이지에 추가할 수 있습니다. 이 예에서는 페이지에 위젯을 두 번 추가합니다.
...
1. 속성 필드에 `route=hello-foo` 입력합니다.
...
1. 속성 필드에 `route=hello-bar` 입력합니다.
...
이 `index.js` 파일은 `HTMLElement` 인터페이스를 확장하는 `WebComponent` 클래스를 생성합니다. 이 클래스는 `App` 를 매개변수로 [`ReactDOM.render`](https://reactjs.org/docs/react-dom.html#render) 호출하는 인터페이스의 [`connectedCallback()`](https://developer.mozilla.org/en-US/docs/Web/Web_Components/Using_custom_elements#using_the_lifecycle_callbacks) 함수를 구현합니다. `App` 이 호출되면 정의된 `"경로"` 속성을 확인하고 해당 값을 사용 가능한 경로와 비교합니다. `hello-foo` 또는 `hello-bar`과 일치하면 해당 경로를 반환하고 렌더링합니다. 그렇지 않으면 `hello-world`반환하고 렌더링합니다.
