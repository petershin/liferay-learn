...
**반환할 필드:** 검색 엔진에서 반환할 저장된 필드의 이름을 쉼표로 구분된 목록으로 입력합니다. 저장된 모든 필드를 반환하려면 비워 두십시오.
...
[**Liferay 7.4+] 속성:** 검색 컨텍스트에 키/값 속성을 추가합니다. 이 섹션은 [검색 Blueprint 적용](../../liferay-enterprise-search/search-experiences/using-a-search-blueprint-on-a-search-page.md)을 검색에 적용하는 데 가장 유용합니다. 페이지. 개발 및 테스트 목적으로도 사용할 수 있습니다. 맞춤 코드에서 읽을 수 있는 페이지의 검색 컨텍스트에 속성을 추가합니다(예: `searchContext.getAttribute("key")`).

## 낮은 수준의 검색 결과 표시

Liferay 회사 인덱스(예: `liferay-20097`)로 인덱싱된 자산은 `ModelSummaryContributor` 구현하여 검색 결과 위젯에서 사용되는 자체 디스플레이 논리를 제공할 수 있습니다. 낮은 수준의 검색은 가져올 요약이 없기 때문에 다른 표시 논리를 사용합니다.
...
1. 옵션 아이콘(![Options icon](../../../images/icon-app-options.png))을 클릭하고 **구성** 선택하여 대체 검색에 참여하도록 모든 위젯을 구성합니다. 각각에 대해 Federated Search Key 설정에 **liferay-0** 입력합니다.
...
1. 검색 표시줄의 기본 쿼리 매개변수(**q**)를 사용하고 검색에 쿼리를 추가하도록 사용자 지정 필터를 구성합니다.
   ...
   Custom Parameter Name 필드에 **q** 입력합니다.
   
   필터 필드 아래에 **제목** 입력하여 쿼리에 제목 필드를 추가합니다.
   
   필터 쿼리 유형에서 **일치** 선택합니다.
   
   대체 색인을 검색하기 위해 기본 쿼리를 재정의하고 있으므로 기본적으로 쿼리에는 아무것도 없습니다. 검색 엔진으로 보내려는 모든 쿼리 절은 [사용자 정의 필터](./filtering-search-results.md) 위젯을 사용하여 수동으로 추가해야 합니다. 
...
1. 이 위젯을 [Low Level Search 라는 새 페이지](./../../../site-building/creating-pages/using-widget-pages/adding-widgets-to-a-page.md) 에 추가합니다.
   ...
   * 하나 [사용자 지정 필터](./filtering-search-results.md) 
...
1. 위젯을 구성합니다.
   ...
   | 검색 결과(두 번째) | **통합 검색 키:** `liferay-0`                                                                                                                             | 표시할 필드 제어와 같은 추가 구성이 있습니다. 이 옵션을 비워두면 [Liferay가 알아낼 것을 요청합니다](#displaying-low-level-search-results) .                                                                      |
...
1. 사이트에 하나 이상의 자산(예: 블로그 항목, 웹 콘텐츠 기사 등)을 추가하고 생성한 모든 자산에 **콘텐츠** 단어를 포함합니다. 

1. 하위 수준 검색 페이지의 검색 표시줄에 **콘텐츠** 입력하고 두 인덱스 모두에서 결과를 얻는지 확인합니다.
