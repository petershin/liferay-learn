...
```{note}
Liferay Commerce 2.1以前のバージョンでは、［*サイト管理*］ → ［*コマース*］ → ［*設定*］ → ［*支払い方法*］に移動します。
```
