...
作成されたパーソナルアクセストークンは、 **repo** と **admin:repo***hook** 以下のすべてのスコープ権限にチェックが入っていなければなりません。 **admin:repo***hook** scope permissions specifically は、リポジトリが DXP Cloud と正常に統合された後に [自動ウェブフック管理を無効に](#personal-access-token-usage) しておけば、後で削除することが可能です。
