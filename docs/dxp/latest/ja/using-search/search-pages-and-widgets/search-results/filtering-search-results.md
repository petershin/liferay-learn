...
| ユースケース | `構成` _(値)_ |
| :--- | :--- |
| **ユーザー タイトル フィルター:** エンド ユーザーは、タイトル |`フィルター フィールド` _(title\_en\_US)_  `フィルター値 ` _(podcast)_  `Filter Query Type` _(Match)_  `Occur Type` _(must_not)_|
| **ハードコーディングされたフィルタリング:** 管理ユーザーは、エンド ユーザーには見えず、変更できないフィルターを使用して検索ページを構成します。 **_上記の設定に追加_**  `Invisible` _(True--checked)_  `Immutable` _(True--checked)_|
