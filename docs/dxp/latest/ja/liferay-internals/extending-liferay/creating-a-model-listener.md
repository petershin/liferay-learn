...
次に、以下の手順で `JournalArticle` モデルのモデルリスナーの例をデプロイします：

1. `Acme Model Listener`をダウンロードし、解凍する。
...
1. 追加されたログメッセージを表示して、サンプルモデルリスナーが追加されたことを確認します。 ブラウザーで `http://localhost/8080` を開き、［サイト］メニュー→ _［コンテンツ & データ］_ → _［Webコンテンツ］_へ行きます。
...
1. `MBMessage` の新しいインポートをファイルの上部にあるインポートセクションに追加します。
   ...
   `JournalArticle`の未使用のインポートを削除します。
...
1. メニューから［ *掲示板* ］をもう一度クリックして、メッセージを表示します。 アクション ![Action](../../images/icon-actions.png) ボタンをクリックし、 *削除*を選択します。 メッセージはリサイクルされただけなので、まだログにメッセージが表示されていないことに注意してください。
