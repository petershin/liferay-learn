...
이 참조에는 조각에 사용 가능한 구성 유형이 나열되어 있습니다. 조각을 구성 가능하게 만드는 방법에 대한 자세한 내용은 [조각에 구성 옵션 추가](../../developing-page-fragments/adding-configuration-options-to-fragments.md)를 참조하세요.
...
* **스타일북의 값** 버튼을 클릭하면 현재 사용되는 [스타일 북](../../../site-appearance/style-books/using-a-에 정의된 색상을 선택할 수 있는 사전 정의된 색상 메뉴가 열립니다. 스타일북-표준화-사이트-appearance.md). 그러면 버튼을 다시 눌러 연결을 해제할 때까지 필드 값이 선택한 토큰에 연결됩니다. 토큰 값의 연결을 해제하면 선택한 색상이 해당하는 16진수 코드 값으로 다시 변환됩니다.
...
- 현재 테마에 사용할 토큰 정의가 없으면(그래서 [색상 팔레트](#color-palette-configuration) 가 대신 사용됨) CSS 색상(예: `rgb(255, 0, 0)`).

## 길이 구성

{bdg-secondary}`사용 가능한 Liferay 7.4 U44 및 GA44+`
...
이 예에서는 `metadataset-structure-key-2` 구조를 사용하는 `img` 또는 `jpg` MIME 유형의 문서만 선택할 수 있도록 지정합니다.
...
콘텐츠의 특정 부분에 액세스해야 하는 경우 `[name-of-field]Object` 키(아래 예에서는 `itemSelector1Object`
) 아래 프래그먼트의 Java 객체에 액세스할 수도 있습니다. 이 예에서는 웹 콘텐츠 기사의 제목, 설명 및 본문을 렌더링합니다.
...
이 구성으로 조각을 구성할 때 드롭다운 메뉴에서 URL을 수동으로 입력('URL')하거나 동일한 사이트에서 다른 페이지를 선택하고 전체 URL('페이지')을 사용하도록 선택할 수 있습니다.
...
`videoSelector` 유형을 사용하면 [외부 비디오](../../../creating-pages/page-fragments-and-widgets/using-fragments/default-fragments-를 통합하는 비디오 선택기를 만들 수 있습니다. reference.md) 다른 조각의 조각.
...
`collectionSelector` 구성 유형을 사용하면 [collection](../../../../content-authoring-and-management/collections-and-collection-pages/about-collections-) 을 포함하는 조각을 개발할 수 있습니다. and-collection-pages.md) 또는 컬렉션 제공자입니다. 수동 컬렉션과 동적 컬렉션 모두에서 `collectionSelector`를 사용할 수 있습니다.

```{note}
개발자는 컬렉션 공급자를 사용하여 고급 기준으로 특정 컬렉션을 만들 수 있습니다. 자세한 내용은 [정보 프레임워크](https://help.liferay.com/hc/ko/articles/360029067271-Creating-an-Information-List-Provider) 개발자 문서에서 [정보 목록 공급자 만들기](https://help.liferay.com/hc/ko/articles/360029067251-Introduction-to-The-Info-Framework) 에 대한 정보를 읽어보세요.
```
...
다음 HTML 코드 샘플과 함께 이 조각 구성을 사용하여 컬렉션 항목을 나열할 수 있습니다. `collectionObjectList`는 [콘텐츠 페이지 편집기](../../../creating-pages/using-content-pages/content-page-editor-ui-reference.md)에서 선택된 컬렉션을 나타냅니다.
...
색상 팔레트 JSON 구성은 색상을 선택해야 하는 경우에 구현할 수 있는 색상 선택기를 생성합니다. [색상 선택기 구성](#color-picker-configuration) 과 달리 현재 사용되는 [Style Book](../../../site-appearance/style-books/using-a-style-book-to-standardize-site-appearance.md)의 색상 시스템에 구성된 테마 색상을 기준으로 옵션만 제공합니다.
