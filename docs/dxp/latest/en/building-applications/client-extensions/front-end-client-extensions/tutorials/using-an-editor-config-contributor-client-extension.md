...
Editor Config Contributor client extensions override existing CKEditor customization done in a `ConfigContributor.java` file. Start with a client extension (from the [sample workspace](https://github.com/liferay/liferay-portal/tree/master/workspaces/liferay-sample-workspace)).
...
1. Install Java (JDK 8 or JDK 11).
   ...
   <!-- I know we're not supposed to use help links, but I didn't find this matrix in learn and the other articles are all pointing to the same link. Eric -->
...
This specifies that everything in the `build/` folder should be included as a static resource in the built client extension `.zip` file. The JS file in an Editor Config Contributor client extension is used as a static resource in Liferay.
...
const editorConfigTransformer = (config) => {
	...
	// Adds an "aicreator" button to the list of extra plugings.
...
1. If the deployment is successful, the Liferay Sample Editor Config Contributor client extension is listed in the client extensions manager.

   ![If the deployment is successful, ](./using-an-editor-config-contributor-client-extension/images/01.png)
