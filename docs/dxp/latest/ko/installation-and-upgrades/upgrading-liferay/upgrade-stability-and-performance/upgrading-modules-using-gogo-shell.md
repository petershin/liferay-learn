...
예를 들어, 모듈은 새로운 [데이터 스키마 마이크로 버전](https://help.liferay.com/hc/en-us/articles/360030959231-Meaningful-Schema-Versioning)을 가질 수 있습니다. 새 마이크로 버전일 뿐이므로 모듈 데이터를 새 스키마로 업그레이드하는 것은 선택 사항입니다. 새 모듈 버전을 배포하면 활성화되지만 해당 데이터는 업그레이드되지 않습니다. 모듈을 새 데이터 스키마로 업그레이드하려는 경우 Gogo Shell에서 `업그레이드` 명령을 사용할 수 있습니다.

그러나 데이터를 업그레이드하지 않고 새로운 [데이터 스키마 마이너/메이저 버전](https://help.liferay.com/hc/en-us/articles/360030959231-Meaningful-Schema-Versioning) 이 있는 모듈을 배포하면 모듈이 비활성화됩니다. 새 모듈 버전을 활성화하려면 모듈의 데이터를 새 데이터 스키마로 업그레이드해야 합니다.
