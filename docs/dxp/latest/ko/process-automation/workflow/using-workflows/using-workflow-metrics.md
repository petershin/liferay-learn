...
_Workflow Metrics_을 사용하려면 Elasticsearch를 사용하여 DXP 데이터를 인덱싱해야 합니다. 자세히 알아보려면 [Elasticsearch 설치](../../../using-search/installing-and-upgrading-a-search-engine/elasticsearch/installing-elasticsearch.md) 기사를 읽어보세요.
...
1. 프로세스에 대한 SLA가 없는 경우 많은 양을 알리는 경고 메시지가 나타납니다. 새 SLA 양식에 직접 액세스하려면 경고에서 _새 SLA 추가_ 링크를 클릭하십시오.
   ...
   또는 옵션(![Options](../../../images/icon-options.png)) 메뉴를 클릭하고 _SLA 설정_를 선택합니다.
...
| 시간    | 시간과 분을 입력합니다. _시간_ 상자의 값은 _23:59_을 초과하지 않아야 합니다. 기간이 1일을 초과하는 경우 _일_ 필드와 함께 사용합니다. |
...
2. SLA 작업 간격 확인 구성 값을 변경하고 _저장_을 클릭합니다.
...
1. 메트릭 애플리케이션에서 옵션 메뉴(![Options](../../../images/icon-options.png))를 열고 _설정_를 클릭합니다.

2. 이제 Workflow Index Actions 화면에서 Workflow Indexes 항목에 대해 _Reindex All_ 을 클릭합니다.
...
워크플로 프로세스의 전체 메트릭에서 워크플로의 단일 항목에 대한 세부 정보에 이르기까지 새로운 워크플로 메트릭 기능은 Liferay _에서 작업을 완료하는 데 걸리는 시간에 대한 통찰력을 제공_.
...
1. _Total Pending_ 상자 위로 마우스를 이동하고 _See Items_ 을 클릭하여 Pending 상태의 보류 중인 워크플로 항목 테이블을 엽니다.
...
1. 재할당할 작업 선택 모달에서 작업을 다시 선택하고 _다음_을 클릭합니다.
