...
Clarity is all set up for authenticating its own team members through Okta, but they need to decide what to do for other visitors that come to their site that require login. Fortunately Liferay provides various security and identity management tools. Clarity plans to require their B2B customers to login with strong passwords and multi-factor authentication.

## Configuring a Password Policy

Clarity understands the importance of requiring strong passwords to make user login secure. They want to increase password strength by adjusting the default password policy in two ways: requiring special characters and having password expirations. To modify the default password policy,
...
Clarity make further changes by creating custom password policies for different types of users. For example, one password policy for B2B customers and another password policy for retail distributors. See our documentation on [password policies](https://learn.liferay.com/w/dxp/users-and-permissions/roles-and-permissions/configuring-a-password-policy) to learn more.

## Enabling Multi-factor Authentication

Clarity wants to further increase login security by requiring multi-factor authentication (MFA). They want each login to be authenticated though an email one-time password. To enable this MFA,
...
!!! note
   For this feature, your [Liferay email service must be configured](https://learn.liferay.com/web/guest/w/dxp/installation-and-upgrades/setting-up-liferay/configuring-mail).

In addition to email password, Liferay supports other types of MFA:
...
See our documentation about [securing Liferay](https://learn.liferay.com/web/guest/w/dxp/installation-and-upgrades/securing-liferay) for full explanation about each MFA option as well as the other security features.
