...
|:-------- |:------------------------- |
| フレンドリURL | 商品ページのURL                 |
| メタタイトル   | 検索エンジンが商品を見つけるのに役立つ商品タイトル |
| 説明       | 検索エンジンで使用される商品の説明         |
| メタキーワード  | 検索エンジンで使用されるキーワード         |
...
| 公開開始日時   | 商品がカタログで最初に販売可能になった日付                     |
| 有効期限     | *[Never Expire]*がオフになっている場合、商品の販売終了日を決定する |
| 期限を設定しない | 商品の有効期限をスケジュールできるかどうかを決定する                |
...
デフォルトでは、商品はすべてのチャネルとカタログに表示されますが、特定のチャネルやアカウントグループに表示を制限することもできます。 詳細は、[Configuring Product Visibility Using Channels](../../../store-management/channels/configuring-product-visibility-using-channels.md)および[Configuring Product Visibility Using Account Groups](./configuring-product-visibility-using-account-groups.md)を参照してください。
