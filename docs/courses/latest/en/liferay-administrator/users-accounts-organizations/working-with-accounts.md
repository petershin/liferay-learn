...
Accounts can represent individual buyers in a B2C (i.e. business to consumer) and B2B (business to business) context. See [Accounts](https://learn.liferay.com/dxp/latest/en/users-and-permissions/accounts.html) to learn more. Delectable Bonsai sells maple syrup to individuals and to other businesses (i.e. various markets across the country). 

<!-- You didn't connect the above information to what's going to be done in this step. What will we do with accounts? What's the exact use case, and why are we doing it? -Rich -->
...
Create a new user for the account you just made. Note, you can create new users for accounts or associate exiting users with accounts.
...
For each account, a business can have multiple users and multiple roles. For example, the account buyer may handle creating orders, but the order manager may handle approving the orders. Assign Noah the order manager role. <!-- Why? -Rich -->
...
1. In the next window, click _Assign User_ (![Add icon](../../images/icon-add.png)). Select Noah Norman and click _Assign_.
   ...
   As Noah's employees add orders to their account, Noah is able to approve and manage orders. See [Commerce Roles Reference](https://learn.liferay.com/commerce/latest/en/users-and-accounts/roles-and-permissions/commerce-roles-reference.html) to learn more about these roles.

## Managing Accounts

Use account groups to help organize multiple accounts. <!-- Why? What's the use case? -Rich -->
...
1. Click the _Accounts_ tab. 

   - Click _Assign Account_ (![Add icon](../../images/icon-add.png)). 
   - In the new window, select _Marvelous Markets NYC_ and click _Assign_. 

   In this way, multiple Marvelous markets in the east coast region an be added to the group.

In addition to account roles, you can assign an account manager role to an organization. Therefore, in a situation where an entity (e.g. Marvelous Markets) has multiple accounts, create an organization for the accounts. Then assign an account manager to that organization. 
...
1. Click _Roles_ in the left navigation. 
