...
* **타임스탬프**: 이 예에서는 `Jun 29 10:07:46.585`입니다.
...
인스턴스 ID의 형식은 서비스가 구성된 배포 유형(배포 또는 상태 저장 세트)에 따라 다릅니다. 자세한 내용은 [배포 유형 이해](../build-and-deploy/understanding-deployment-types.md) 을 참조하십시오.
...
* **서비스 이름**: 이 예에서는 `liferay`입니다.

* 임의로 생성된 **버전 ID**: 이 예에서는 `7485669bdd`입니다. 이는 변경 가능성이 있는 배포된 서비스 버전에 해당합니다. 새 배포, 환경 변수 변경 및 수동으로 서비스를 다시 시작하는 경우에도 모두 새 버전 ID가 생성됩니다(구성에서 새 변경이 수행되었을 수 있기 때문).

* 임의로 생성된 **컨테이너 ID**: 이 예에서는 `7ktfl`입니다. 새 컨테이너가 생성될 때마다(예: 새 빌드를 배포하고 서비스를 다시 시작한 후) 서비스가 시작될 때마다 새 컨테이너가 생성되기 때문에 새 컨테이너 ID가 생성됩니다. 다시 시작하면 버전 ID가 변경되지 않는 경우에도(예: [활성 또는 준비 프로브 실패](../troubleshooting/self-healing.md)가 다시 시작을 트리거하는 경우) 새 컨테이너 ID가 생성됩니다.

### 상태 저장 세트 유형 로그의 인스턴스 ID

상태 저장 세트 유형 로그에는 더 작고 더 일관된 인스턴스 ID가 있습니다. 상태 저장 세트 유형 서비스의 로그 메시지 예를 참조하십시오.
...
1. 환경 메뉴에서 *로그* 을 클릭합니다.

모든 환경 서비스에서 애플리케이션, 상태 및 빌드 로그를 보거나 드롭다운 메뉴를 사용하여 결과를 필터링합니다.

로그를 다운로드하려면 *로그 다운로드* 버튼을 클릭합니다.

![그림 1: 로그 페이지에서 환경 로그 보기.](./reading-liferay-cloud-service-logs/images/02.png)

개별 서비스 로그는 각 서비스의 전용 페이지에 있는 *Logs* 탭에서도 사용할 수 있습니다.

![그림 2: 각 서비스의 전용 페이지에 있는 로그 탭에서 개별 서비스 로그에 액세스하고 다운로드합니다.](./reading-liferay-cloud-service-logs/images/03.png)
