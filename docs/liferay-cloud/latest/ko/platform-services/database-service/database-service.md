...
데이터베이스 서비스(MySQL)는 애플리케이션의 설정, 운영 및 확장을 단순화하는 분산 관계형 데이터베이스 서비스입니다. 애플리케이션 환경 내부의 개인 서비스이며 공용 인터넷이 아닌 다른 서비스와만 통신할 수 있습니다.
...
이러한 환경 변수를 설정하여 데이터베이스 서비스를 구성할 수 있습니다. `LCP_MASTER_USER_NAME`, `LCP_MASTER_USER_PASSWORD`및 `LCP_DBNAME`를 설정할 때 데이터베이스 서비스에 의존하는 다른 서비스(예: 백업 및 Liferay 서비스)에 대해 동일한 값을 사용해야 합니다. 첫 번째 배포 전에 이러한 변수를 설정해야 합니다. 이러한 변수에 대한 새 값으로 빌드가 생성되면 후속 배포가 실패합니다. 개발 환경에서 서비스를 삭제하고 `LCP.json` 파일을 이러한 변수에 대한 새 값으로 업데이트하는 것이 가능할 수 있지만 프로덕션 환경에서는 불가능합니다.
...
| `LCP_GCP_MW_UPDATE_TRACK` | `canary`, `stable` | `카나리아` 을 설정하여 다른 환경보다 최대 1주일 먼저 업데이트하십시오. |

### Google Cloud MySQL 플래그

MySQL 플래그를 환경 변수로 전달할 수 있습니다. 사용 가능한 플래그는 [Google Cloud 문서](https://cloud.google.com/sql/docs/mysql/flags)에 나열되어 있습니다. Liferay Cloud에서 작동하려면 각 플래그 앞에 `LCP_GCP_DATABASE_FLAG_` 이 추가되어야 합니다. 다음은 개발 환경에서 디버깅에 유용할 수 있지만 상당한 성능 비용이 발생하므로 프로덕션 환경에서 사용하면 안 되는 일반적인 플래그입니다.
...
| 이름                                     | 허용 가능한 값  | 기정값 |
|:-------------------------------------- |:--------- |:--- |
| `LCP_GCP_DATABASE_FLAG_GENERAL_LOG`    | `on, off` | `끔` |
| `LCP_GCP_DATABASE_FLAG_SLOW_QUERY_LOG` | `on, off` | `끔` |
