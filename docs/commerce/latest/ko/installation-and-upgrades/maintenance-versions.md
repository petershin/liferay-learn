...
다양한 유형의 패치에 대해 알아보려면 [패치 Liferay](https://learn.liferay.com/dxp/latest/en/installation-and-upgrades/maintaining-a-liferay-installation/patching-dxp-7-3-and-earlier.html) 및 [패치 유형 이해](https://learn.liferay.com/dxp/latest/en/installation-and-upgrades/maintaining-a-liferay-installation/patching-dxp-7-3-and-earlier/understanding-patch-types-for-dxp-7-3-and-earlier.html) 을 참조하십시오.

패치를 적용하는 방법을 알아보려면 [패치 설치](https://learn.liferay.com/dxp/latest/en/installation-and-upgrades/maintaining-a-liferay-installation/patching-dxp-7-3-and-earlier/installing-patches-for-dxp-7-3-and-earlier.html)을 참조하십시오.

Liferay Commerce 3.0(오픈 소스) 사용자는 Liferay Portal 7.3의 새 릴리스를 통해 동일한 버그 및 보안 업데이트에 액세스할 수 있습니다.

## Liferay Commerce 2.1 이하 업데이트

주기적으로 Liferay는 오픈 소스 및 엔터프라이즈 구독자 모두를 위해 Liferay Commerce에 대한 업데이트(유지 관리 버전)를 릴리스합니다. 특히 유지 보수 버전은 버전 번호의 세 번째 숫자의 변경 사항을 나타냅니다. 각 유지 관리 버전에는 최신 버그 수정이 포함되어 있으며 향상된 수준의 보안 및 안정성을 제공합니다. 사용자 지정은 일반적으로 안전하지만 구현하기 전에 사용자 지정을 검토하는 것이 좋습니다.

새로운 유지 관리 버전으로 업데이트하는 것은 Liferay Commerce의 새로운 주 버전 또는 부 버전으로 업그레이드하는 것과 매우 유사합니다.

### 로드맵

1. [다운로드 및 배포](#download-and-deploy)
1. [부실 데이터 지우기 및 서버 다시 시작](#clear-stale-data-and-restart-the-server)
...
### 부실 데이터 지우기 및 서버 다시 시작

1. `${liferay.home}/osgi/state` 폴더를 삭제합니다. > OSGi 폴더에 대한 자세한 내용은 [수동으로 앱 설치](https://help.liferay.com/hc/en-us/articles/360017895412-Installing-Apps-Manually#using-your-file-system-to-install-apps)를 참조하십시오.
1. 애플리케이션 서버를 시작하십시오.
...
Liferay Commerce 인스턴스가 업데이트되었습니다. 다시 인덱싱할 필요가 없으며 모든 데이터가 변경되지 않은 상태로 유지됩니다.

## 추가 정보

Liferay Commerce는 Liferay 디지털 경험 플랫폼을 기반으로 합니다. 경우에 따라 핵심 Liferay DXP 플랫폼을 업데이트해야 하며 최신 주 버전, 부 버전 또는 유지 관리 버전으로 업데이트해야 합니다. Liferay Commerce에 대한 수정 사항은 Liferay와 독립적입니다.
...
* [유효 종료](https://www.liferay.com/subscription-services/end-of-life/commerce)
