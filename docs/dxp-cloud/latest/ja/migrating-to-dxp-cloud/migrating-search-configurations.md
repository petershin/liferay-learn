...
Webサーバーの構成がLiferay Cloudに展開されたので、次の移行段階として、検索構成の移行を行います。 検索エンジンをElasticsearchに移行し、Elasticsearchの設定ファイルやプラグインをすべてLiferay Cloud環境にデプロイする作業が必要です。
...
先に [クローンしたプロジェクトリポジトリから](./matching-dxp-versions.md#clone-the-dxp-cloud-repository) `search/configs/{ENV}` フォルダ（Liferay Cloud環境に対応）に移動して、Elasticsearch設定ファイルやシェルスクリプトをこのパターンで適切な環境のフォルダに配置します。
...
リストにない検索プラグインをお持ちの場合は、Liferay Cloud プロジェクトのリポジトリに追加してください。
...
次に、これらの変更を含むビルドを作成し、デプロイして、Liferay Cloud 環境に適用します。
