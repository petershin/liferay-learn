...
테마 스프라이트 맵 클라이언트 확장 프로그램을 사용하면 페이지의 아이콘에 사용되는 기본 [클레이](https://clayui.com/docs/components/icon.html) 스프라이트 맵을 재정의할 수 있습니다. [샘플 워크스페이스](https://github.com/liferay/liferay-portal/tree/master/workspaces/liferay-sample-workspace) 에서 시작하여 클라이언트 확장 프로그램을 빌드하고 배포하세요.

```{note}
SVG 스프라이트 맵은 여러 개의 아이콘을 그룹으로 묶은 이미지로, 각각 고유 ID가 있습니다. SVG를 로드하고 ID로 개별 아이콘을 참조할 수 있습니다. 이렇게 하면 스프라이트 맵을 한 번 다운로드하고 캐시하여 모든 사이트 아이콘을 표시할 수 있으므로 성능이 약간 향상됩니다.
```
...
이제 첫 번째 테마 스프라이트 맵 클라이언트 확장 프로그램을 배포할 수 있는 도구가 생겼습니다.

## 테마 스프라이트 맵 프로젝트 살펴보기

테마 스프라이트 맵 클라이언트 확장 프로그램은 샘플 워크스페이스의 `client-extensions/liferay-sample-theme-spritemap-1/` 폴더에 있습니다. `client-extension.yaml` 파일에 정의되어 있습니다:
...
클라이언트 확장 프로그램의 ID는 `liferay-sample-theme-spritemap-1` 이며 추가할 유형 및 스프라이트 맵 파일을 포함하여 테마 스프라이트 맵 클라이언트 확장 프로그램의 주요 구성이 포함되어 있습니다. 사용 가능한 속성에 대한 자세한 내용은 [테마 스프라이트 맵 YAML 구성 참조](../theme-sprite-map-yaml-configuration-reference.md) 를 참조하세요.
...
이는 `assets/` 폴더에 있는 모든 것이 빌드된 클라이언트 확장자 `.zip` 파일에 정적 리소스로 포함되도록 지정합니다. 테마 스프라이트 맵 클라이언트 확장에 있는 SVG 스프라이트 맵은 Liferay에서 정적 리소스로 사용됩니다.

```{important}
샘플 프로젝트의 `assets/spritemap.svg`는 Liferay에서 사용되는 [클레이 아이콘](https://clayui.com/docs/components/icon.html) 스프라이트 맵의 복사본입니다. Liferay에 사용된 모든 아이콘은 예제 스프라이트 맵에 존재하므로 UI가 변경된 것을 느끼지 못할 것입니다. 

스프라이트 맵 클라이언트 확장 프로그램은 선택한 페이지의 전체 스프라이트 맵을 대체하므로 아이콘이 누락되지 않도록 필요한 모든 아이콘에 해당 ID가 있는지 확인합니다. 
```
...
아래는 상단 탐색 모음에 사용되는 아이콘의 ID입니다.

| 숫자 | 아이콘                                                                  | ID          |
|:-- |:-------------------------------------------------------------------- |:----------- |
| 1  | ![편집 아이콘](../../../../images/icon-edit-pencil.png)                   | 연필          |
| 2  | ![톱니바퀴 아이콘](../../../../images/icon-cog.png)                         | cog         |
| 3  | ![시뮬레이션 아이콘](../../../../images/icon-simulation.png)                 | 시뮬레이션 메뉴 닫힘 |
| 4  | ![애널리틱스 아이콘](../../../../images/icon-analytics.png)                  | 해석학         |
| 5  | ![AB 테스트 아이콘](../../../../images/icon-ab-testing.png)                | 시험          |
| 6  | ![정보 아이콘](./using-a-theme-spritemap-client-extension/images/02.png)  | 정보 서클       |
...
2. 이 아이콘을 사용자 지정 아이콘으로 바꾸고 동일한 ID를 부여합니다(`연필`). 물고기 아이콘은 아래 `기호` 를 사용합니다.
...
```{note}
테마 스프라이트 맵 클라이언트 확장은 현재 [릴리스 기능 플래그](../../../../시스템-관리/설정-라이프플래이/기능-플래그.md#릴리스-기능-플래그) 뒤에 있습니다. 사용을 시작하기 전에 기능 플래그를 활성화해야 합니다.
```
...
테마 스프라이트 맵 클라이언트 확장 프로그램을 성공적으로 사용했습니다. 다음으로 다른 클라이언트 확장 유형을 배포해 보세요.
