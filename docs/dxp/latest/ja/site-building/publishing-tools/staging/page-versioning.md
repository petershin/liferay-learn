...
ページのバージョニングを有効にすると、サイトのページの複数のバージョンで並行して作業することができます。 異なるバージョンは **variation** と呼ばれ、ステージングバーからウィジェットページとコンテンツページに対して作成、管理することができます。 ページのバージョニングは履歴を管理し、必要に応じてステージングされたページに加えられた変更を以前のバージョンに戻すために使用します。
...
```{important}
Page Versioningを使用し，その後無効にした場合，Page Versionの履歴は削除され，最新のPublication Versionのみが保持されます． これは、Local LiveとRemote Live Stagingのどちらにも当てはまります。
```
