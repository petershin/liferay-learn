...
Liferay Cloud では、アプリケーションを管理するために組み合わせて使用する 2 つのプローブを提供しています。
...
| プロパティ名                | Description                                                                                                                   |
|:--------------------- |:----------------------------------------------------------------------------------------------------------------------------- |
| `initialDelaySeconds` | コンテナが起動してからプローブが開始されるまでの秒数。                                                                                                   |
...
| `failureThreshold`    | Liferay Cloud が、失敗したプローブをあきらめるまで繰り返す回数。 活性プローブの場合、あきらめるとサービスが再起動する。 準備プローブの場合、あきらめると、プローブは失敗としてマークされる。 デフォルトは `3`で、 最小値は `1`。 |
