...
| `LCP_CI_ARTIFACT_NUM_TO_KEEP`         | `1`       | _아티팩트_ 및 _스태시_가 보존되는 최근 빌드 수를 설정합니다.                                                                                                                                                                                                                                                                 |
...
| `LCP_CI_DEPLOY_BRANCH`                | `develop` | [자동 배포](../build-and-deploy/automatically-deploying-ci-service-builds.md) 에 사용할 분기를 지정합니다. 유효한 분기 이름으로 설정되지 않으면 자동 배포가 비활성화됩니다.                                                                                                                                                            |
| `LCP_CI_DEPLOY_TARGET`                |           | 자동 배포 [이 배포할 환경](../build-and-deploy/automatically-deploying-ci-service-builds.md) 을 설정합니다. 'LCP_CI_DEPLOY_BRANCH'가 설정된 경우에만 사용됩니다.                                                                                        |
...
| `LCP_CI_PRESERVE_STASHES_BUILD_COUNT` | `20`      | _스태시_가 보존되는 최근 빌드 수를 설정합니다. 'LCP_CI_ARTIFACT_NUM_TO_KEEP' 변수에서 허용하는 것보다 더 많은 빌드에 대해서는 스태시를 보존할 수 없습니다.                                                                                      |
