...
[클러스터링](./using-the-liferay-dxp-service/setting-up-clustering-in-liferay-cloud.md) 및 [로드 밸런싱](.//configuring-the-cloud-network/load-balancer.md) 은 미션 크리티컬 사이트가 온라인 상태를 유지할 수 있도록 가용성이 높고 다운타임이 없는 배포를 제공합니다. Liferay Cloud는 다중 지역 아키텍처를 통해 데이터를 제공하여 고가용성을 보장합니다.
