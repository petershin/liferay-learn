...
| 사용 사례                                                               | `구성` (**값**) |
|:------------------------------------------------------------------- |:--------------------------------------------------------------------------------------------------------------------------- |
| **사용자 제목 필터:** 최종 사용자는 필터 키를 직접 조작하여 제목을 사용하여 원하는 결과로 결과를 구체화합니다.   | `필터 필드` (**title\** en\ **US**) <br /> `필터 값` (**팟캐스트**) <br /> `필터 쿼리 유형` (**일치**) <br /> `발생 유형` (**must** not)_ |
| **하드 코딩된 필터링:** 관리 사용자가 최종 사용자에게 보이지 않고 변경할 수 없는 필터로 검색 페이지를 구성합니다. | ****위 구성에 추가_** <br /> `보이지 않음` (**True-체크됨**) <br /> `변경 불가능` (**True-체크됨**) |
