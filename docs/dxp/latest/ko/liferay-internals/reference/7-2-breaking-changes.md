...
이는 com.liferay.portal.kernel.service</code>패키지 `에서 <code>InvokableService` 및 `InvokableLocalService` 을 사용한 모든 사용자에게 영향을 미칩니다.
...
- 0 React 문서에 설명된 대로 ES6 `클래스` API를 사용하기 위해 `</a>` API에서 맞춤 버튼을 포팅합니다. 예를 들어, [이전 `createClass`기반 구현](https://github.com/liferay/alloy-editor/blob/2826ab9ceabe17c6ba0d38985baf8a787c23db43/src/ui/react/src/components/buttons/button-ol.jsx)에서 [ES6 클래스 기반 버튼](https://github.com/liferay/alloy-editor/blob/b082c312179ae6626cb2ddcc04ad3ebc5b355e1b/src/components/buttons/button-ol.jsx) 으로 이동하면서 변경된 사항을 참조하십시오.</p></li> 
  
  - 호환 어댑터를 제공하십시오. [create-react-class 패키지](https://www.npmjs.com/package/create-react-class) (여기서 설명 [](https://reactjs.org/docs/react-without-es6.html))을 페이지에 삽입하여 `createClass` API를 복원할 수 있습니다.</ul> 
...
이전에 Liferay는 `AssetEntryLocalService` 및 `AssetCategoryLocalService`에서 `AssetEntry` 과 `AssetCategory` 사이의 관계에 매핑 테이블과 해당 인터페이스를 사용했습니다. 이 매핑 테이블과 해당 인터페이스는 AssetEntryAssetCategoryRel</code> 테이블 `과 AssetEntryAssetCategoryRelLocalService`서비스 `로 대체되었습니다.</p>

<h4 spaces-before="0">누가 영향을 받습니까?</h4>

<p spaces-before="0">이것은 <code>AssetEntryLocalService` 및 `AssetCategoryLocalService`를 통해 `AssetEntries_AssetCategories` 관계에 대한 이전 인터페이스 호출에 의존하는 모든 콘텐츠 또는 코드에 영향을 미칩니다.
...
`getParameter`대신 기존 메소드 `getParameterOptional` 을 사용하십시오. `queryString` 의 사용법을 `com.liferay.dynamic.data.mapping.data.provider.DDMDataProviderRequest.Builder` 의 매개변수</code> 으로 메소드 `로 대체하여 필요한 모든 매개변수를 추가하십시오.</p>

<h4 spaces-before="0">이렇게 변경된 이유는 무엇입니까?</h4>

<p spaces-before="0">이 변경 사항은 <a href="https://issues.liferay.com/browse/LPS-81563">LPS-81563</a>의 데이터 공급자 API 리팩토링의 일부입니다.</p>

<hr />

<h3 spaces-before="0">DDMDataProviderRequest에서 대체된 메소드</h3>

<ul>
<li><strong x-id="1">날짜:</strong> 2020-7-14</li>
<li><strong x-id="1">JIRA 티켓:</strong> <a href="https://issues.liferay.com/browse/LPS-81563">LPS-81563</a></li>
</ul>

<h4 spaces-before="0">무엇이 바뀌었나요?</h4>

<p spaces-before="0"><code>com.liferay.dynamic.data.mapping.data.provider.DDMDataProviderRequest`의 `getDDMDataProviderInstanceId` 메서드가 `getDDMDataProviderId`로 대체되었습니다.
...
- `오류` 메소드를 호출하는 대신 `com.liferay.dynamic.data.mapping.data.provider.DDMDataProviderResponse.Builder`의 `withStatus` 메소드를 사용하십시오.
- `메서드를` Builder `의`withStatus `및`</code> `로 바꿉니다.</li>
<li><code>getDataMap` 호출을 `Builder`의 `withOutput` 메서드를 사용하여 출력 추가로 교체하고 메서드 `getOutputOptional`을 통해 가져옵니다.

메서드 `withOutput` 은 필요한 만큼 여러 번 호출할 수 있습니다.
...
이 변경 전에는 `.cfg` 파일이`.cfg` 또는 `.config` 형식을 사용할 수 있었습니다. 이제 `.cfg` 구성 파일은 `</a>` 여기에 정의된 대로 형식을 사용해야 합니다.</p> 
