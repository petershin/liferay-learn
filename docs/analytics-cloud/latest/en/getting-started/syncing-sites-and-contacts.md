...
If you're also new to Liferay DXP, there isn't much on your Liferay instance for Analytics Cloud to start tracking. Use Liferay Commerce's [Minium Accelerator Site](https://learn.liferay.com/commerce/latest/en/starting-a-store/using-the-minium-accelerator-to-jump-start-your-b2b-store.html) to get some sample content that you can use to explore Analytics Cloud.
...
1. Click the Add icon (![Add icon](../images/icon-add.png)) and select _Minium_. Give the sample site a name (e.g. Minium Site). Click _Add_. The Minium sample site has now been added.

    ![Select the Minium Site and add it to your Liferay instance.](./syncing-sites-and-contacts/images/01.png)

Let's add a few sample users to our Liferay instance for Analytics Cloud to track. 
...
2. In the Script window, paste the following:
    ...
    Click _Execute_. Three new users are added to your instance.

3. Open the Global Menu (![Global Menu](../images/icon-applications-menu.png)) and navigate to _Control Panel_ &rarr; _Instance Settings_. Under the _Platform_ section, click _User Authentication_. Uncheck `Require strangers to verify their email address?
` and click _Save_. Note, use this setting to utilize these samples users without the need for email verification. 
...
Next, we'll sync the existing contacts you just created. 
...
2. Under the Contact Sync Options, click _Sync Contacts_. Use the switch to enable _Sync All_ if you wish to sync all your contacts. Alternatively, click on _Sync by User Groups_ or _Sync by Organizations_ if you prefer to sync a subset of your contacts. Click _Save and Next_. For this example, select _Sync All_.

3. Liferay DXP stores contact data in two separate tables (Contact and User). Switch between the Contact tab and User tab to select the fields to sync. Analytics Cloud requires some fields to be synced, so those are grayed out. For this example, just keep the default selections. Click _Save_.
...
Great, now you're all set up! As individuals visit and interact with your site, Analytics Cloud will begin tracking what's happening. Next let's dive into [Understanding Your Site's Data](./understanding-your-sites-data.md) and see the different interactions that get tracked.

The contents of the article are also covered in this video tutorial [Scoping Sites and Individuals in Analytics Cloud](https://youtu.be/_aHOdw-8Z2M).