...
이전 버전에서 Widget Pages는 사용자 지정 레이아웃 및 사용자 지정 가능한 열과 같은 독점 기능을 제공했습니다. 이러한 독점 기능 중 다수는 7.3의 [콘텐츠 페이지](../../using-content-pages.md) 에서 사용할 수 있으므로 대부분의 경우 콘텐츠 페이지를 만들고 싶을 것입니다.

이전 버전에서 Liferay Portal 7.3으로 업그레이드하고 기존 위젯 페이지를 마이그레이션하는 경우 이를 콘텐츠 페이지로 변환할 수 있습니다. 위젯 페이지를 콘텐츠 페이지로 바로 변환하거나 변환을 미리 보고 변경한 다음 페이지를 변환할 수 있습니다.

변환하려는 페이지가 여러 개인 경우 사이트 관리를 통해 일괄 변환하거나 사용 가능한 API 및 내장 스크립트 편집기를 사용하여 모든 위젯 페이지를 한 번에 변환할 수 있습니다. 모든 접근 방식은 여기에 설명되어 있습니다.
...
1. *Publish* 을 클릭하여 미리보기 초안을 게시하거나 *Discard Conversion Draft* 을 클릭하여 위젯 페이지를 원래 상태로 재설정합니다. 경고가 있는 경우 아래 설명된 대로 최선의 변환이 완료됩니다.
...
```{note}
사용자 정의 레이아웃 템플릿을 변환할 수 있음을 이미 확인한 경우 레이아웃을 사용하는 위젯 페이지를 변환할 때마다 계속 표시되지 않도록 레이아웃 템플릿에 대한 레이아웃 템플릿 변환 경고를 비활성화할 수 있습니다. Liferay DXP 메뉴를 열고 *제어판* -> *구성* -> *시스템 설정*으로 이동합니다. 콘텐츠 및 데이터 아래에서 *페이지*를 선택하고 시스템 범위 아래의 "검증된 레이아웃 템플릿 ID" 목록에 레이아웃 템플릿 ID를 추가합니다.
```
...
1. 위젯 페이지 또는 여러 위젯 페이지에 대한 확인란을 선택하고 관리 도구 모음에서 작업 메뉴를 열고 *콘텐츠 페이지로 변환* 옵션을 선택합니다.

   ![컨텍스트 메뉴를 통해 여러 위젯 페이지를 변환할 수 있습니다.](./converting-widget-pages-to-content-pages/images/01.png)

1. 변환을 완료하기 위해 표시되는 프롬프트에서 *확인* 을 클릭합니다.

## API를 사용하여 모든 위젯 페이지를 콘텐츠 페이지로 대량 변환

내장된 스크립트 편집기를 사용하여 사이트의 모든 위젯 페이지를 콘텐츠 페이지로 일괄 변환할 수 있습니다. 다음과 같이하세요:

1. Liferay DXP 메뉴를 열고 *제어판* &rarr; *구성* &rarr; *서버 관리* &rarr; *스크립트*으로 이동합니다.

1. 스크립트 창에 이 스크립트를 입력하고 그룹 ID를 자신의 ID로 바꾸십시오. Groovy 스크립트는 [`BulkLayoutConverter` 인터페이스](https://docs.liferay.com/dxp/portal/7.3-latest/apps/layout-3.0.0/javadocs/com/liferay/layout/util/BulkLayoutConverter.html) 을 사용하여 지정된 그룹 ID가 있는 모든 위젯 페이지를 콘텐츠 페이지로 변환합니다.
