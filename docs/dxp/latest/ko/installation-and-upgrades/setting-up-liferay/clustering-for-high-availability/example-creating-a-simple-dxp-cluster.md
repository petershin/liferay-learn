...
DXP 클러스터에는 모든 DXP 클러스터 노드에 액세스할 수 있는 데이터 소스가 필요합니다. 데이터 소스는 JNDI 데이터 소스이거나 데이터베이스 서버 또는 데이터베이스 서버 클러스터에 대한 직접 연결일 수 있습니다. DXP 버전이 지원하는 데이터베이스 서버에 대해서는 [호환성 매트릭스](https://help.liferay.com/hc/en-us/articles/360049238151) 참조하세요.
