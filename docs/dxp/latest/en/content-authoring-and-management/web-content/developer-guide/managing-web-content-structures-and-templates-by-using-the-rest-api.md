...
The REST service can also be called with a Java class.
...
See the code below and its comments for more information:
...
public class ContentStructures_GET_FromSite {
   ...
   public static void main(String[] args) throws Exception {
      ...
      // Provides authentication credentials to the ContentStructureResource
      // and constructs the contentStructureResource object (.build()).
      ContentStructureResource contentStructureResource =
         builder.authentication(
               "test@liferay.com", "learn"
         ).build();

      // Retrieves the command-line arguments specified when running the Java application to obtain the site ID and parses it to a long type.
      long siteId = Long.parseLong(System.getProperty("siteId"));

      // The method call returns a Page<ContentStructure> object
      // which represents a page of content structures retrieved from the site.
      Page<ContentStructure> page =

         // Calls the getSiteContentStructuresPage() method with these parameters:
      ...
      // Prints the page of content structures to the console.
...
1. Run the `ContentStructures_GET_Permissions.java` class. replace the `contentStructureId` value with your web content structure's ID:
   ...
   The `ContentStructures_GET_Permissions.java` works similarly to the `ContentStructures_GET_FromSite.java` file. The only differences are that the `getContentStructurePermissionsPage()` method call returns a `Page<Permission>` object which represents a page of permissions result instead of a Page<ContentStructure> and the method receives a `Long contentStructureId` (instead of the `siteId`) and a `String roleNames` as parameters. `roleNames` is set as `null` to return all permissions available.
...
1. Run the `ContentStructures_PUT_Permissions.java` class. replace the `contentStructureId` value with your web content structure's ID, the `actions` value for an action or a list of actions (separate them with commas), and the `role` value with the desired role for updating permissions:
...
public class ContentStructures_PUT_Permissions {
   ...
   public static void main(String[] args) throws Exception {
      ...
      Long contentStructureId = Long.valueOf(System.getProperty("contentStructureId"));

      // Retrieves the command-line arguments specified when running the Java application to obtain the web content structure ID, actions, and role.
      String actions = System.getProperty("actions");
      String role = System.getProperty("role");

      // Deals with multiple actions. Splits and places the actions in an array of action IDs.
      String[] actionIds = actions.split("\\s*,\\s*");

      // Instantiates a new Permission object and populates it with the action IDs array and the role.
      Permission permission = new Permission();
      permission.setActionIds(actionIds);
      permission.setRoleName(role);

      // An array of Permission objects is created with the permission object created earlier.
      Permission[] permissions = new Permission[]{permission};

      Page<Permission> page =

               // Calls the putContentStructurePermissionsPage() method with these parameters:
