...
!!! important
    New filtering and ordering capabilities introduced with Liferay 7.4 2024.Q1+ are breaking changes.
...
When viewing all assets, assets in a specific folder, or assets resulting from a web content search, you can use four different filter and sort options. In addition, as of Liferay 7.4 2024.Q1+, you can filter web content articles through highlighted web content structures:
...
You can combine (single or multiple) filters and order your results for an enhanced experience. For example, if you select the "Product Advertisement Blog Post" [highlighted structure](#filter-by-highlighted-web-content-structures), the [Mine](#filter-by-navigation) and the [Draft](#filter-by-status) filters, and the [order by Title](#order-by) in Ascending order filter, you obtain a list of your articles that use the "Product Advertisement Blog Post" structure, are in Draft status, and are ordered by title in ascending alphabetical order.

!!! note
    All filtering options apply exclusively to the current folder. Web content articles located elsewhere are not included in the filtered results.
...
| Recent                                           | Assets that have been modified recently. The [order by](#order-by) option is unavailable with this filter. |
| Structures                                       | Filters web content articles based on web content structure.                                               |
| {bdg-secondary}`Liferay 7.4 2023.Q4+` Categories | Filters web content based on its category.                                                                 |
| {bdg-secondary}`Liferay 7.4 2023.Q4+` Tags       | Filters web content based on its assigned tags.                                                            |
...
| Approved      | All published assets that have at least one version in the Approved status.                                                                                                                    |
| Expired       | All assets with at least one expired version. This filter includes web content where a published version expired before a new version was scheduled or published.                              |
...
| Filter Option | Description                          |
|:--------------|:-------------------------------------|
| {bdg-secondary}`Liferay 7.4 2024.Q1+` Web Content   | Web content articles' last versions. |
...
Once you use the search bar and search for specific keywords, use these options to enhance your search results further.
