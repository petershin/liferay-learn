...
조합 차트는 다른 차트와 약간의 차이가 있습니다. 조합 차트에서는 각 데이터 세트의 표시 유형을 정의해야 합니다.`AREA`, `AREA_SPLINE`, `AREA_STEP`, `BAR`, `BUBBLE`, `DONUT`, `GAUGE`, `LINE`, `PIE`, `SCATTER`, `SPLINE` 또는 `STEP`.
조합 차트의 각 데이터 세트는 [`TypedMultiValueColumn\` 객체](https://resources.learn.liferay.com/reference/latest/en/dxp/javadocs/modules/apps/frontend-taglib/com.liferay.frontend.taglib.chart/com/liferay/frontend/taglib/chart/model/TypedMultiValueColumn.html) 의 인스턴스입니다. 각 객체는 ID, 표현 유형 및 데이터 값을 받습니다. 조합 차트를 사용하도록 포틀릿을 구성하려면 다음 단계를 따르십시오.
