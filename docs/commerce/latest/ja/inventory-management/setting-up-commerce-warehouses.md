...
```{note}
コマース倉庫を設定するとき、ユーザーは最初に倉庫の該当する国および地域の場所（州または県）を有効にする必要がある場合があります。 See [Adding Regions](../store-management/adding-regions.md) for more information.
```
...
新しい倉庫がアクティブになり、_［出荷］_タブでオプションとして利用可能になりました。
