---
toc:
- ./manage-and-optimize/auto-scaling.md
- ./manage-and-optimize/application-metrics.md
- ./manage-and-optimize/real-time-alerts.md
- ./manage-and-optimize/quotas.md
- ./manage-and-optimize/environment-teams-and-roles.md
- ./manage-and-optimize/team-activities.md
---
# 관리 및 최적화

```{toctree}
:maxdepth: 1

manage-and-optimize/auto-scaling.md
manage-and-optimize/application-metrics.md
manage-and-optimize/real-time-alerts.md
manage-and-optimize/quotas.md
manage-and-optimize/environment-teams-and-roles.md
manage-and-optimize/team-activities.md
```

## 성능 및 모니터링

* [자동 크기 조정](./manage-and-optimize/auto-scaling.md)
* [애플리케이션 지표](./manage-and-optimize/application-metrics.md)

## 사용량 관리

* [실시간 알림](./manage-and-optimize/real-time-alerts.md)
* [할당량](./manage-and-optimize/quotas.md)

## 계정 및 팀 관리

* [환경 팀 및 역할](./manage-and-optimize/environment-teams-and-roles.md)
* [팀 활동](./manage-and-optimize/team-activities.md)
