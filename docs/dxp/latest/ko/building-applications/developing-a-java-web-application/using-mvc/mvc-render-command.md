...
MVC 렌더 명령은 렌더링할 페이지를 처리하는 클래스입니다. `MVCPortlet` 렌더링 URL 및 요청에 의해 호출됩니다. 렌더링 로직이 단순하다면 포틀릿 클래스에서 [것을 구현할 수 있습니다](./rendering-views-with-mvc-portlet.md). 렌더링 논리가 복잡하거나 렌더링 경로를 깔끔하게 분리하려는 경우 MVC 렌더링 명령을 사용하세요.

## MVC 렌더링 명령 호출

여기서는 MVC 렌더링 명령을 사용하여 보기를 렌더링하는 예제 포틀릿을 배포합니다.
...
`A4P1Portlet` 최소 [`MVCPortlet`](https://github.com/liferay/liferay-portal/blob/[$LIFERAY_LEARN_PORTAL_GIT_TAG$]/portal-kernel/src/com/liferay/portal/kernel/portlet/bridges/mvc/MVCPortlet.java)입니다.
...
[`@Component`](https://osgi.org/javadoc/r6/residential/org/osgi/service/component/annotations/Component.html) `javax.portlet.name` 특성은 포틀릿의 이름을 설정합니다.
...
MVC 렌더 명령 클래스는 [`MVCRenderCommand`](https://github.com/liferay/liferay-portal/blob/[$LIFERAY_LEARN_PORTAL_GIT_TAG$]/portal-kernel/src/com/liferay/portal/kernel/portlet/bridges/mvc/MVCRenderCommand.java) 직접 구현할 수 있습니다. `A4P1AbleMVCRenderCommand` `MVCRenderCommand`직접 구현합니다. 다음은 `A4P1AbleMVCRenderCommand`입니다.
...
`A4P1BakerMVCRenderCommand` MVC 명령 이름이 `/a4p1/baker` 이고 렌더 메소드가 뷰 경로 `/a4p1/baker.jsp`반환한다는 점을 제외하면 `A4P1AbleMVCRenderCommand` 과 유사합니다.

이러한 예제 MVC 렌더 명령은 데모 목적으로 간단한 기능을 제공합니다. 뷰를 렌더링하는 데 필요한 로직을 사용하여 MVC Render Command의 `render` 메서드를 구현합니다.

예제 URL은 MVC 렌더 명령을 트리거합니다.

## 포틀릿 렌더링 URL 검사

`` 및 `Baker.jsp` 파일은 포틀릿 렌더링 URL을 사용하여 서로 간접적으로 링크됩니다. 여기 ``있습니다:
...
`portlet:renderURL` 태그는 포틀릿 taglib에서 사용 가능하며 접두어 `portlet`이 지정됩니다. 이 렌더링 URL은 `mvcRenderCommandName` 포틀렛 매개변수 값 `/a4p1/baker`--- `A4P1AbleMVCRenderCommand`의 MVC 명령 이름을 선언합니다. 변수 `BakerURL` 은 이 렌더링 URL을 참조합니다.

하이퍼링크 `<a href="<%= bakerURL %>">Go to Baker</a>` 는 렌더링 URL을 작업에 바인딩합니다. 사용자가 하이퍼링크를 클릭하면 포틀릿은 `mvc.command.name` 구성 요소 속성 값 `/a4p1/baker` 가 `mvcRenderCommandName` 매개변수 값과 일치하기 때문에 [`RenderRequest`](https://docs.liferay.com/portlet-api/2.0/javadocs/javax/portlet/RenderRequest.html) ~ `A4P1BakerMVCRenderCommand` 보냅니다.

`Baker.jsp` 포틀릿 렌더링 URL `mvcRenderCommandName` 매개변수 값이 `/a4p1/able`이라는 점을 제외하면 `` 과 유사합니다. 각 JSP의 `portlet:renderURL` 태그는 MVC Render Command의 `mvc.command.name` 속성 값을 태그의 `mvcRenderCommandName` 포틀릿 매개변수에 할당하여 MVC Render Command에 매핑됩니다.

| `` 포틀릿 렌더링 URL 매개변수                                                       | `A4P1BakerMVCRenderCommand` 구성 요소 속성 |
|:------------------------------------------------------------------------- |:------------------------------------ |
...
이제 MVC Render Command 클래스에서 렌더링 논리를 구현하는 방법을 알았습니다. 다음으로 [MVC Resource Command](./mvc-resource-command.md) 클래스를 사용하여 파일과 같은 리소스에 대한 작업을 수행할 수 있습니다.
