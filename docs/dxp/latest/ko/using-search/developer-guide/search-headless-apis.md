...
| `name`               | `카테고리`, `사용자 지정`, `날짜 범위`, `폴더`, `중첩된`, `사이트`, `태그`, `유형`, 또는 `사용자`. 각 유형에 대한 자세한 내용은 [검색 패싯](https://learn.liferay.com/en/w/dxp/using-search/search-pages-and-widgets/search-facets) 문서를 참조하세요.                                                                                |
