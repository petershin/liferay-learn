...
{bdg-secondary}`Liferay 7.4 U58以降／GA58以降で利用可能`
...
これにより、アカウント関連フィールドが必須となり、ユーザーはエントリーを作成する際にアカウントを選択する必要があります。 利用可能なアカウントオプションは、各ユーザーの権限によって決定されます。 アカウント制限が有効な場合のロールパーミッションの構成については、 [アカウント制限とユーザーロール](#account-restriction-and-user-roles) を参照してください。 エントリー作成後、選択されたアカウントはエントリーのオーナーとなり、変更することはできません。
...
* 組織ロールがオブジェクトエントリーを作成するには、これらの権限を持つ必要があります。 `Users and Organizations &rarr; 組織。アカウント管理` と `ユーザーと組織 &rarr; 組織。サブ組織のアカウントを管理する`. これらの権限がない場合、組織ロールを持つユーザーは、エントリ作成時にアカウントを選択することができません。

* 組織およびアカウントロールユーザーがカスタムオブジェクトにアクセスするには、 `コントロールパネルでのアクセス` (company-scoped)または `サイトでのアクセス` (site-scoped) 権限を付与する別の通常ロールを持つ必要があります。 詳しくは、 [オブジェクトアプリケーションの権限](../../understanding-object-integrations/permissions-framework-integration.md#application-permissions) を参照してください。
...
ユーザーが、アカウントロールとオブジェクトの権限が異なる複数のアカウントに所属している場合、これらの権限は、ユーザーが所属する各アカウントに適用されます。
```
...
これを実現するには、アカウント制限を有効にしたClaimsオブジェクトを作成します。 次に、CSMをグループ化するための組織を作成し、各組織を適切なビジネスアカウントと関連付けます。 最後に、以下の [アカウント](#account-role) 、 [組織](#organization-role) 、 [正規](#regular-role) の役割を作成し、適切なユーザーに割り当てます。 保険請求オブジェクトは会社スコープなので、アカウントと組織のユーザーは、`Portal: View Control Panel Menu`および`Claims: Access in Control Panel`権限を付与する別の標準ロールも持っている必要があります。 利便性のため、これらの権限をデフォルトのユーザーロールに割り当てることができます。
...
```{important}
アカウントロールには、`Claims &rarr; Claim: View` リソース権限を割り当てないでください。 この許可は、標準的な役割行動を妨げる可能性があります。
```
