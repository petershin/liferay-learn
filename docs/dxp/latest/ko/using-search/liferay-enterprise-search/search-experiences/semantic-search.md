...
_Semantic_ 검색은 검색된 구 뒤에 있는 의도를 평가합니다. 한편, _키워드_ 검색은 검색된 구문을 인덱스 텍스트 필드에 일치시키고 의미에 따라 일치를 생성할 수 없습니다.
...
```{important}
효과적인 시맨틱 검색 솔루션에는 데이터 도메인에 대해 훈련된 모델이 필요합니다. 가장 간단한 접근 방식은 사전 학습된 적합한 모델을 찾은 다음 데이터에 맞게 미세 조정하는 것입니다. 여기에 설명된 예는 생산 준비가 된 시맨틱 검색 솔루션이 아닙니다.
```
...
예를 들어 사용자가 _검색하면 스케이트는 어떻게 움직일까요?_수 있듯이 해양 생물학에 대해 훈련된 모델은 레크리에이션에 대해 훈련된 모델과 다른 결과를 제공합니다.
...
텍스트 임베딩을 색인화하려면 Index Actions 화면을 사용하고 Semantic Search System 또는 Instance Settings의 Asset Entry Class Names 설정에서 활성화하려는 모델 유형에 대해서만 _Reindex_ 버튼을 클릭합니다.
...
**언어:** 변환할 언어와 현지화를 선택합니다. 기본적으로 나열된 모든 언어가 선택됩니다. _Ctrl + 클릭_을 사용하여 목록에서 여러 언어를 선택합니다.
...
* [LIFERAY] 추가 시맨틱 검색 처리가 발생합니다.
  ...
  * [TEXT EMBEDDING PROVIDER] 먼저 매개변수에 따라 스니펫을 토큰화하는 구성된 모델에 따라 스니펫이 처리됩니다. 자주 사용되는 BERT 모델의 경우 모델이 처리하는 최대 토큰 수는 512개입니다. 이는 시스템/인스턴스 설정의 시맨틱 검색 &rarr; 최대 문자 수 설정에서 설정한 문자 수의 영향을 받습니다.
  * [텍스트 임베딩 공급자] 텍스트 임베딩이라고 하는 소스의 벡터 표현은 변환기에서 사용하는 모델을 기반으로 생성됩니다.
  * [LIFERAY] 텍스트 임베딩 프로세스의 결과는 각 문서[^1]에 대한 [dense_vector](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/dense-vector.html) 필드로 [Liferay Company Index](../../search-administration-and-tuning/elasticsearch-indexes-reference.md) 에 저장됩니다.
  * 시스템/인스턴스 설정의 구성에 따라 텍스트 스니펫이 텍스트 임베딩 공급자에게 전송되고 텍스트 임베딩이 발생하며 변환기에서 사용하는 모델을 기반으로 벡터 표현이 생성됩니다. 텍스트 임베딩 프로세스의 결과는 각 문서[^1]에 대한 [dense_vector](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/dense-vector.html) 필드로 [Liferay Company Index](../../search-administration-and-tuning/elasticsearch-indexes-reference.md) 에 저장됩니다.
