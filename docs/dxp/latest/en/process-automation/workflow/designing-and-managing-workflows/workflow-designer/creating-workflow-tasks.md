...
Task nodes can also use [task timers](./using-task-timers.md) to trigger an action after a specified time period passes. [Timers](../../developer-guide/workflow-task-node-reference.md#task-timers) are useful for ensuring a task does not go unattended for a long time. Available timer actions include sending an additional notification, reassigning the asset, or creating a timer action.
