...
デフォルトでは、必要な権限（最低でも _メッセージの追加_, _メッセージへの返信_, _ファイルの追加_, _表示_）をもって認証されたユーザのみがスレッドを作成できるようになっています。 詳細は、[Message Boards Permissions Reference](./message-boards-permissions-reference.md)を参照してください。 管理者は、ゲストがスレッドを投稿できるようにする機能を有効にすることができます。
...
1. ポータルに存在するアセット（メディアファイル、ブログ投稿など）を選択してスレッドに関連付けるには、_［Related Assets］_セクションを展開し、_［Select］_ボタンを使用してそのアセットを選択します。
    ...
    ![図3. タグと関連するアセットを追加する](./creating-message-boards-threads/images/04.png)
...
掲示板スレッドは、アイデアや意見を共有するだけのものではありません。 フォーラムのメンバーは、自分の投稿を他のサイトメンバーへの質問に変えることができます。 質問に対する最も適切な回答は、回答としてマークすることができます。
...
1. _［Mark as an Answer］_ をクリックします。
    ...
    ![図6. 返信を、掲示板の質問に対する回答としてマークできます。](./creating-message-boards-threads/images/02.png)
...
1. _［返信］_ボタンをクリックします。 クイックリプライフォームが開き、返信を入力するためのテキストフィールドだけが表示されます。
