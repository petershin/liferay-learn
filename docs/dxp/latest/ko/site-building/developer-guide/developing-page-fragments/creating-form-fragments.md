...
{bdg-secondary}`사용 가능 Liferay 7.4 U45+/GA45+`
...
1. **Add**( ![Add Button](../../../images/icon-add.png) )을 클릭하고 **Form Fragment** 유형을 선택한 후 **Next** 클릭합니다.

   ![양식 조각 유형을 선택하고 다음을 클릭합니다.](./creating-form-fragments/images/01.png)
...
1. **추가** 클릭합니다. 이렇게 하면 초안 조각이 생성되고 조각 편집 페이지로 이동됩니다.
...
1. (선택 사항) 구성 탭에서 JSON을 사용하여 조각을 구성하고 옵션을 추가합니다. 다양한 조각 구성 유형에 대해 자세히 알아보려면 조각 구성 유형 참조 [조각 구성 유형 참조](../reference/fragments/fragment-configuration-types-reference.md) 참조하세요.

   ![구성 탭에서 JSON을 사용하여 조각을 구성하고 옵션을 추가합니다.](./creating-form-fragments/images/04.png)
...
지원되는 필드 유형을 지정하려면 `조각.json`의 `typeOptions` 에 `fieldTypes` 속성을 추가하세요. 필드 유형을 배열로 입력합니다.
...
양식 조각에는 각 조각의 JavaScript 및 FreeMarker 코드와 함께 사용할 수 있는 `입력` 변수가 포함되어 있습니다. 이 변수에는 조각 구성을 위한 속성이 포함되어 있습니다.
...
| 재산                             | 유형                      | 묘사                                                                                                                                          |
|:------------------------------ |:----------------------- |:------------------------------------------------------------------------------------------------------------------------------------------- |
| `allowedFileExtensions`        | `string`                | 허용되는 파일 확장자. 지원되는 파일 정보 표시가 활성화된 경우 허용되는 확장자가 필드의 도움말 텍스트에 나타납니다.                                                                           |
...
| `selectFromDocumentLibraryURL` | `string` or `undefined` | `selectFromDocumentLibrary` 이 true인 경우 이 속성에는 해당 `ItemSelector`을 렌더링하기 위한 URL이 포함됩니다.                                                       |
