...
サービスは、Liferay Cloud 環境の中核となるものです。 この記事では、Liferay Cloud サービスで発生する可能性のある問題を調査し、対処するために使用できるトラブルシューティングテクニックについて説明します。
...
```{tip}
Liferay サービスについては、次のメッセージを探して、再起動後の最初のログを知らせます: `[LIFERAY] To SSH into this container, run: "docker exec -it liferay-<node-id> /bin/bash".`
```
