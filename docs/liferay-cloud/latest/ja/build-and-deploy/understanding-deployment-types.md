...
サービスのデプロイタイプによって、パフォーマンス、メモリ使用量、ファイルまたはボリュームへのアクセスなど、サービスの機能に関するさまざまな側面が決まります。 Liferay Cloud はそのサービスに対して 2 つのデプロイメントタイプを提供します。 `デプロイメント` と `StatefulSet`です。
...
Liferay Cloudのサービスは、デフォルトで、大半のユースケースに適合するようにあらかじめ設定されています。 ただし、各サービスのデプロイメントの種類は、サービスの `LCP.json`の `kind` 変数を介して設定できます。
