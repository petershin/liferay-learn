...
Amazon의 단순 스토리지 서비스(S3)는 DXP의 S3 Store가 파일을 클라우드에 원활하게 저장하는 데 사용하는 클라우드 기반 스토리지 솔루션입니다. AWS 계정이 있고 S3 **버킷** 생성했으면 S3 스토어를 구성할 수 있습니다.

S3는 파일 저장을 위해 버킷 개념을 사용합니다. Amazon UI에서 DXP 파일에 대한 [버킷](https://docs.aws.amazon.com/AmazonS3/latest/user-guide/create-bucket.html) 생성합니다. DXP 서버 호스트에 가능한 한 가까운 [지역](https://docs.aws.amazon.com/general/latest/gr/s3.html) 에 버킷을 구성합니다.

[AWS 계정](https://aws.amazon.com/s3/) 만들 때 Amazon은 귀하를 귀하의 계정에 연결하는 [고유한 키](https://docs.aws.amazon.com/general/latest/gr/aws-sec-cred-types.html/) 할당합니다. 이 키를 사용하여 S3 Store를 구성합니다.
...
Amazon S3에는 SAXParser가 필요합니다. 환경에서 SAXParser를 사용할 수 없는 경우

1. [Xerces SAXParser](https://xerces.apache.org/mirrors.cgi) 애플리케이션 서버의 전역 라이브러리 폴더(Tomcat의 경우 `/lib/ext` 또는 JBoss EAP 및 WildFly의 경우 `/module` 에 다운로드합니다.

1. [`system-ext.properties`](../../../installation-and-upgrades/reference/system-properties.md) 파일에 SAXParser를 지정합니다.
...
1. [`Portal-ext.properties`](../../../installation-and-upgrades/reference/portal-properties.md)구성:
...
1. 애플리케이션 서버에 아직 SAXParser가 없는 경우 [Xerces SAXParser](https://xerces.apache.org/mirrors.cgi) 각 노드의 애플리케이션 서버 전역 라이브러리 폴더에 복사합니다.
