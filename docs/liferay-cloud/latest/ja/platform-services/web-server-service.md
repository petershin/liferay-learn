...
| `LCP_WEBSERVER_MODSECURITY`               | `Off`       | ModSecurityを有効または無効にします。 この値を `On` に設定するとルールが有効になって処理され、`DetectionOnly` に設定すると破壊的なアクションを実行せずにルールが処理される。 [Web Application Firewall](../infrastructure-and-operations/security/web-application-firewall.md) を参照。 |
