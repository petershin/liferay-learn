...
For example, consider a scenario where you set Discount A as the channel default for a business account, but only Order Type X is eligible for Discount A. Without overriding eligibility, Liferay does not apply Discount A to orders of a different type. Instead, Liferay applies the next highest priority discount for which the order is eligible, if any exists. However, if you override eligibility, Liferay applies Discount A to all account orders, regardless of eligibility.
