...
```{important}
Liferay 7.4 U85/GA85以前のバージョンでは、ユーザーは公開フィールドを削除できません。 公開後は、オブジェクトのサイドテーブルに追加されたフィールド（例：`[Initial_Table_Name]_x`）のみを削除することができます。 詳しくは、 [オブジェクトドラフトの公開](../creating-objects.md#publishing-object-drafts) をご覧ください。
```
...
| [暗号化済み](./encrypted-fields.md)   | 文字列             | 暗号化された文字列値を格納する。                                                                                                                                                              |
