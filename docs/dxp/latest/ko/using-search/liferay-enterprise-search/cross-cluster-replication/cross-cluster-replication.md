...
![클러스터 간 복제를 사용하면 서로 다른 데이터 센터에서 Liferay DXP 인덱스와 동기화된 Elasticsearch 클러스터를 유지할 수 있습니다.](./cross-cluster-replication/images/01.png)
...
클러스터 간 복제도 지원하는 Liferay DXP에서 지원되는 버전을 사용하여 Elasticsearch 클러스터를 설정합니다. 자세한 내용은 [LES 호환성 매트릭스](https://help.liferay.com/hc/en-us/articles/360016511651#Liferay-Enterprise-Search) 참조하십시오.

[Liferay DXP에 필요한 Elasticsearch 플러그인을 설치하고 클러스터 이름을 제공](../../installing-and-upgrading-a-search-engine/elasticsearch/installing-elasticsearch.html#configure-elasticsearch)하여 팔로워 및 리더 클러스터.

CCR에는 Elasticsearch Platinum 수준 라이선스가 필요하지만 [LES 고객](../../liferay-enterprise-search.html) 은 이미 이를 가지고 있습니다. 로컬에서 테스트하는 경우 각 클러스터에서 [평가판 라이선스](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/start-trial.html) 시작합니다.
...
Liferay DXP에는 CCR 설정을 완료하는 논리가 포함되어 있지만 구성 파일 [](../../../system-administration/configuring-liferay/configuration-files-and-factories/using-configuration-files.md)않고 시스템 설정 UI에서 CCR 기능을 활성화하는 데 의존합니다. 최소한 `readFromLocalClusters` 속성은 UI에서 트리거되어야 합니다. CCR이 구성되면 남은 것은 인덱스 복제를 확인하고 검색을 시작하는 것입니다.

CCR을 처음 활성화할 때(구성에서 _업데이트_ 클릭한 후 --- [로컬 팔로워 데이터 센터에서 CCR 구성](./configuring-ccr-in-a-local-follower-data-center.md)참조) 로컬 클러스터 구성의 각 항목이 처리됩니다. 먼저, [클러스터 업데이트 설정 API](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/cluster-update-settings.html)을 통해 [원격 클러스터](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/modules-remote-clusters.html) 등록합니다. 원격 클러스터의 각 인덱스( `.` 로 시작하는 인덱스 또는 Excluded Indexes 설정에 정의된 인덱스 제외)에 대해 [Create Follower API](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/ccr-put-follow.html) 이 호출되어 원격 인덱스와의 팔로워/리더 관계를 설정합니다. 
