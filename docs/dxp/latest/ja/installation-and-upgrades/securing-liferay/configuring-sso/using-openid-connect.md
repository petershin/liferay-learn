...
```{note}
クライアントとして、LiferayはOpenID Connectを使用する場合、PKCE(Proof Key for Code Exchange）をサポートしています。 詳細は[PKCE仕様](https://www.rfc-editor.org/rfc/rfc7636)をご参照ください。 値を確認し、適切にリクエストを承認または拒否することは、プロバイダーに任されています。
```
