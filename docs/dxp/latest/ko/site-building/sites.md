...
```{note}
사이트 수준에서 생성된 페이지, 콘텐츠 및 기타 애플리케이션 데이터는 해당 사이트로 범위가 제한되며 일반적으로 사이트 간에 공유할 수 없습니다. 그러나 일부 데이터는 [계층 구조의 사이트](./사이트/사이트-계층 구조.md) 간에 공유할 수 있습니다. 
...
라이프레이 사이트는 [채널](https://learn.liferay.com/commerce/latest/en/store-management/channels.html) 과 함께 이커머스 스토어를 생성하는 데 사용됩니다. 각 채널의 사이트 유형(예: B2B, B2C 또는 B2X)에 따라 사이트가 계정과 작동하는 방식이 결정됩니다. 자세한 내용은 [커머스 사이트 유형](https://learn.liferay.com/commerce/latest/en/starting-a-store/sites-and-site-types.html) 을 참조하세요.
