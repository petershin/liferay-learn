...
1. ページ要素のサイドパネルにある「**一般**」 タブをクリックします。

![フラグメントを選択し、ページ要素サイドパネルの一般設定にアクセスします。](./general-settings-reference/images/01.png)
...
| フレックス・ラップ（フレックス列またはフレックス柱用） {bdg-secondary}`7.4 U19+/GA19+` | フレックスの行と列の折り返しを設定する：折り返しなし、折り返し、折り返し反転。                                               |
| アイテムの整列（フレックスコラム用） {bdg-secondary}`7.4 U19+/GA19+`          | 列の要素をどのように整列させるかを決定します：Start、Center、End、Stretch、Baseline。                             |
...
| クリア      | ［クリア］をクリックして、選択した色を削除します。                 | <!-- For Liferay DXP 7.4 U9+, these settings are listed under the \[Styles\](./styles-reference.md) tab.--> 
## スライダーオプション
...
| ソース (**for Category**)         | 対象コレクションのフィルタリングに使用する語彙またはカテゴリを選択します。                                                                                                                                           |
| シングル・セレクション (**for Category**) | エンドユーザがコレクションに1つまたは複数のフィルタのみを適用できるかどうかを決定します。                                                                                                                                   |
| 検索フィールドを含む (**for Category**)  | エンドユーザーが使用可能なフィルタ値を検索するためのフィールドを含めるかどうかを決定します。                                                                                                                                  |
| ラベルを表示                       | UIにフィルタのラベルを表示する場合はオンにします。                                                                                                                                                      |
| ラベルテキスト                      | ラベルにローカライズ可能なテキストを入力します。 他の言語のラベルテキストを設定するには、まず編集バーでページやテンプレートの言語を切り替えます。 次に、目的のテキストを入力します。 詳細は、[フラグメント設定フィールドのローカライズ](./localizing-fragment-configuration-fields.md)を参照してください。 |
...
| リストのスタイル                  | 使用可能なスタイルから選択して項目の一覧表示方法を決定します。 利用可能なスタイルは、選択したコレクションまたはコレクションプロバイダによって異なります。                                                 |
| レイアウト (**グリッド用**)           | コレクションリストの列数を決定する：1-6.                                                                                                        |
| リスト項目のスタイル (**for テンプレート**) | 使用可能なスタイルから選択して、各エントリーの表示方法を決定します。 利用可能なスタイルは、選択したコレクション資産の種類によって異なります。                                                       |
...
| サクセス・テキスト (**for Embedded**)                | ローカライズ可能なサクセスメッセージを入力します。                                                                        |
| サクセス・ステートをプレビュー (**for Embedded**)          | 埋め込まれた成功メッセージをプレビューする。                                                                           |
| コンテンツの表示                                  | コンテナのコンテンツをブロック、フレックス行、フレックス列のいずれで表示するかを決定します。                                                   |
| フレックス・ラップ (**for Flex Row or Flex Column**) | フレックスの行と列の折り返しを設定する：折り返しなし、折り返し、折り返し反転。                                                          |
| アイテムの整列 (**for Flex Column**)               | 列の要素をどのように整列させるかを決定します：Start、Center、End、Stretch、Baseline。                                        |
| コンテンツのジャスティファイ (**for Flex Row**)           | 行内の要素の両端揃え方法を決定します：開始]、[中央]、[終了]、[間]、または[周囲]。                                                    |
...
| 項目        | 説明                                                                                                                                                                                                                                                                                                                                                                                                                   |
|:--------- |:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ボキャブラリの種類 | 語彙の可視性に基づいてカテゴリーを表示する。 詳しくは [Vocabulary Visibility](../../../../../content-authoring-and-management/tags-and-categories/organizing-content-with-categories-and-tags.md#vocabulary-visibility) を参照。 この設定にアクセスするには、 `Enable Categorization` がオブジェクト上で有効になっていなければならない。 詳しくは、 [オブジェクトの作成](../../../../../building-applications/objects/creating-and-managing-objects/creating-objects.md#editing-object-drafts) を参照してください。 |
