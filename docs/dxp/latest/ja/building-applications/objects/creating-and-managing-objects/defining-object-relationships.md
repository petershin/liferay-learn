...
```{important}
オブジェクトエントリーのリレーションシップを表示するには、カスタムレイアウトを作成する必要があります。 詳しくは、[オブジェクトレイアウトの設計](./designing-object-layouts.md)を参照してください。 
```
...
```{important}
オブジェクトはドラフトですが、親側からリレーションシップを編集および削除できます。 ただし、オブジェクトが公開されると、既存のリレーションシップまたは新しく追加されたリレーションシップを削除することはできません。 ユーザーは、リレーションシップのラベルと[削除タイプ](#configuring-deletion-type)のみを構成できます。
```
...
1. ダイアログウィンドウで、リレーションシップの名前を入力し、 ［**Done**］ をクリックして確定します。
