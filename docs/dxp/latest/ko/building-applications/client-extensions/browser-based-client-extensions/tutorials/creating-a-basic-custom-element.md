...
사용자 정의 요소는 Liferay의 프런트 엔드 인프라를 사용하여 외부 원격 애플리케이션을 Liferay 플랫폼에 등록하고 이를 위젯으로 렌더링하는 클라이언트 확장 유형입니다.
...
생성된 코드에는 `hello-world` (기본값), `hello-foo`및 `hello-bar`의 세 가지 경로가 포함됩니다. 경로는 애플리케이션을 실행할 때 사용할 수 있는 대체 코드 세트입니다. 기본 예제는 [사용자 지정 요소와 함께 경로 사용](./using-routes-with-custom-elements.md) 을 참조하십시오.
