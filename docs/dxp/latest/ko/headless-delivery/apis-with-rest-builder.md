...
REST 빌더는 `rest-config.yaml` 및 `rest-openapi.yaml` 파일에서 정의한 구성을 사용하여 API가 한 번에 작동하는 데 필요한 대부분의 코드를 생성합니다. 클래스 이름과 코드를 넣을 사이트를 구성하면 REST Builder가 필요한 모든 파일을 생성합니다. 그런 다음 구현 논리를 추가합니다.
