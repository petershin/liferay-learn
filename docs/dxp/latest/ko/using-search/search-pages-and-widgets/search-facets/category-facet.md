...
카테고리 패싯을 구성하려면 패싯의 _옵션_ 메뉴(![Options](../../../images/icon-app-options.png))를 열고 *구성*을 클릭하십시오.
...
패싯에 표시할 어휘 및 범주를 선택합니다. 이를 활성화하려면 System Settings &rarr; Search &rarr; Category Facet Field의 구성을 기본 필드인 `assetCategoryIds`에서 `assetVocabularyCategoryIds` 필드로 전환합니다. 이 설정은 범주 패싯에서 집계를 만드는 데 사용되는 필드를 지정합니다. `assetVocabularyCategoryIds` 을 설정하면 카테고리 패싯 위젯의 구성 화면 내에서 어휘 설정이 활성화됩니다.
...
이 동작을 통해 강력한 검색 패싯 경험을 제공할 수 있습니다. 하나의 사용 사례에 대해서는</a> 아래의
...
1. 사이트 이름을 Foo로 지정하고 _추가_을 클릭합니다.
...
1. `assetVocabularyCategoryIds` 필드를 사용하여 집계를 생성하도록 Category Facet 위젯을 변경합니다. 시스템 설정 &rarr; 검색으로 이동합니다. 카테고리 패싯 필드 항목의 카테고리 패싯 필드 선택기에서 `assetVocabularyCategoryIds` 을 선택하고 구성을 업데이트하십시오.
...
1. Foo 사이트의 홈 페이지에서 Foo 어휘의 범주만 표시하도록 구성된 범주 패싯 위젯을 추가합니다. 
   ...
   - 디스플레이 템플릿 선택기를 열고 _Vocabulary Layout_을 선택합니다.
   - 범주 매개변수 이름을 _foocategories_로 변경하십시오.
   - _어휘 선택_ 라디오 버튼을 클릭합니다. _Global_ 을 확장하고 _Foo_을 선택합니다.
   - _저장_을 클릭합니다.
