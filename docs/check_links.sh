...
function check_includes {
	...
	do
		...
		then
			local link_type="Literal include"

			use_english_path ${1}

			# this won't work from a translation folder, we probably can't use ag for this
			local resources_dir_name=$(echo ${_LINK_FILE_NAME} | sed 's/\(.*\/resources\)\/.*/\1/g')

			if [[ $(ag -g ${resources_dir_name}/.*service\.xml) ]]
			then
				local generated_code_warning="Warning: this may reference code generated by service builder"
			fi

			if [[ $(ag --file-search-regex "${resources_dir_name}/update_example\.sh" "generate_custom_element") && ! $(ag --files-with-matches liferay-.*-overlay) ]]
			then
				local generated_code_warning="Warning: this may reference code generated by generate_custom_element"
			fi
		else
			local link_type="Include"
...
function use_english_path {
	if [[ ${1} != *"latest/en"* ]]
	then
		_LINK_FILE_NAME=$(pwd ${_LINK_FILE_NAME})/${_LINK_FILE_NAME}

		_LINK_FILE_NAME=$(echo "${_LINK_FILE_NAME/\/ja\//\/en\/}")
		_LINK_FILE_NAME=$(echo "${_LINK_FILE_NAME/\/ko\//\/en\/}")
	fi
}
