...
1. ドキュメント `_source` のフェッチを有効にするには、ブループリントの _Configuration_ タブを開き、この JSON を Advanced Configuration ボックスに入力します：
...
1. ツールバーの _プレビュー_ をクリックし、ネストしたフィールドを持つ結果を検索し、そのフィールドを展開します。
