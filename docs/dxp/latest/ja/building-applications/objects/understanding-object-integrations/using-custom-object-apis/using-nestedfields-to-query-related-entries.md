...
1. `able-one` のIDをパラメータとして、 `Baker_POST_ToCompany` を実行する。
   ...
   `` `ableToBakerERC , r_ableToBaker_c_ableId , r_ableToBaker_c_ableERC の3つの関係フィールドを持つ。` `` ``
...
1. `baker-one` のIDをパラメータとして、 `Charlie_POST_ToCompany` を実行する。
   ...
   これは、3 つの Charlie エントリを作成し、 `bakerToCharlie` リレーションシップを使用して、指定された Baker エントリに関連付けます。
...
1. `Charlie_GET_ById` をチャーリー・エントリーIDをパラメータとして実行する。
