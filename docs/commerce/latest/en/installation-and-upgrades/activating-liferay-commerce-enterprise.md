...
```{important}
Starting from Liferay DXP 7.4 U48+/GA48+, you don't need a separate license to use Commerce features. They are available out-of-the-box and cannot be disabled. The portal property to disabled Commerce modules (`enterprise.product.commerce.enabled`) has also been removed.
```
