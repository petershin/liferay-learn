...
CLI 도구를 사용하여 Liferay Cloud 서비스를 보고 관리할 수 있습니다. 일단 설치되면 터미널에서 `lcp --help` 을 실행하여 사용 가능한 작업을 볼 수 있습니다.

```{tip}
추가 로깅 및 정보를 보려면 `lcp` 명령 끝에 `-v` 또는 `--verbose` 옵션을 실행할 수 있습니다. `lcp` 명령의 끝에서 `--help` 플래그를 실행하여 함께 사용할 수 있는 플래그를 확인하십시오.
```
...
* [Liferay 클라우드 환경에 배포](#deploying-to-your-liferay-cloud-environment)
* [서비스의 셸에 액세스](#accessing-a-services-shell)
...
권한 오류가 발생하면 명령을 다시 실행하고 `bash` 을 `sudo bash`으로 바꾸십시오.
...
1. 터미널을 열고 다음을 실행하십시오: `lcp uninstall`.

1. `lcp`명령을 실행하여 설치 제거가 완료되었는지 확인하십시오.
...
버전 2가 제거되면 위의 [지침](#installing-the-cli-tool) 에 따라 CLI 도구 버전 3을 설치할 수 있습니다.

## CLI 원격 구성

CLI 도구를 통해 Liferay Cloud 서비스에 액세스하려면 Liferay Cloud의 원격 URL: `liferay.cloud`을 가리키도록 구성해야 합니다. CLI의 원격을 나열하려면 다음 명령을 실행하십시오.

```shell
lcp remote
```

다음 명령을 사용하여 CLI 도구에 새 원격을 추가할 수 있습니다.
...
`lcp log` 명령을 사용하여 특정 프로젝트, 서비스 및 인스턴스에 대한 로그를 표시합니다. 다음 예제에는 몇 가지 일반적인 명령이 포함되어 있지만 `lcp log --help`을 실행하여 사용 가능한 명령 옵션을 볼 수 있습니다.

프로젝트 환경에 대한 모든 서비스 로그 보기:

```shell
lcp log -p <project>-<environment>
```

프로젝트 환경에서 단일 서비스에 대한 로그 보기:

```shell
lcp log -p <project>-<environment> -s <service>
```

또는 서비스의 전체 URL을 사용하여 해당 로그를 봅니다.
...
특정 시간부터 현재까지 프로젝트 환경에 대한 모든 서비스 로그 보기:

```shell
lcp log -p <project>-<environment> --since <start_time>
```

특정 기간의 프로젝트 환경에 대한 모든 서비스 로그 보기:

```shell
lcp log --since "<start_time>" --until "<end_time>"
```

명령 끝에 `>> "<new-file.txt>"` 을 추가하여 `lcp log` 명령의 출력을 파일로 파이프할 수도 있습니다. 실행하면 터미널의 현재 경로에 새 파일이 생성됩니다.
...
`lcp scale` 명령을 사용하여 환경 서비스를 개별적으로 확장합니다. 다음 예제에는 몇 가지 일반적인 명령이 포함되어 있지만 `lcp scale --help`을 실행하여 사용 가능한 명령 옵션을 볼 수 있습니다.

프로젝트 환경에 대한 모든 서비스 및 해당 인스턴스를 본 다음 확장할 서비스를 선택합니다.

```shell
lcp scale -p <project>-<environment>
```

그런 다음 터미널 프롬프트에 따라 서비스를 선택하고 인스턴스 수를 결정합니다.
...
`lcp list` 명령을 사용하여 각각 이미지와 상태가 있는 프로젝트 서비스를 봅니다. 다음 예에는 몇 가지 일반적인 명령이 포함되어 있지만 `lcp list --help`을 실행하여 사용 가능한 명령 옵션을 볼 수 있습니다.

소유하거나 공동 작업하는 프로젝트, 서비스 및 인스턴스의 전체 목록을 봅니다.
...
`lcp restart` 명령을 사용하여 환경 서비스를 다시 시작하십시오. 다음 예에는 몇 가지 일반적인 명령이 포함되어 있지만 `lcp restart --help`을 실행하여 사용 가능한 명령 옵션을 볼 수 있습니다.

모든 프로젝트 환경 및 서비스를 본 다음 다시 시작할 환경 서비스를 선택합니다.
...
또는 전체 URL을 `lcp restart`에 전달하여 서비스를 다시 시작하십시오.
...
`lcp deploy` 명령을 사용하여 특정 서비스 또는 모든 서비스를 환경 중 하나에 배포합니다.

```shell
lcp deploy
```

저장소의 루트 디렉터리에서 `lcp deploy` 을 실행하면 적용 가능한 모든 서비스가 선택한 환경에 배포됩니다. 한 서비스의 폴더(해당 폴더에 서비스의 `LCP.json` 파일 포함)에서 명령을 실행하여 해당 서비스를 환경에 구체적으로 배포합니다.

`-r` 과 함께 명령을 실행하여 기본값이 아닌 원격(ID로)을 지정합니다.

```shell
lcp deploy -r <remote-id>
```

자세한 내용은 [CLI 도구를 통해 변경 사항 배포](../build-and-deploy/deploying-changes-via-the-cli-tool.md) 을 참조하십시오.

## 서비스의 셸에 액세스

서비스 컨테이너의 셸에 액세스하려면 다음 명령을 실행합니다.

```shell
lcp shell
```

그러면 컨테이너의 모든 서비스가 나열되고 액세스할 서비스를 선택 메시지가 표시됩니다.
...
`lcp docs` 명령을 사용하여 공식 문서, 도움말 센터 콘텐츠 등을 포함한 Liferay Cloud 리소스에 액세스합니다.

## CLI 도구 버전 3 제거

Mac 및 Linux의 경우 다음 명령을 실행합니다.
