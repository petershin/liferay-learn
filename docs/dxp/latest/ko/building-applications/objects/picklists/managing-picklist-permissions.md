...
리소스 권한은 특정 애플리케이션 리소스에 대한 권한을 부여합니다. 이러한 권한 중 일부는 데이터베이스 엔터티</a> (예: 모델 리소스)에서
작업을 수행할 수 있도록 합니다. 다른 것들은 응용 프로그램 컨텍스트에서 자원 관련 작업 [을 수행할 수 있는 권한을 부여합니다(예](#actions-on-database-entities) 새 자원 엔터티 생성).</p> 
