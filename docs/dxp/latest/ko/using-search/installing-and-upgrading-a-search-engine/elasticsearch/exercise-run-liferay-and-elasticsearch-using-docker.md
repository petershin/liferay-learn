...
1. `elasticsearch-liferay`이라는 Elasticsearch 컨테이너를 구성하고 시작합니다.
...
1. 실행 중인 Elasticsearch 컨테이너의 IPv4 주소를 가져옵니다.
   ...
   이 예에서는 `172.17.0.2`입니다. 시스템에서 다른 IP 주소를 제공하는 경우 Liferay DXP를 실행할 때 `docker run --add-host elasticsearch-liferay:[IP]...` 명령에서 사용해야 합니다.
