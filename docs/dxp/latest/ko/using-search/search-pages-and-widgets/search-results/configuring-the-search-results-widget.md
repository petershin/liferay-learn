...
**컨텍스트에서 선택한 결과 표시:** 동일한 사이트의 페이지에서 감지된 경우 클릭한 자산을 기본 디스플레이 위젯에 표시합니다. 예를 들어 검색 결과에서 블로그 게시물을 클릭하면 블로그 애플리케이션에 블로그 항목이 표시됩니다. 검색 결과를 클릭하면 더 이상 검색 컨텍스트에 있지 않습니다.
...
**표시할 필드:** 통합 검색 키 구성을 사용하여 [대체 인덱스](./understanding-low-level-search-options.md) 검색하는 경우 해당 인덱스에서 검색할 필드를 지정합니다. 검색 결과 위젯에 표시되려면 필드가 모두 인덱싱되고 [저장](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/mapping-store.html) 되어야 합니다.
...
자세한 내용은 더 나은 검색어 [에 대한 제안을 반환하는 방법을 확인](./enabling-search-suggestions.md) (예: "...을(를) 찾으셨습니까?") 초기에 충분한 결과가 반환되지 않은 경우.
