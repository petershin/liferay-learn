...
[사이트 대시보드](../touchpoints/sites-dashboard.md) 및 [개인 대시보드](../people/individuals/individuals-dashboard.md)을 통해 다양한 분석 데이터에 액세스할 수 있습니다. Analytics Cloud의 API를 사용하여 Analytics 데이터를 가져오고 내보낼 수도 있습니다. 특히 계정 데이터, 개별 데이터, 세그먼트 데이터 또는 페이지 데이터를 가져올 수 있습니다.

## 데이터 내보내기 요청

1. 작업 공간의 액세스 토큰을 기록해 두십시오. 액세스 토큰을 생성하거나 얻으려면 [인증](authentication.md) 을 참조하십시오.

1. 데이터 요청은 다음 유형의 형식을 취합니다.
   ...
   `{token}` 를 특정 액세스 토큰으로 바꿉니다. `{type}` 를 **계정**, **개인**, **페이지**또는 **세그먼트**으로 바꿉니다. `fromDate=`다음에 `{ISO 8601 date and time}` 를 교체하여 시작 날짜와 시간을 지정합니다. `toDate=`다음에 `{ISO 8601 date and time}` 를 교체하여 종료 날짜와 시간을 지정합니다. [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) 날짜 및 시간 형식을 사용해야 합니다. 요청에는 시작 날짜 및 시간과 종료 날짜 및 시간이 필요합니다.
...
1. 동일한 명령을 다시 실행하면 터미널 창에 데이터가 반환됩니다. 요청이 아직 처리 중인 경우 `"status":"RUNNING"` 메시지가 표시되며 나중에 다시 확인하십시오.

   데이터를 JSON 파일로 다운로드하려면 요청 끝에 `>> {filename.json}` 를 추가하십시오. 예를 들어:
