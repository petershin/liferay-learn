...
* **스타일북의 값** 버튼을 클릭하면 현재 사용 중인 [스타일북](../../../사이트-외관/스타일북/스타일북을 사용하여 사이트-외관 표준화.md)에 정의된 사전 정의된 색상 메뉴를 열 수 있습니다. 이렇게 하면 필드 값이 선택한 토큰에 연결되며, 버튼을 다시 눌러 연결을 해제할 때까지 연결이 유지됩니다. 토큰 값의 연결을 해제하면 선택한 색상이 해당 16진수 코드 값으로 다시 변환됩니다. 
  
* 색상 선택기에 기본값이 정의되어 있지 않은 경우 **기본값** 드롭다운 메뉴를 클릭하여 현재 사용 중인 스타일북에서 원하는 색상을 선택합니다. 이는 값을 선택할 때 스타일북에서 값 버튼과 동일하게 작동합니다.

![색상 선택기 구성을 사용하면 색상 값을 직접 입력하거나, 범위에서 하나를 선택하거나, 스타일 북에서 하나를 선택할 수 있습니다.](./fragment-configuration-types-reference/images/02.png)

```{note}
사용 중인 테마에 [스타일 북에 대한 토큰 정의](../../../site-appearance/style-books/developer-guide/style-book-token-definitions.md)가 없는 경우 색상 선택기 페이지의 구성은 [색상 팔레트](#color-palette 구성) 구성으로 대체됩니다.
```

이 JSON 구성은 ``색상 선택기 필드를 생성합니다.
...
조각이 렌더링되면 토큰 `${configuration.OBJECT_NAME}` 가 선택한 색상으로 대체됩니다. 색상이 선택되는 방식에 따라 값 유형이 달라집니다.
...
{bdg-secondary}`사용 가능 Liferay 7.4 U44 및 GA44+`

`길이` 구성 유형은 숫자를 입력하고 사전 정의된 단위를 선택하거나 너비, 여백 및 패딩과 같은 설정에 대한 자체 단위를 지정할 수 있는 필드를 생성합니다. 
...
![맞춤 조각의 '크기'라는 필드는 길이 구성을 사용하여 사용 가능한 단위의 드롭다운을 표시합니다.](./fragment-configuration-types-reference/images/10.png)

## 항목 선택기 구성

{bdg-secondary}`사용 가능 Liferay 7.3+`

이 구성은 조각에 포함할 기존 콘텐츠(기본적으로 웹 콘텐츠 기사, 블로그 항목 또는 문서) 하나를 선택하기 위한 선택기를 만듭니다.
...
작성자가 특정 유형의 콘텐츠만 선택할 수 있도록 하는 고급 구성을 제공할 수 있습니다. 아래 구성은 웹 콘텐츠 기사만 선택할 수 있도록 지정합니다. 선택적 `itemSubtype` 속성은 선택한 웹 콘텐츠 기사가 선택되기 위해 구조 `기사 구조-키-15` 을 사용해야 함을 지정합니다.
...
이 예에서는 구조 `메타데이터세트-구조-키-2` 를 사용하는 `img` 또는 `jpg` MIME 유형의 문서만 선택할 수 있도록 지정합니다.
...
그런 다음 웹 콘텐츠 기사에 대한 이 HTML 조각을 사용하여 조각의 콘텐츠를 렌더링할 수 있습니다.
...
콘텐츠의 특정 부분에 액세스해야 하는 경우 `[name-of-field]Object` 키(아래 예에서는`itemSelector1Object` ) 아래 조각의 Java 객체에 액세스할 수도 있습니다. 이 예에서는 웹 콘텐츠 기사의 제목, 설명 및 본문을 렌더링합니다.
...
![항목 선택기 구성은 기존 콘텐츠를 표시하기 위한 옵션 선택이 필요할 때 유용합니다.](./fragment-configuration-types-reference/images/04.png)
...
이 구성으로 조각을 구성할 때 드롭다운 메뉴에서 URL을 수동으로 입력하거나(`URL`) 동일한 사이트에서 다른 페이지를 선택하고 전체 URL을 사용하도록 선택할 수 있습니다(`Page`).
...
```{note}
`videoSelector` 유형은 [외부 비디오](../../../creating-pages/page-fragments-and-widgets/using-fragments/default-fragments-reference.md#external-video와 호환됩니다. ) 조각이지만 [동영상 URL](../../../creating-pages/page-fragments-and-widgets/using-fragments/default-fragments-reference.md) 조각은 아닙니다.
```
...
`collectionSelector` 구성 유형을 사용하면 [컬렉션](../../../../content-authoring-and-management/collections-and-collection-pages/about-collections-and-collection-pages.md) 또는 컬렉션 공급자를 포함하는 조각을 개발할 수 있습니다. 수동 및 동적 컬렉션 모두에 `collectionSelector` 을 사용할 수 있습니다.

```{note}
개발자는 컬렉션 공급자를 사용하여 고급 기준으로 특정 컬렉션을 만들 수 있습니다. 자세한 내용은 [정보 목록 공급자 만들기](https://help.liferay.com/hc/ko/articles/360029067271-Creating-an-Information-List-Provider) 에 대한 정보를 읽어보세요. 프레임워크](https://help.liferay.com/hc/ko/articles/360029067251-Introduction-to-The-Info-Framework) 개발자 문서.
```
...
다음 HTML 코드 샘플과 함께 이 조각 구성을 사용하여 컬렉션 항목을 나열할 수 있습니다. `collectionObjectList` [콘텐츠 페이지 편집기](../../../creating-pages/using-content-pages/content-page-editor-ui-reference.md)에서 선택된 컬렉션을 나타냅니다.

HTML에서 이 컬렉션을 참조하려면 JSON 구성에서 컬렉션 `이름` 과 `ObjectList` 접미사를 사용하세요. 이전 JSON 코드 발췌에서 컬렉션 `이름` 은 `컬렉션` 이므로 HTML은 `collectionObjectList`사용하여 컬렉션을 참조합니다.
...
![컬렉션 구성을 사용하여 컬렉션 선택기로 조각을 개발할 수 있습니다.](./fragment-configuration-types-reference/images/08.png)
...
기본적으로 최대 항목 수에 대한 제한을 정의하지 않으면 컬렉션의 모든 항목이 반환됩니다. JSON 구성에서 `numberOfItems` 사용하여 이 제한을 설정할 수 있습니다.
...
색상 팔레트 JSON 구성은 색상을 선택해야 하는 경우에 구현할 수 있는 색상 선택기를 생성합니다. [color picker 구성](#color-picker-configuration) 과 달리 현재 사용되는 [Style Book](../../../site-appearance/style-books/using-a-style-book-to-standardize-site-appearance.md)의 색상 시스템에 구성된 테마 색상을 기준으로 옵션만 제공합니다.

이 구성은 `textColor`이라는 색상 팔레트 필드를 생성합니다.
...
흰색을 선택한다면 `h3` 태그 제목은 클래스 `text-white'`갖게 됩니다.
