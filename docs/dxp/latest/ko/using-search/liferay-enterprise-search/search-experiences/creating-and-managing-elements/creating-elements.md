...
검색 청사진</a>과 함께 제공되는
개의 시스템 요소가 많이 있습니다. 어떤 시스템 요소도 요구 사항을 충족하지 못하는 경우 고유한 검색 청사진 요소를 만드는 다양한 방법이 있습니다.</p> 
...
요소 소스 편집기에서 미리 정의된 변수를 요소에 추가하려면 변수가 있을 위치에 커서를 놓은 다음 왼쪽 사이드바에서 변수를 클릭하여 삽입하십시오. Custom JSON Element에서 변수를 편집기에 직접 입력해야 합니다.
...
- 쿼리 절에 필드 값을 전달할 때와 같이 요소의 다른 위치에서 필드를 참조하는 데 사용할 수 있는 `이름` 을 입력합니다: `${configuration.name}`.
...
- `typeOptions` 속성을 사용하여 각 필드 유형에 사용할 수 있는 옵션을 구성합니다. 예를 들어 선택 필드에 대해 `옵션` 표시를 설정합니다. 
   ...
   - `형식` 은 허용 가능한 날짜 형식 [을 결정합니다(예:](https://www.elastic.co/guide/en/elasticsearch/reference/7.x/mapping-date-format.html)</code>`.</li>
<li><code>nullable` 은 null 값을 전달할 수 있는지 여부를 설정합니다.
