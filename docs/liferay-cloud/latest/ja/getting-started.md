...
[クラスタリング](./using-the-liferay-dxp-service/setting-up-clustering-in-liferay-cloud.md) と [ロードバランシング](.//configuring-the-cloud-network/load-balancer.md) 、可用性の高い、ダウンタイムのないデプロイを提供し、ミッションクリティカルなサイトがオンラインのままであることを保証します。 Liferay Cloudは、高可用性を確保するため、マルチリージョンアーキテクチャでデータを配信します。
