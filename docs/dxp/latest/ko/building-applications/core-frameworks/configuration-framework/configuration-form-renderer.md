...
구성 인터페이스에서 구성 가능한 속성을 정의합니다. 샘플 프로젝트에는 하나의 구성 가능한 속성이 있습니다: `b7r2Color`.
...
1. `ConfigurationFormRenderer`구현을 만듭니다. `@Component` 주석에서 서비스를 `ConfigurationScreen.class`으로 선언합니다.

   ```java
   @Component(service = ConfigurationScreen.class)
   ```

1. `getPid()` 메서드를 재정의합니다. 구성 클래스의 `@Meta.OCD` 주석에 지정된 대로 전체 구성 ID를 반환해야 합니다.
...
1. `getRequestParameters()` 메서드를 재정의합니다. 메서드에서 사용자 지정 UI에서 보낸 매개 변수를 읽고 키가 구성 인터페이스에서 찾은 필드와 일치하는 맵에 넣습니다.
...
1. `<aui:input>` 태그를 사용하여 사용자가 JSP 파일에 제출하는 새 구성 입력을 읽습니다.

   ```jsp
   <aui:input name="b7r2Color" value="<%= b7r2WebConfiguration.b7r2Color() %>" />
   ```

이 샘플 프로젝트는 `ConfigurationFormRenderer`을 사용하여 기본 사용자 지정 구성 UI를 보여줍니다. 애플리케이션에서 자체 코드를 작성하여 구성 UI의 모양과 느낌을 추가로 사용자 정의하십시오.
