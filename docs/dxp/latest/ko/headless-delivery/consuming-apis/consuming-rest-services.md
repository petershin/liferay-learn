...
Liferay DXP에는 대부분의 애플리케이션을 위한 REST 서비스가 포함되어 있습니다. 이러한 서비스는 [OpenAPI](https://app.swaggerhub.com/apis/liferayinc/headless-delivery) 를 완벽하게 준수합니다. 여기에서 소비하는 방법을 알아보세요. 이 과정은 세 단계만 거치면 됩니다:

1. 사용하려는 서비스를 식별합니다.
1. 필요한 데이터가 포함된 사이트를 식별합니다.
1. 데이터에 액세스할 수 있는 자격 증명을 사용하여 서비스 호출을 수행합니다.

이 예제에서는 Liferay DXP를 새로 설치한 Docker 이미지를 사용합니다.
...
2. **Show Schemas** 버튼을 클릭하면 화면 오른쪽에 해당 카테고리의 모든 스키마 목록이 나타납니다.

3. 브라우저 탭을 스키마 브라우저에 열어 둡니다. `BlogPosting`을 PUT하려면 해당 스키마가 필요합니다.
...
### OAuth2를 사용하여 서비스 호출하기

프로덕션의 경우 [OAuth2 애플리케이션](../using-oauth2/creating-oauth2-applications.md) 을 생성하고 OAuth2 프로세스를 사용하여 인증 토큰을 받습니다. 토큰을 받으면 HTTP 헤더에 토큰을 입력합니다:
...
위의 쿼리를 실행하여 모든 블로그 게시글을 가져오면 게시글이 없는 것을 확인할 수 있습니다:
...
1. 스키마 브라우저가 포함된 브라우저 탭으로 돌아갑니다. 오른쪽에서 `BlogPosting` 항목을 클릭하여 스키마를 표시합니다(위 참조). 여기에는 `블로그 포스팅`에 대한 전체 데이터 구조가 표시되지만 필수 필드는 두 개뿐입니다:

    * `articleBody`
    * `headline`

1. 블로그 항목을 게시하기 위한 간단한 JSON 문서를 작성합니다:

    ```json
    {
        "headline": "REST 서비스에서 블로그 항목 테스트",
...
이제 첫 번째 쿼리를 반복하여 게시한 블로그 항목이 있는지 확인할 수 있습니다:

```bash
curl "http://localhost:8080/o/headless-delivery/v1.0/sites/20122/blog-postings/" -u 'test@liferay.com:learn'
```

블로그 항목 목록을 반환합니다. 추가한 항목이 목록에 있는 유일한 항목입니다:
...
요청을 할 때마다 Liferay DXP는 다른 가능한 엔드포인트를 반환했습니다. 이 중 하나는 ID로 단일 블로그 항목을 가져오는 것입니다. 응모작의 ID를 알고 있으면 검색할 수 있습니다:

```bash
curl "http://localhost:8080/o/headless-delivery/v1.0/blog-postings/35215" -u test@liferay.com:learn
```

그러면 동일한 블로그 항목이 반환됩니다.

### 블로그 항목 삭제하기

해당 아이디를 알고 있다면 블로그 항목을 삭제할 수도 있습니다:

```bash
curl -X DELETE "http://localhost:8080/o/headless-delivery/v1.0/blog-postings/35215" -u test@liferay.com:learn
```

이 경우 아무것도 반환되지 않지만 위에서와 같이 요청하여 항목이 사라졌는지 확인할 수 있습니다:

```bash
curl "http://localhost:8080/o/headless-delivery/v1.0/blog-postings/35215" -u test@liferay.com:learn
```

그러면 Liferay DXP가 이 JSON 문서를 응답으로 반환합니다:
...
축하합니다! 이제 Liferay DXP의 REST 서비스를 호출하는 방법을 배웠습니다. 위의 예제에서는 기본 인증을 사용했습니다. 프로덕션 환경에서는 OAuth2를 사용하여 안전한 방식으로 서비스를 호출하세요.
