...
1. 템플릿의 이름을 **Goo** 로 업데이트하고 **Copy** 을 클릭합니다.

   ![템플릿의 이름을 "Goo"로 업데이트합니다.](./advanced-web-content-api/images/09.png)

1. 템플릿 탭에서 새 **Goo** 템플릿을 클릭합니다.
...
이전에 게시한 [이미지의 ID를 얻으려면](#adding-the-images-to-liferay-dxp) Documents **GET** FromSite.sh `스크립트를 사용`. 이 스크립트는 `GET` HTTP 메서드 및 [사이트 ID](#identify-the-site-id) 매개변수와 함께 `문서` 서비스를 사용합니다.
