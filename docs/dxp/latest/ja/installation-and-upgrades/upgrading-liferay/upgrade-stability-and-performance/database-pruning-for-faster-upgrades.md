...
* **Web コンテンツの中間バージョン:** Liferay DXP は、変更（翻訳を含む）後に新しい Web コンテンツのバージョンを生成します。 不要なバージョンは削除することを検討しよう。 特に、削除されたバージョンに固有の画像ファイルなどのオブジェクトがある場合は、かなりのスペースが空く可能性があります。 詳細は、 [Example: Removing Intermediate Journal Article Versions](./example-removing-intermediate-journal-article-versions.md) を参照してください。
