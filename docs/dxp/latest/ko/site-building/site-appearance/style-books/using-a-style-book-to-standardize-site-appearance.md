...
1. 페이지 상단 근처에 있는 *편집* (![Edit icon](../../../images/icon-edit.png))을 클릭하십시오.
...
![페이지 조각에 대한 Style Book의 효과를 미리 보는 경우 선택한 컬렉션에서 조각의 모든 구성을 미리 볼 수 있습니다.](./using-a-style-book-to-standardize-site-appearance/images/04.png)
...
```{tip}
색상과 관련이 없는 너비, 여백 및 안쪽 여백과 같은 옵션을 수정할 때 사전 정의된 단위 세트에서 선택하거나 사용자 지정 단위를 지정할 수 있습니다. 일부 옵션에는 픽셀(PX), 백분율(%), em 너비(EM) 및 루트 em(REM)이 포함됩니다.
```
...
![색상 시스템 범주 아래의 옵션을 사용하여 디스플레이 또는 UI 요소에 사용되는 색상을 변경합니다.](./using-a-style-book-to-standardize-site-appearance/images/08.png)

```{tip}
색상과 관련이 없는 너비, 여백 및 안쪽 여백과 같은 토큰을 수정할 때 사전 정의된 단위 세트에서 선택하거나 사용자 정의 단위를 지정할 수 있습니다. 이러한 유연성을 통해 스타일을 보다 정확하게 제어할 수 있습니다. 일부 옵션에는 픽셀(PX), 백분율(%), em 너비(EM) 및 루트 em(REM)이 포함됩니다.
```
...
이러한 옵션은 기본 유형의 버튼 스타일을 수정합니다. 클래식 테마에서 사용되는 각 버튼의 배경, 전경 및 테두리 색상을 사용자 정의할 수 있습니다.
