...
選択リストは、Liferayの権限フレームワークと統合されています。 つまり、 [アプリケーション](#application-permissions) と [リソース](#resource-permissions) の権限をユーザー・ロールに割り当てることで、Picklists アプリケーションとそのデータにアクセスできるユーザーを決定できます。
...
| 設定              | Picklistsアプリケーションの設定オプションの表示と変更 |
| 権限設定            | 選択リストの権限を表示および変更します             |
| 表示設定            | Picklistsアプリケーションの環境設定の表示と変更    |
| 表示              | ピックリストアプリケーションを見る               |
...
Picklistsアプリケーションには以下のリソース権限があります。
...
1. 希望するピックリストの **Actions** ボタン (![Actions Button](../../../images/icon-actions.png)) をクリックし、 **Permissions** を選択します。

   ![目的の選択リストのアクションボタンをクリックし、［権限］を選択します。](./managing-picklist-permissions/images/02.png)

1. チェックボックスを使用して、 [データベースエンティティ](#actions-on-database-entities) の権限を必要なロールに付与します。
