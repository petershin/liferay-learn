...
1. JDBCコネクタをインストールします。 DXP バンドルには、 `$CATALINA_BASE/webapps/ROOT/WEB-INF/shielded-container-lib` フォルダ（以前のバージョンの Liferay では `$CATALINA_BASE/lib/ext` フォルダ）に、いくつかのオープンソースの JDBC コネクタが含まれています。 OracleやDB2などの専用データベースのコネクタ（下の表を参照）は、ベンダーからダウンロードする必要があります。
