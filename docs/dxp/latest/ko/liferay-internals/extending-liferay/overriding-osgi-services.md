...
제공된 `api`는 `able.impl` 모듈에 의해 구현되는 OSGi 서비스 유형을 정의하며, 이는 제공된 `portlet`에서 사용됩니다. 이제 모든 것이 배포되었으므로 재정의 작동 방식을 실험할 수 있습니다.
...
1. (선택 사항) 기존 서비스의 `component.name` 참조하여 호출합니다.

샘플 `S1J6BakerImpl` 모듈은 `S1J6AbleImpl`재정의하기 위해 제공됩니다.
...
여기서 `S1J6BakerImpl` `S1J6AbleImpl` (즉, `com.acme.s1j6.S1J6`)과 동일한 서비스 유형을 구현하고 필요한 `@Component` 주석, `서비스` 속성 및 `service.ranking` 속성을 포함합니다. 또한 기존 서비스(예: `component.name=com.acme.s1j6.able.internal.S1J6AbleImpl`)를 참조하고 인터페이스 메서드 재정의의 일부로 위임합니다. 샘플 모듈에는 `S1J6AbleImpl` 및 `S1J6BakerImpl`재정의하기 위한 두 개의 다른 S1J6 구현도 포함되어 있습니다.
...
다음 단계에 따라 `S1J6BakerImpl`, `S1J6CharlieImpl`및 `S1J6DogImpl`배포합니다.
...
1. `S1J6BakerImpl` Gogo Shell을 통해 인스턴스에 성공적으로 배포되고 바인딩되었는지 확인합니다.

   ```shell
   scr:info com.acme.s1j6.web.internal.portlet.S1J6Portlet
   ```

   성공하면 `S1J6Portlet` `S1J6AbleImpl`보다 순위가 높기 때문에 `S1J6BakerImpl`에 바인딩됩니다.
...
1. `S1J6CharlieImpl` 및 `S1J6DogImpl` `s1j6-liferay`에서 Docker 컨테이너로 동시에 배포합니다.
...
1. `S1J6CharlieImpl` 및 `S1J6DogImpl` 모두 Gogo Shell을 통해 인스턴스에 성공적으로 배포되었는지 확인합니다.
