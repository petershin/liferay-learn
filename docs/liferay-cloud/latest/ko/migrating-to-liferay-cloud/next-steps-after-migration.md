...
기본적으로 Liferay Cloud는 동일한 지역 내에서 [자동 재해 복구](../troubleshooting/disaster-recovery-overview.md#automatic-disaster-recovery-strategy) 을 제공하여 서비스 중단으로 인한 가동 중지 시간을 완화합니다. 그러나 재해로 인해 Liferay 클라우드 환경을 서비스하는 영역에서 중단이 발생하는 경우 프로덕션 인스턴스에서 사용자의 가동 중지 시간을 최소화하기 위해 대체 환경(또는 재해 복구 환경)을 사용하는 것도 고려해야 합니다.

[재해 복구 환경을 구성](../troubleshooting/configuring-cross-region-disaster-recovery.md)하여 계획되지 않은 다운타임으로부터 가능한 가장 강력한 보호를 유지합니다. 자동 및 지역 간 재해 복구에 대해 자세히 알아보세요[여기](../troubleshooting/disaster-recovery-overview.md).
