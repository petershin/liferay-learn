...
**Identifier：** 解像度のID。 デフォルトでは、名前から自動的に生成されます。 _［Custom］_オプションを選択し、新しい_ID_を入力することにより、カスタム識別子を指定できます。 サードパーティのアプリケーションは、このIDを使用して、アダプティブメディアのAPIを介して解像度の画像を取得できます。
