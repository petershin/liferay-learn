...
アップグレードのステップが多く複雑な場合は、 `getPreUpgradeSteps()` と `getPostUpgradeSteps()` メソッドを、 `UpgradeProcess` クラスで使用することを検討してください。 このアプローチは、アップグレードプロセスをよりコントロールしやすくします。 各アップグレード・ステップには、デバッグを容易にするために、 `Release_` テーブルで異なるスキーマ・バージョンが与えられます。 ステップに失敗し、アップグレードを再実行しなければならない場合、アップグレード処理は自動的にチェックし、最新の失敗ポイントからアップグレードを再開します。
...
この例では、 `getPreUpgradeSteps()` が最初に実行され、新しいカラムを追加する単純なステップを含む。 `doUpgrade()` メソッドでは、新しいカラムの入力を行う。 `doUpgrade()` メソッドが失敗した場合、開発者はデバッグして必要な変更を加え、アップグレードを再実行できます。 プロセスは、最初のステップが成功したことを認識し、自動的に次のステップに進む。

アップグレードステップを利用するには、 `com.liferay.portal.kernel.upgrade.UpgradeStep` クラスをアップグレードプロセスにインポートしてください。
