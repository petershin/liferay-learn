...
Liferay Cloud は、ディザスタリカバリのために 2 つの戦略を提供します。自動とクロスリージョンです。

## 自動ディザスタ リカバリ戦略

Liferay Cloudは、同一地域内の異なる場所にある3つのゾーンにサービスインスタンスを分散させ、1つのゾーンがダウンした場合は別のゾーンでサービスを再開するという自動ディザスターリカバリープロトコルを実行します。 ゾーンのいずれかが利用できなくなった場合、ロードバランサーは、顧客側でDNSを変更する必要なく、残りのゾーンに自動的にルーティングします。 この状況では、 **インシデント中にお客様からのアクションは必要ありません** 。
...
Liferay Cloud の DR 戦略は、 **Warm** です。 Liferay Cloudのユーザーは、災害時の対応についてサポートに連絡する必要はありませんが、提供されるツールを使用するため、地域をまたぐ災害時には行動することが求められます。

## ディザスタリカバリのシナリオ例

ここでは、架空の災害シナリオを想定して、Liferay Cloud Disaster Recoveryのツールやアプローチがどのように機能するのかを例示します。
...
| 時間      | イベント                                                                      |
|:------- |:------------------------------------------------------------------------- |
| 午後4時00分 | チームに **europe-west2** リージョンが回復し、インシデントが終了したことが通知されます。 チームは、元の本番環境に移行する準備をします。 |
