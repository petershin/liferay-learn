...
Read the [Elasticsearch documentation](https://www.elastic.co/guide/en/elasticsearch/reference/8.13/security-settings.html#transport-tls-ssl-settings) to learn more about the SSL verification mode setting.
```
...
Elasticsearch 8 uses the bundled OpenJDK 19 to generate certificates and private keys with the [`elasticsearch-certutil`](https://www.elastic.co/guide/en/elasticsearch/reference/8.13/certutil.html) command. When Liferay runs on JDK 8, it cannot recognize certificates generated with JDK 16 or higher.
