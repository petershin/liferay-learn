...
[`[Liferay Home]`](../../reference/liferay-home.md): JBoss 서버 폴더가 포함된 폴더( `$JBOSS_HOME`이라고 함). DXP 설치 및 배포 후 `data`, `deploy`, `logs` 폴더를 생성합니다.

`$JBOSS_HOME`: JBoss 서버 폴더. 일반적으로 이름은 `jboss-eap-[version]`입니다.

## DXP WAR 설치

1. 깨끗한 JBoss 설치로 시작하고 `$JBOSS_HOME/standalone/deployments/ROOT.war` 폴더가 존재하는 경우 해당 하위 폴더와 파일을 모두 삭제합니다.
1. DXP WAR 파일을 `$JBOSS_HOME/standalone/deployments/ROOT.war` 폴더에 압축 해제합니다(이 폴더가 없으면 생성).

```{important}
WAR 파일 이름을 변경하여 기본 Liferay Portal 웹 컨텍스트를 변경할 수 있지만(예: `localhost:8080/`에서 `localhost:8080/myportal`로) 이는 권장되지 않습니다.
```
...
1. 종속성 ZIP 파일을 `$JBOSS_HOME/modules/com/liferay/portal/main` 폴더에 압축 해제합니다(이 폴더가 없으면 생성).
1. `$JBOSS_HOME/modules/com/liferay/portal/main` 폴더에 `module.xml` 이라는 파일을 만듭니다. 파일에서 포털 모듈과 필요한 모든 리소스 및 종속성을 선언합니다.
...
```{note}
이것은 DXP가 여러 JBoss 서버의 클러스터 환경에서 실행되는 것을 막지 않습니다. 독립 실행형 모드에서 실행되는 JBoss EAP 서버에서 실행되는 DXP 인스턴스의 클러스터를 설정할 수 있습니다. 자세한 내용은 [클러스터링 문서](../../setting-up-liferay/clustering-for-high-availability.md)를 참조하십시오.
```
...
`$JBOSS_HOME/standalone/configuration/standalone.xml`를 다음과 같이 수정합니다.
...
1. 닫는 `</extensions>` 태그를 찾습니다. 닫는 태그 바로 아래에 다음 시스템 속성이 아직 없으면 삽입합니다.
...
`$JBOSS_HOME/bin/` 폴더에서 독립 실행형 도메인의 구성 스크립트 파일 `standalone.conf`를 수정합니다.
...
JBoss 서버와 함께 IBM JDK를 사용하는 경우 다음 추가 단계를 완료하십시오.

1. DXP 7.3 및 이전 버전의 경우 `$JBOSS_HOME/modules/com/liferay/portal/main/module.xml` 파일로 이동하여 `<dependencies>` 요소 내에 이 종속성을 삽입합니다.

    `<module name="ibm.jdk" />`

1. `$JBOSS_HOME/modules/system/layers/base/sun/jdk/main/module.xml` 파일로 이동하여 `<paths>...</paths>` 요소 내부에 다음 경로를 삽입합니다.
...
추가된 경로는 포털 배포 예외 및 이미지 업로드 문제와 관련된 문제를 해결합니다.
...
## JBoss EAP의 데이터 소스 구성

JBoss를 사용하여 데이터 소스를 관리하는 경우 다음 단계를 따르십시오.

1. DXP WAR(7.4+) 또는 데이터베이스 공급업체에서 JDBC JAR을 가져와 `$JBOSS_HOME/modules/com/liferay/portal/main` 폴더에 복사합니다.

1. `$JBOSS_HOME/modules/com/liferay/portal/main` 폴더에 `module.xml` 이라는 파일을 만듭니다. 파일에서 포털 모듈 및 JDBC JAR을 선언합니다.
...
1. `$JBOSS_HOME/standalone/configuration/standalone.xml` 파일의 `<datasources>` 요소 내부에 데이터 소스를 추가합니다.
...
1. `<datasources>` 요소 내에 있는 `standalone.xml` 파일의 `<drivers>` 요소에 드라이버를 추가합니다.
...
1. Liferay Home의 [`Portal-ext.properties`](../../reference/portal-properties.md) 파일에서 데이터 소스를 지정합니다. 예를 들어,

    ```properties
    jdbc.default.jndi.name=java:jboss/datasources/ExampleDS
    ```

이제 데이터 소스가 구성되었으며 데이터베이스에 연결할 준비가 되었습니다.

## 메일 서버에 연결

데이터베이스 구성과 마찬가지로 구성하기 가장 쉬운 메일 세션은 DXP입니다. DXP의 내장 메일 세션을 사용하려면 이 섹션을 건너뛰고 [제어판에서 메일 서버](../../setting-up-liferay/configuring-mail.md) 에 연결합니다.

JBoss에서 메일 세션을 구성하려면 다음 단계를 따르십시오.

1. 다음과 같이 `$JBOSS_HOME/standalone/configuration/standalone.xml` 파일에서 메일 하위 시스템을 지정합니다.
