...
샘플 프로젝트에서 `providerPopulatedColors` 채울 구성 필드 이름입니다.
...
`@Component` 주석을 사용하여 서비스를 등록하십시오. 이전 단계의 `configuration.field.name` 포함합니다. `configuration.pid` 을 구성 인터페이스의 정규화된 클래스 이름으로 설정합니다.
...
`getOptions` 메서드를 추가하여 `Option`목록을 반환합니다. 샘플 프로젝트에는 `optionValue` 색상의 문자열로 설정하고 `optionLabel` 해당 색상의 `Langauge.properties` 파일에 저장된 문자열로 설정하는 배열이 포함되어 있습니다.
