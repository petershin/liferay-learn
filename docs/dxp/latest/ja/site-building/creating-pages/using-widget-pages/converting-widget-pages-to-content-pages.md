...
以前のバージョンでは、ウィジェットページはカスタムレイアウトやカスタマイズ可能な列などの排他的な機能を提供していました。 これらの専用機能の多くは、7.3以降の [コンテンツページ](../../using-content-pages.md) で利用できるため、ほとんどの場合、コンテンツページを作成することになるでしょう。

旧バージョンからLiferay Portal 7.3または7.4にアップグレードし、既存のWidget Pagesを移行する場合、それらをContent Pagesに変換することができます。 ウィジェットページをすぐにコンテントページに変換することも、変換をプレビューして変更を加えてからページを変換することもできます。

変換するページが複数ある場合は、サイト管理から一括変換するか、利用可能なAPIと内蔵のスクリプトエディタを使用して、すべてのウィジェットページを一度に変換することが可能です。 ここでは、すべてのアプローチについて説明します。
...
1. 警告を確認し、変換下書きに必要な調整を行います。 また、この時点でドラフトに好きなフラグメントを追加することができます。
...
1. ウィジェットページ、または複数のウィジェットページのチェックボックスをオンにします。 管理ツールバーの「**アクション**」 メニューを開き、「**コンテンツページに変換する**」 オプションを選択します。
...
内蔵のスクリプトエディターで、サイト上のすべてのウィジェットページをコンテンツページに一括変換することができます。
