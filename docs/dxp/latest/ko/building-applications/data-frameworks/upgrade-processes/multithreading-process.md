...
1. 다음 스크립트를 실행하여 일부 항목을 추가합니다.
   ...
   이제 J7Z3_J7Z3Entry 테이블에 세 개의 항목이 있습니다.

   ![세 항목은 데이터베이스 테이블에서 확인할 수 있습니다.](./multithreading-process/images/01.png)

## 업그레이드 실행

1. `1.0.1` 디렉토리로 이동하여 빌드하고 배포합니다.
...
1. `upgrade:execute com.acme.j7z3.service`명령을 입력하여 업그레이드를 실행합니다. 출력 창에 업그레이드가 완료되었음을 표시합니다.
   ...
   ![업그레이드를 실행하면 출력에 업그레이드가 완료되었음을 표시해야 합니다.](./multithreading-process/images/02.png)
...
샘플 프로젝트는 `이름` 필드가 `processConcurrently()` 메서드에 의해 처리되고 수정되는 간단한 예를 보여줍니다.

1. `select j7z3EntryId, name from J7Z3_J7Z3Entry` - SQL 문이 모든 항목을 쿼리합니다.
...
`processConcurrently()` 메서드에는 두 개의 서로 다른 서명이 있습니다. 하나의 서명은 이 자습서 예제에서 볼 수 있듯이 SQL 쿼리를 소스로 전달합니다. 다른 서명은 배열을 소스로 전달합니다. 자세한 내용은 [BaseDBProcess javadocs](https://learn.liferay.com/reference/latest/en/dxp/javadocs/portal-kernel/com/liferay/portal/kernel/dao/db/BaseDBProcess.html#processConcurrently-java.lang.String-com.liferay.petra.function.UnsafeFunction-com.liferay.petra.function.UnsafeConsumer-java.lang.String-) 을 참조하십시오.
