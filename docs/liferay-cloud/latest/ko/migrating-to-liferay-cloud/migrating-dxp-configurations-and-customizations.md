...
먼저 마이그레이션하려는 온프레미스 설치에서 Liferay 구성 파일을 구성합니다. 여기에는 사용 중인 [개의 포털 속성 파일](../using-the-liferay-dxp-service/configuring-the-liferay-dxp-service.md#portal-properties) 이 포함됩니다.
...
포털 속성 <a href="../using-the-liferay-dxp-service/configuring-the-liferay-dxp-service.md#portal-properties">파일(예:</a> portal-ext.properties`) 각각의 적절한 환경 폴더에 넣습니다.

[OSGi 구성](../using-the-liferay-dxp-service/configuring-the-liferay-dxp-service.md#osgi-configurations) 파일(예: `.cfg` 또는 `.config` 파일)을 `osgi/`이라는 적절한 환경 폴더의 하위 폴더에 넣습니다.
...
온프레미스 Liferay 설치에 사용되는 사용자 정의 OSGi 모듈이 있는 경우 모든 [OSGi 모듈 소스 코드](https://learn.liferay.com/dxp/latest/en/liferay-internals/extending-liferay/overriding-osgi-services.html) 을 적절한 환경의 `liferay/modules/{ENV}/` 폴더(동일한 작업 공간 폴더 구조 포함)에 넣습니다.

변경 사항이 배포되면 사용자 정의 모듈 코드가 자동으로 컴파일된 다음 `liferay` 서비스 컨테이너의 `$LIFERAY_HOME/deploy/` 폴더에 배포됩니다.

## 사용자 지정 테마 구성

다음으로 모든 [사용자 지정 테마](https://learn.liferay.com/dxp/latest/en/site-building/site-appearance/themes/theme-development/getting-started/setting-up-an-environment-and-creating-a-theme.html) 의 소스 코드를 해당 환경의 `liferay/themes/{ENV}/` 폴더에 넣습니다.

변경 사항이 배포되면 사용자 지정 테마가 자동으로 빌드되어 `liferay` 서비스 컨테이너의 `$LIFERAY_HOME/deploy/` 폴더에 배포됩니다.

## WAR 파일 구성

다음으로 Liferay 설치의 [WAR 파일](https://learn.liferay.com/dxp/latest/en/building-applications/reference/deploying-wars-wab-generator.html) 을 모두 해당 환경의 `liferay/wars/{ENV}/` 폴더에 넣습니다.
