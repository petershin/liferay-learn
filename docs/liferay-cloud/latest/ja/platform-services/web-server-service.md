...
```{note}
バージョン3.x.xのサービスを使用している場合、webserverスクリプトは適切な`lcp/webserver/script/{ENV}/`ディレクトリに置かれます。 バージョン確認の詳細については、[サービススタックのバージョンについて](../reference/understanding-service-stack-versions.md) を参照してください。
```
