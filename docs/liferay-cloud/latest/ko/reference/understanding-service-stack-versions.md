...
모든 Liferay Cloud 환경은 서비스의 Docker 이미지 버전을 다양한 사이트에 표시합니다. 환경의 _개요_ 또는 _서비스_ 페이지 또는 각 개별 서비스의 페이지를 포함하여 이러한 사이트 중 하나에서 표시된 이미지 이름 끝에 있는 번호를 검사하여 환경의 서비스 스택 버전을 확인할 수 있습니다.
