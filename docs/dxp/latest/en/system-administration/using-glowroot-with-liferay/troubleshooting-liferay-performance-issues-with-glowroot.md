...
Liferay utilizes [Ehcache](https://www.ehcache.org/) for most of its entities. Use the Mbean tree to inspect the configuration and utilization of specific entities. For example, inspect the entity cache for `UserImpl`.
...
Note, it is also possible to clear cache. In Liferay, navigate to _Control Panel_ &rarr; _Server Administration_. Cache clearing is available under the Resources tab.

## Custom SQL Queries

As custom queries are not cached, they can sometimes require long transaction times. See the [expensive SQL queries](#expensive-sql-queries) above to see the impact of custom queries on your system.

## Large Tables

Over time, your Liferay database tables may grow to become quite large. This may also cause transaction slowdown. Some examples are:
...
* _CPU time_ - the time spent computing
...
Use the Glowroot gauges to monitor garbage collection time and to check for any unusually long times. Navigate to the _JVM_ tab and click _Gauges_ in the left menu. Scroll down and select the garbage collector `CollectionTime` attribute to be displayed.
...
Metaspace size is set when configuring your application server. A crash can occur when the JVM tries to claim more than what is configured. Use the Glowroot gauges to inspect the metaspace usage. Navigate to the _JVM_ tab and click _Gauges_ in the left menu. Scroll down and select the garbage collector `CollectionTime` attribute. Scroll down and select the java `Metaspace` attribute to be displayed.
...
1. Input the class name and method name. Select what to capture and fill in the details based on your specific needs.

1. Click _Add_. The new transaction is available to view under the _Transactions_ tab.

See [Glowroot instrumentation](https://glowroot.org/instrumentation.html) for more information.

## False Alarms

Slow transactions may be an indication of potential system errors. However, there are some scenarios in which it may be expected. For example, when the Liferay server first starts up, you might see large SQL queries that take a long time. Slow transactions might also happen after a long idle period in which the cache is empty. JSP requests might also show up as slow transactions. Unlike Java, JSPs are compiled at runtime and a large number of JSP requests may cause a slowdown.
