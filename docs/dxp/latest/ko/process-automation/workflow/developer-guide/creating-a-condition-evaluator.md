...
Acme R6J9 구현 프로젝트에 의해 자동 로드된 R6J9 조건부 승인자 워크플로 정의는 Liferay DXP/Portal과 함께 제공되는 단일 승인자 정의와 거의 동일합니다. 첫 번째 차이점은 `생성` 상태 노드에 있습니다. 단일 승인자 정의는 항상 검토 전환을 실행하지만 R6J9 조건부 승인자는 `확인-역할` 전환을 실행합니다. 주요 차이점은 **조건 노드** 이라는 동일한 이름의 추가입니다. 그런 다음 조건 노드는 `R6J9ConditionEvaluator` 에 의존하여 조건 논리를 실행합니다.
