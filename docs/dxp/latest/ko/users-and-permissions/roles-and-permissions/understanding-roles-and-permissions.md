...
Liferay DXP에서 작업을 수행하려면 [사용자](./../users/understanding-users.md) 은 적절한 권한이 있어야 합니다. 역할은 사용자에게 권한을 부여합니다. 역할 및 권한에 대한 대부분의 관리 작업은 *제어판* &rarr; *사용자* &rarr; *역할*에서 수행됩니다.
...
예를 들어 _Message Boards Admin_ 역할 및 사용자 _Jane Slaughter_ 을 사용하여 수행할 수 있는 단계는 다음과 같습니다.

1. 필요한 기능 결정: _Liferay DXP에서 게시판 관리._
1. 범위 결정: _가상 인스턴스 전체(전역)._
1. 적절하게 범위가 지정된 역할을 생성하여 기능을 강화하는 권한을 할당합니다: _Message Boards Admin(일반 역할)._
1. 사용자 컬렉션 만들기: _Message Boards Administrators라는 사용자 그룹._
1. 사용자 컬렉션에 역할 할당: _Message Boards Administrators &rarr; Message Boards Admin Role._
1. 사용자 컬렉션에 사용자 할당: _Jane Slaughter &rarr; Message Boards Administrators._
...
개별 [사용자](./../users/understanding-users.md) 을 역할에 수동으로 할당할 수 있습니다. 이 방법은 사용자 컬렉션을 사용하는 것보다 덜 효율적입니다. 사용자는 기본 사용자 연결이라는 가상 인스턴스 [을 통해 모든 범위의 역할에 자동](../../system-administration/virtual-instances/configuring-a-virtual-instance-users.md#default-user-associations) 할당될 수도 있습니다.
...
* [세그먼트](./../../site-building/personalizing-site-experience/segmentation/creating-and-managing-user-segments.md) 은 특정 조건과 일치하는 사이트 사용자를 보유합니다.
...
사이트 범위 역할을 사이트 역할이라고 합니다. 사이트 역할에 대한 권한은 전역 수준에서 정의되며 하나의 특정 [사이트](../../site-building/introduction-to-site-building.md)에 적용됩니다. 개별 사용자, 조직 및 사용자 그룹을 사용하여 사이트 구성원을 제어하고 사이트 역할을 할당할 수 있습니다. 
...
Liferay DXP의 페이지에 있는 각 위젯 인스턴스와 사이트 메뉴 &rarr; 콘텐츠 및 데이터 섹션의 각 관리 응용 프로그램에 대한 권한을 부여할 수 있습니다. 자세한 내용은 [위젯 권한](./../../site-building/widget-permissions.md) 을 참조하십시오.

### 자산 범위 권한

자산 수준 권한(예: 개별 블로그 게시물을 편집하거나 문서 및 미디어 라이브러리의 폴더를 볼 수 있는 권한)은 제어판이 아닌 개별 자산에서 관리됩니다. 자세한 내용은 [위젯 권한](./../../site-building/widget-permissions.md) 을 참조하십시오.
