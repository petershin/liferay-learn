...
**Segmented Downloads(セグメント化されたダウンロード）：** 1つまたは複数のアナリティクスクラウドセグメントに属する、または属さない既知の個人の割合を示すドーナツチャートです。 中央は、Known Individualsの総数を示しています。 ツールチップには、そのセクションの既知個体数(例：1つまたは複数のセグメントに属する既知個体の数）が表示されます。
