...
**Semantic** 검색은 검색된 구 뒤에 있는 의도를 평가합니다. 한편, **키워드** 검색은 검색된 구문을 인덱스 텍스트 필드에 일치시키고 의미에 따라 일치를 생성할 수 없습니다.
...
예를 들어 사용자가 **검색하면 스케이트는 어떻게 움직일까요?** 수 있듯이 해양 생물학에 대해 훈련된 모델은 레크리에이션에 대해 훈련된 모델과 다른 결과를 제공합니다.
...
텍스트 임베딩을 색인화하려면 Index Actions 화면을 사용하고 Semantic Search System 또는 Instance Settings의 Asset Entry Class Names 설정에서 활성화하려는 모델 유형에 대해서만 **Reindex** 버튼을 클릭합니다.
...
**언어:** 변환할 언어와 현지화를 선택합니다. 기본적으로 나열된 모든 언어가 선택됩니다. **Ctrl + 클릭** 을 사용하여 목록에서 여러 언어를 선택합니다.
