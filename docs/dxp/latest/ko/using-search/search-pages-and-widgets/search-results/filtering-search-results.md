...
| 사용 사례                                                               | `구성` (**값**) |
|:------------------------------------------------------------------- |:--------------------------------------------------------------------------------------------------------------------------- |
| **사용자 제목 필터:** 최종 사용자는 필터 키를 직접 조작하여 제목을 사용하여 원하는 결과로 결과를 구체화합니다.   | `Filter Field` _(title\_en\_US)_ <br /> `Filter Value` _(podcast)_ <br /> `Filter Query Type` _(Match)_ <br /> `Occur Type` _(must_not)_ |
| **하드 코딩된 필터링:** 관리 사용자가 최종 사용자에게 보이지 않고 변경할 수 없는 필터로 검색 페이지를 구성합니다. | **_Add to the above configuration_** <br /> `Invisible` _(True--checked)_ <br /> `Immutable` _(True--checked)_ |
