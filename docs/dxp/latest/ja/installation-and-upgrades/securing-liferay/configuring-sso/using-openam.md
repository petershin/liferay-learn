...
|:--------------- |:------------------ |:---------------------------------------------------------------------------------------------------------- |:------ |
| **バージョン** | `バージョン`            | 使用するOpenAMバージョン（12以下または13）                                                                                 | `文字列`  |
| **有効** | `有効`               | OpenAM認証を有効にするには、このボックスをオンにします。 OpenAMは、LDAP認証も有効になっていて、Liferay DXPの認証タイプが画面名に設定されている場合にのみ機能することに注意してください。 | `ブール値` |
| **LDAPからインポート** | `importFromLDAP`   | これをオンにすると、Liferay DXPに存在しないOpenAMから認証されたユーザがLDAPからインポートされます。 LDAPを有効にする必要があります。                            | `ブール値` |
| **ログインURL** | `loginURL`         | OpenAMサーバーのログインページへのURL                                                                                    | `文字列`  |
| **ログアウトURL** | `logoutURL`        | OpenAMサーバーのログアウトページへのURL                                                                                   | `文字列`  |
| **サービスURL** | `serviceURL`       | 認証されたWebサービスを使用するためにOpenAMにアクセスできるURL。 OpenAM Express 8以降を使用している場合は、サーバーでJava 6を実行する必要があります。               | `文字列`  |
| **スクリーン名の属性** | `screenNameAttr`   | ユーザーの画面名を表すOpenAM上の属性の名前                                                                                   | `文字列`  |
| **メールアドレスの属性** | `emailAddressAttr` | ユーザーの電子メールアドレスを表すOpenAM上の属性の名前                                                                             | `文字列`  |
| **名前属性(名**) | `firstNameAttr`    | ユーザーの名を表すOpenAM上の属性の名前                                                                                     | `文字列`  |
| **名前属性(姓**) | `lastNameAttr`     | ユーザーの姓を表すOpenAM上の属性の名前                                                                                     | `文字列`  |
