...
```{note}
기본 데이터 스토리지(데이터베이스가 지원하지 않는 데이터 저장)에 사용되는 Elasticsearch 인덱스가 있는 경우 [스냅샷 및 복원 방식](./upgrading-elasticsearch/backing-up-elasticsearch.md) 을 사용하여 해당 데이터를 새 Elasticsearch 클러스터로 가져올 수 있습니다. Liferay 자체 검색 튜닝 인덱스(결과 순위 및 동의어용)는 Liferay DXP 7.2 및 7.3의 기본 스토리지 인덱스입니다.
```
...
Elasticsearch 8의 경우 Liferay와 Elasticsearch 사이에 방화벽이나 프록시가 있는 경우 각 요청에서 Liferay가 설정하는 호환성 HTTP 헤더가 네트워크 트래픽에서 허용되고 보존되는지 확인하십시오. 자세한 내용은 [Elasticsearch의 REST API 호환성 요청](https://www.elastic.co/guide/en/elasticsearch/reference/current/rest-api-compatibility.html#request-rest-api-compatibility) 참조하십시오.
...
- [부트스트랩 검사, 개발 및 프로덕션 모드](https://www.elastic.co/guide/en/ elasticsearch/reference/8.8/bootstrap-checks.html)
```
...
Elasticsearch [부트스트랩 검사](https://www.elastic.co/guide/en/elasticsearch/reference/8.8/bootstrap-checks.html) 시작 시 구성을 검사하고 누락되거나 의심스러운 구성에 대한 경고를 기록합니다. 프로덕션 환경에서는 잘못 구성된 경우 시작을 중지하도록 부트스트랩 검사를 구성해야 합니다.
...
```{tip}
로그에 'Elasticsearch가 예기치 않게 종료됨' 오류 메시지와 함께 Elasticsearch가 시작되지 않으면 힙 크기를 늘립니다. 테스트하는 동안 `config/jvm.options` 파일을 열고 `-Xms4g` 및 `-Xmx4g`에 대한 예제 설정의 주석을 제거할 수 있습니다. 프로덕션 환경에서는 해당 파일의 지침에 따라 이러한 매개변수를 올바르게 설정하십시오.
