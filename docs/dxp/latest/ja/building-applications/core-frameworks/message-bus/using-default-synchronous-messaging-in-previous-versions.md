...
1. `m4q7:sendMessage`のGogoシェルコマンドを呼び出し、メッセージを渡します。
1. `M4Q7BakerOSGiCommands` の `sendMessage(String)` メソッドが Gogo シェルコマンドをトリガーし、コマンドの引数をメッセージとして `acme/m4q7_able` に送信します。
1. メッセージバスのスレッドが `M4Q7CharlieMessageListener` にメッセージを送信する。
1. `M4Q7CharlieMessageListener` はメッセージのペイロードを記録し、元のメッセージの応答先である `acme/m4q7_baker` に自身のクラス名を応答メッセージとして送信する。
1. `M4Q7BakerMessageListener` は応答メッセージを受信し、そのペイロードを記録する。
1. 処理は `M4Q7BakerOSGiCommands` に戻り、そこで元のメッセージに対する応答を記録する。
