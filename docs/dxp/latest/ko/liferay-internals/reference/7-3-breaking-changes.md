...
`<liferay-ui:icon iconCssClass="icon-user">` `<liferay-ui:icon icon="user" markupView="lexicon" />`로 바꾸기
...
jsp에서 아이콘을 직접 사용하는 개발자는 위에서 설명한 대로 `liferay-ui:icon` 태그를 사용하거나 `clay:icon` 태그를 대신 사용하여 svg 기반 아이콘을 생성할 수 있습니다.
...
이는 `liferay.frontend.ProgressBar`에 의존하는 모든 코드에 영향을 미칩니다. 이는 일반적으로 `soy` 통해 `{call liferay.frontend.ProgressBar /}`로 수행됩니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

`liferay.frontend.ProgressBar` 구성 요소를 직접 대체할 수는 없습니다. 이에 의존하는 구성 요소가 있는 경우 이전 구현의 복사본을 같은 사이트에 두고 이를 모듈 내에서 로컬로 사용할 수 있습니다.
...
`G_P_N_V` 로 끝나는 Finder 메소드가 `P_N_V`으로 끝나는 메소드로 대체되었습니다.
...
예를 들어, `Category.getLeftCategoryId()` 및 `Category.getRightCategoryId()`통해 카테고리 ID로 작업하는 대신 `Category.getTreePath()`를 통해 트리 경로를 가져올 수 있습니다. 그런 다음 트리 경로를 사용하십시오.
...
`` 메서드 호출이 여전히 필요한지 확인하려면 기존 코드를 다시 평가하는 것이 좋습니다.
...
이는 `liferay.frontend.Slider`에 의존하는 모든 코드에 영향을 미칩니다. 이것은 일반적으로 `soy` `{call liferay.frontend.Slider /}`로 수행됩니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

`liferay.frontend.Slider` 구성 요소를 직접 대체할 수는 없습니다. 이에 의존하는 구성 요소가 있는 경우 이전 구현의 복사본을 같은 사이트에 두고 이를 모듈 내에서 로컬로 사용할 수 있습니다.
...
Java 클래스 `com.liferay.asset.taglib.servlet.taglib.soy.AssetTagsSelectorTag` 제거되었습니다.
...
`Asset:asset-tags-selector` 및 해당 구성 요소가 React로 마이그레이션되어 이전 태그와 해당 인프라가 불필요해졌습니다.
...
이는 사용자 그룹 페이지를 사용자 개인 사이트로 복사하기 위해 `user.groups.copy.layouts.to.user.personal.site` 속성을 `true` 으로 설정한 모든 사람에게 영향을 미칩니다.
...
`com.liferay.portal.upgrade.internal.configuration.ReleaseManagerConfiguration.config` 에 정의된 OSGi 속성 `autoUpgrade` 포털 속성 `업그레이드.database.auto.run`로 대체되었습니다.
...
이 속성은 `Portal-developer.properties`에서 `true` 로 설정됩니다.

#### 누가 영향을 받나요?

이 변경 사항은 서버 시작 또는 모듈 배포 시 자동으로 업그레이드를 실행하려는 모든 환경에 영향을 미칩니다. `Upgrade.database.auto.run` `true` 으로 설정하는 것은 프로덕션 환경에서 권장되지 않습니다. 그러나 서버 시작 시 업그레이드해야 하는 경우 먼저 Liferay 데이터베이스와 파일 저장소(문서 라이브러리)를 백업하십시오.

`Upgrade.database.auto.run` `false` (기본값)으로 설정했지만 데이터베이스 업그레이드가 필요한 경우 Liferay는 필요한 업그레이드에 대한 정보를 인쇄하고 시작을 중단합니다. 데이터베이스 업그레이드는 일반적으로 주요/부 Liferay 릴리스에 필요하며 초기 CE Portal GA 릴리스 및 특정 서비스 팩(예외적인 경우)에 필요할 수 있습니다. 수정 팩에는 데이터베이스 업그레이드가 필요하지 않습니다. 시작 시 Liferay는 보류 중인 미세 변경 사항에 대한 정보를 인쇄합니다. 언제든지 Gogo Shell 콘솔과 릴리스 노트를 사용하여 이러한 변경 사항을 확인한 다음 실행 여부를 결정할 수 있습니다.
...
시그니처 `String, Class, GroupedModel, String, Object, UriInfo` 또는 `String, Class, Long, String, String, Object, Long, UriInfo`와 함께 `addAction` 메소드를 사용하십시오.
...
- 이제 제어 메뉴는 `사이트:고정` 사용하여 동작을 제어합니다.
- 새로운 고정 동작을 설명하기 위해 컨트롤 메뉴 내부의 메뉴 스타일이 업데이트되었습니다.

#### 누가 영향을 받나요?

이는 사용자 정의된 `portlet.ftl` 템플릿이 있는 사용자 정의 제어판 테마를 가진 개발자 또는 고정 막대로 작동하고 `*ControlMenuEntry` API를 사용하여 포함된 사용자 정의 메뉴를 개발한 개발자에게 영향을 미칠 수 있습니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

##### 제어판 테마

사용자 정의 제어판 테마를 사용하는 개발자는 호출(있는 경우)을 자신의 `portlet.ftl`포틀릿 섹션 위의 `@liferay.control_menu` 매크로로 이동해야 합니다.
...
- 컨트롤 메뉴 전: `com.liferay.product.navigation.taglib#/page.jsp#pre`사용
- 컨트롤 메뉴 뒤: `com.liferay.product.navigation.taglib#/page.jsp#post`사용

#### 왜 이렇게 변경되었나요?

이러한 변경은 접근성을 향상시키고 상단 메뉴 사이트 지정 및 제어에 필요한 로직을 단순화하기 위해 이루어졌습니다. 이는 일반적인 시각적 결함을 방지하는 보다 정확하고 예상되는 마크업을 제공합니다.
...
이전에는 `jQuery` 기본적으로 모든 페이지에 포함되었으며 전역 `window.$` 및 범위가 지정된 `AUI.$` 변수를 통해 사용할 수 있었습니다. 이 변경 후에는 jQuery `` 더 이상 기본적으로 포함되지 않으며 해당 변수는 `undefine`입니다.

#### 누가 영향을 받나요?

이는 사용자 정의 스크립트에서 `AUI.$` 또는 `window.$` 사용한 모든 개발자에게 영향을 미칩니다.
...
접두사가 `인 레이아웃.parallel.render` 있는 속성이 제거되었습니다. 이는 AJAX 렌더링이 활성화된 경우에만 병렬 렌더링이 지원된다는 의미입니다.
...
속성 파일에서 접두사가 `인layout.parallel.render` 인 속성을 모두 제거합니다.
...
Headless Delivery API에서는 ContentField 스키마 내부의 속성 이름 `값` 이 `contentFieldValue`으로 변경되었습니다.

#### 누가 영향을 받나요?

이는 ContentField `값` 속성 이름에 따라 REST 클라이언트에 영향을 미칩니다.
...
이 변경으로 인해 Headless API의 모든 값 속성 이름 `{schemaName}+Value`과의 일관성이 복원되었습니다.
...
`liferay-editor-image-uploader` AUI 플러그인이 제거되었습니다. 해당 코드는 Alloy Editor 및 CKEditor에서 사용되는 `addimages` CKEditor 플러그인으로 병합되었습니다.
...
`liferay-editor-image-uploader` 플러그인을 직접 대체할 수는 없습니다. 이에 의존하는 구성 요소가 있는 경우 이전 구현의 복사본을 같은 사이트에 두고 이를 모듈 내에서 로컬로 사용할 수 있습니다.
...
- `com.liferay.frontend.editor.tinymce.web 검색`.
...
이는 Liferay Front-end Editor Simple Web 모듈을 사용하는 모든 사람에게 영향을 미칩니다.
...
- 구성을 유지하세요.
- 브라우저에서 https://repository.liferay.com/nexus/index.html을 엽니다.
- `com.liferay.frontend.editor.simple.web 검색`.
...
`Asset.vocabulary.default` 은 이제 언어 키이며 더 이상 `Topic`의 고정 값을 갖지 않습니다.
...
이는 `Liferay.Poller`에 의존하는 모든 코드에 영향을 미칩니다. 이는 일반적으로 JSP에서 `Liferay.Poller.init()` 호출을 통해 수행됩니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

`Liferay.Poller` 유틸리티를 직접 대체할 수 있는 유틸리티는 없습니다. `Liferay.Poller`초기화해야 하는 경우 아래 코드를 사용하도록 JSP를 업데이트하세요.
...
`Liferay.Poller` 구성 요소는 보관된 Chat 애플리케이션에서만 사용되었습니다. 기본적으로 초기화를 건너뛰면 일반적인 경우에 대한 페이지 로드가 간소화됩니다.
...
`ContentTransformerListener` 이제 기본적으로 비활성화됩니다.

#### 누가 영향을 받나요?

이는 `ContentTransformerListener`에서 제공하는 기존 웹 콘텐츠 기능(예: 다른 웹 콘텐츠 내에 웹 콘텐츠 포함, 레거시 편집 인프라, 토큰 교체(`@article_group_id@`, `@articleId;elementName@`))을 사용하는 Liferay Portal 설치에 영향을 미칩니다. ), 등.
...
`ContentTransformerListener` 기사 요소에 대한 비용이 많이 드는 문자열 처리(기사 필드에서 `HtmlUtil.stripComments` 및 `HtmlUtil.stripHtml` 호출)로 인해 성능 개선을 위해 비활성화되었습니다.
...
`Liferay.BrowserSelectors.run()` 함수는 더 이상 페이지에서 호출되지 않으며 결과적으로 여는 `<html>` 태그에서 일부 CSS 클래스가 제거됩니다. 이들 중 다수는 이제 대신 `<body>` 요소에 추가됩니다.

#### 누가 영향을 받나요?

이는 `<html>` 요소의 CSS 클래스에 의존하는 모든 코드에 영향을 미칩니다.
...
`Liferay.BrowserSelectors.run()` 함수를 직접 대체할 수는 없지만 대신 `<body>` 요소의 새 클래스를 대상으로 CSS 및 JavaScript를 조정할 수 있습니다. 이러한 클래스는 현재 사용 중인 브라우저를 반영하기 위해 `<body>` 요소에 추가됩니다.
...
예를 들어 다음 속성은 엔터티 `com.liferay.portal.kernel.model.User`에 대한 것입니다.
...
`ddm__keyword__` 및 `ddm__text__` 으로 시작하는 Elasticsearch의 동적 데이터 매핑 필드는 새로운 중첩 문서 `ddmFieldArray`로 이동되었습니다.

`ddmFieldArray` 다음 필드가 포함된 여러 항목이 있습니다.

- `ddmFieldName`: 동적 데이터 매핑 구조 필드 이름을 포함합니다. 이 이름은 `DDMIndexer.encodeName` 메소드를 사용하여 생성됩니다.
- `ddmFieldValue*`: 인덱싱된 데이터를 포함합니다. 이 필드의 이름은 `DDMIndexer.getValueFieldName` 사용하여 생성되며 필드의 데이터 유형 및 언어에 따라 달라집니다.
...
이는 `ddm__keyword__*` 및 `ddm__text__*` 필드를 사용하여 Elasticsearch 인덱스에서 쿼리를 실행하는 사용자 지정 개발을 수행하는 모든 사람에게 영향을 미칩니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

Elasticsearch 쿼리에 새 중첩 문서 `ddmFieldArray` 사용해야 합니다.
...
이는 Lexicon 아이콘 경로를 직접 사용하는 사용자 정의 솔루션에 영향을 미칩니다. `어휘` 경로에 아이콘을 구축하기 위한 Gradle 작업이 제거됩니다.

### 내 코드를 어떻게 업데이트해야 합니까?

`어휘`대신 참조 `클레이` 에 대한 경로 업데이트
...
`countByClassNameId`, `findByClassNameId`및 `RemoveByClassNameId` 메소드가 다음 클래스에서 제거되었습니다.
...
이 메서드 대신 `isShowAddButton(GroupscopeGroup)` 메서드를 사용할 수 있습니다.
...
`buffered.increment.enabled` 포털 속성이 제거되었습니다. 이제 조회수 페이지의 `활성화` 속성을 사용하여 조회수를 전역적으로 활성화 및 비활성화할 수 있습니다.

특정 엔터티에 대한 보기 수 동작 비활성화는 더 이상 포털 속성에서 수행되지 않습니다. 예를 들어 7.3에서 `view.count.enabled[SomeEntity]=false` 또는 7.2에서 `buffered.increment.enabled[SomeEntity]=false` 설정했지만 지금은 그렇습니다. 보기 수 페이지의 `비활성화된 클래스 이름` 값 목록에 엔터티 클래스 이름을 추가하면 됩니다.

#### 누가 영향을 받나요?

이는 포털 속성 설정이 `view.count.enabled=false` 또는 `buffered.increment.enabled=false`인 모든 사람에게 영향을 미칩니다.

이는 포털 속성 설정 `view.count.enabled[SomeEntity]=false` (초기 7.3 버전) 또는 `buffered.increment.enabled[SomeEntity]=false` (7.2 포털)을 사용하여 일부 엔터티(예: `SomeEntity`)에 대한 보기 수를 비활성화한 모든 사람에게 영향을 미칩니다.
...
포털 속성 `module.framework.properties.file.install.ionalImportRefreshScope` 이 제거되었습니다. 파일 설치는 이제 새로 고쳐야 하는 선택적 패키지가 포함된 번들을 검색할 때 관리되는 번들만 확인합니다.

#### 누가 영향을 받나요?

이는 포털 특성 설정이 `module.framework.properties.file.install.ionalImportRefreshScope`인 모든 사람에게 영향을 미칩니다.

#### 내 코드를 어떻게 업데이트해야 합니까?

속성 `module.framework.properties.file.install.ionalImportRefreshScope`제거합니다. 다른 동작을 사용하도록 파일 설치를 구성할 수 없습니다.
